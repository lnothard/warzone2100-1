...
RtrBestResult::RtrBestResult(RTR_DATA_TYPE type, BaseObject * obj)
...
Order::Order(ORDER_TYPE type, BaseObject & target)
  : type{type}, pos{0, 0}, pos2{0, 0}, direction{0}, index{0},
    rtrType{NO_RESULT}, target{&target}, structure_stats{nullptr}
{
}

Order::Order(ORDER_TYPE type, BaseObject & target, RTR_DATA_TYPE rtrType)
  : type{type}, pos{0, 0}, pos2{0, 0}, direction{0}, index{0},
    rtrType{rtrType}, target{&target}, structure_stats{nullptr}
{
}

Order::Order(ORDER_TYPE type, BaseObject & target, unsigned index)
...
Droid* checkForRepairRange(Droid* psDroid)
{
	...
	for (BaseObject * object : gridStartIterate(psDroid->getPosition().x,
...
static std::pair<Structure*, ACTION> checkForDamagedStruct(Droid* psDroid)
{
	...
	for (auto object : gridStartIterate(psDroid->getPosition().x, psDroid->getPosition().y, radius))
	{
		auto distanceSq = droidSqDist(psDroid, object);
		// droidSqDist returns -1 if unreachable, (unsigned)-1 is a big number
		...
		if (!structure || // must be a structure.
			  ...
			  !aiCheckAlliances(psDroid->playerManager->getPlayer(),
                          structure->playerManager->getPlayer()) || // must be a friendly structure
	...
	if (psData->type == ORDER_TYPE::RECOVER) {
		...
		for (auto psCurr : *psGroup->getMembers())
		{
          ...
          psCurr->getOrder()->type ==	ORDER_TYPE::RTR_SPECIFIED) {
			...
			auto const currdist = objectPositionSquareDiff(psCurr->getPosition(), psData->target->getPosition());
			if (currdist < mindist) {
				psChosen = psCurr;
				mindist = currdist;
			}
      ...
      if (psCurr->getType() == DROID_TYPE::SENSOR && psData->target) {
 ...
 */
static void orderPlayFireSupportAudio(BaseObject const* psObj)
{
  ...
  else if (auto psStruct = dynamic_cast<Structure const*>(psObj)) {
 ...
 */
bool orderStateLoc(Droid const* psDroid, ORDER_TYPE order, unsigned* pX, unsigned* pY)
{
	if (order != psDroid->getOrder()->type) return false;
...
Order chooseOrderObj(Droid* psDroid, BaseObject * psObj, bool altOrder)
{
  using enum ORDER_TYPE;
	...
	if (altOrder &&
      ...
      psDroid->playerManager->getPlayer() == psObj->playerManager->getPlayer()) {
  ...
  auto psFeat = dynamic_cast<Feature*>(psObj);
	...
	else if (psObj->playerManager->getPlayer() == psDroid->playerManager->getPlayer() &&
		       dynamic_cast<Droid*>(psObj) &&
	...
	// repair droid
	else if (aiCheckAlliances(psObj->playerManager->getPlayer(), psDroid->playerManager->getPlayer()) &&
           dynamic_cast<Droid*>(psObj) &&
	...
	// guarding constructor droids
        ...
        : -1);
			...
			if (intDemolishSelectMode() && psObj->playerManager->getPlayer() == psDroid->playerManager->getPlayer()) {
			...
			else if (psStruct->damageManager->getHp() < structureBody(psStruct)) {
		...
		if (order.type == NONE) {
			...
			if (psStruct->getStats()->type == STRUCTURE_TYPE::REPAIR_FACILITY &&
				...
				psStruct->damageManager->getResistance() < (int)structureResistance(
 ...
 */
static void orderPlayOrderObjAudio(unsigned player, BaseObject * psObj)
{
	ASSERT_PLAYER_OR_RETURN(, player);
	...
	{
    if (!psDroid.damageManager->isSelected()) continue;
...
Droid* FindATransporter(Droid* embarkee)
{
	auto isCyborg_ = isCyborg(embarkee);
	Droid* bestDroid = nullptr;
	auto bestDist = ~0u;

	for (auto& psDroid : playerList[embarkee->playerManager->getPlayer()].droids)
	{
  ...
  using enum SECONDARY_ORDER;
	...
	case FIRE_DESIGNATOR:
		...
		if ((sec == ASSIGN_PRODUCTION &&
         FindAFactory(psDroid->playerManager->getPlayer(),
    ...
    // don't show the range levels if the droid doesn't have a weapon with different ranges
		if (numWeapons(*psDroid) > 0) {
			...
			{
				const auto weaponStats = psDroid->weaponManager->weapons[i].stats.get();

				if (proj_GetLongRange(weaponStats, psDroid->playerManager->getPlayer()) ==
              proj_GetShortRange(weaponStats, psDroid->playerManager->getPlayer())) {
					supported = false;
				}
				else {
					supported = true;
	...
	case RECYCLE: // only if player has got a factory.
		if ((FindAFactory(psDroid->playerManager->getPlayer(),
                      STRUCTURE_TYPE::FACTORY) == nullptr) &&
		  	(FindAFactory(psDroid->playerManager->getPlayer(),
                    STRUCTURE_TYPE::CYBORG_FACTORY) == nullptr) &&
		  	(FindAFactory(psDroid->playerManager->getPlayer(),
                    STRUCTURE_TYPE::VTOL_FACTORY) == nullptr) &&
		  	(FindARepairFacility(psDroid->playerManager->getPlayer()) == nullptr)) {
			supported = false;
		}
		break;
	default:
...
static bool secondaryCheckDamageLevelDeselect(Droid* psDroid, SECONDARY_STATE repairState)
{
  ...
  if (psDroid->damageManager->getHp() * 100 > repairLevel * psDroid->damageManager->getOriginalHp()) {
    return false;
  }
  // only deselect the droid if there is another droid selected.
  if (!psDroid->damageManager->isSelected() || selectedPlayer >= MAX_PLAYERS) {
    return true;
  }
  for (auto& psTempDroid : playerList[selectedPlayer].droids)
  {
    if (&psTempDroid != psDroid && psTempDroid.damageManager->isSelected()) {
      DeSelectDroid(psDroid);
      break;
    }
  }
