...
void WzGameFindTitleUI::start()
{
	...
	botForm->setCalcLayout(LAMBDA_CALCLAYOUT_SIMPLE({
			psWidget->setGeometry(MULTIOP_OPTIONSX, MULTIOP_OPTIONSY, MULTIOP_CHATBOXW, 415);
			// FIXME: Add box at bottom for server messages
			}));

	addSideText(FRONTEND_SIDETEXT, MULTIOP_OPTIONSX - 3, MULTIOP_OPTIONSY, _("GAMES"));

	// cancel
	addMultiBut(psWScreen, FRONTEND_BOTFORM, CON_CANCEL, 10, 5, MULTIOP_OKW, MULTIOP_OKH,
	            _("Return To Previous Screen"),
	            IMAGE_RETURN, IMAGE_RETURN_HI, IMAGE_RETURN_HI);

	//refresh
	addMultiBut(psWScreen, FRONTEND_BOTFORM, MULTIOP_REFRESH, MULTIOP_CHATBOXW - MULTIOP_OKW - 5, 5, MULTIOP_OKW,
	            MULTIOP_OKH,
	            _("Refresh Games List"), IMAGE_RELOAD_HI, IMAGE_RELOAD, IMAGE_RELOAD);
	//filter toggle
	addMultiBut(psWScreen, FRONTEND_BOTFORM, MULTIOP_FILTER_TOGGLE, MULTIOP_CHATBOXW - MULTIOP_OKW - 45, 5, MULTIOP_OKW,
	            MULTIOP_OKH,
...
TITLECODE WzGameFindTitleUI::run()
{
	...
	bool handleUserRefreshRequest = queuedRefreshOfGamesList && ((realTime - lastFetchRealTime) >= (
		GAME_TICKS_PER_SEC));
	...
	if (id == MULTIOP_REFRESH || id == MULTIOP_FILTER_TOGGLE)
	{
		if (id == MULTIOP_FILTER_TOGGLE)
		{
			...
			toggleFilter
				? widgSetButtonState(psWScreen, MULTIOP_FILTER_TOGGLE, WBUT_CLICKLOCK)
...
public:
	void display(int xOffset, int yOffset) override
	{
		...
		remoteGameListHeaderCache.wzHeaderText_GameName.render(xPos - 2 + GAMES_GAMENAME_START + 48, yPos + 9,
		                                                       WZCOL_YELLOW);

		remoteGameListHeaderCache.wzHeaderText_MapName.setText(_("Map Name"), font_small);
		remoteGameListHeaderCache.wzHeaderText_MapName.render(xPos - 2 + GAMES_MAPNAME_START + 48, yPos + 9,
		                                                      WZCOL_YELLOW);
		...
		remoteGameListHeaderCache.wzHeaderText_Status.
		                          render(xPos - 2 + GAMES_STATUS_START + 48, yPos + 9, WZCOL_YELLOW);
	}
...
static bool joinLobbyGame(uint32_t gameNumber, bool asSpectator = false)
{
	...
	std::vector<JoinConnectionDescription> connectionDesc = {
		JoinConnectionDescription(gamesList[gameNumber].desc.host, gamesList[gameNumber].hostPort)
	};
	ActivityManager::instance().willAttemptToJoinLobbyGame(NETgetMasterserverName(), NETgetMasterserverPort(),
	                                                       gamesList[gameNumber].gameId, connectionDesc);
	...
	static std::shared_ptr<WzLobbyGameWidget> make(UDWORD gameIdx)
	{
		...
		if (NETisCorrectVersion(gamesList[gameIdx].game_version_major, gamesList[gameIdx].game_version_minor))
		{
			...
			if (spectatorInfo.availableSpectatorSlots() > 0)
			{
				...
				ASSERT_OR_RETURN(nullptr, widget->spectateButton != nullptr,
				                 "Failed to create WzLobbyGameSpectateButton");
				widget->attach(widget->spectateButton);
				widget->spectateButton->setGeometry(gameDetailsX1 + 2, 0, widget->spectateButton->idealWidth(),
				                                    GAMES_GAMEHEIGHT);
