...
enum SECONDARY_ORDER
{
	...
	DSO_CIRCLE,
	/**< circling target position and engage. */
};
...
struct DroidOrder
{
	explicit DroidOrder(DroidOrderType type = DORDER_NONE)
		: type(type), pos(0, 0), pos2(0, 0), direction(0), index(0), rtrType(RTR_TYPE_NO_RESULT), psObj(nullptr),
		  psStats(nullptr)
	{
	}

	DroidOrder(DroidOrderType type, Vector2i pos)
		: type(type), pos(pos), pos2(0, 0), direction(0), index(0), rtrType(RTR_TYPE_NO_RESULT), psObj(nullptr),
		  psStats(nullptr)
	{
	}
	...
	DroidOrder(DroidOrderType type, BASE_OBJECT* psObj)
		: type(type), pos(0, 0), pos2(0, 0), direction(0), index(0), rtrType(RTR_TYPE_NO_RESULT), psObj(psObj),
		  psStats(nullptr)
	{
	}
	...
	DroidOrder(DroidOrderType type, BASE_OBJECT* psObj, uint32_t index)
		: type(type), pos(0, 0), pos2(0, 0), direction(0), index(index), rtrType(RTR_TYPE_NO_RESULT), psObj(psObj),
		  psStats(nullptr)
	{
	}
