...
static std::vector<WavecastTile> generateWavecastTable(unsigned radius)
{
	...
	for (unsigned diamond = 1; diamond * TILE_UNITS < radius * 2; ++diamond)
	// Factor is a bit more than needed to surround the circle with diamonds.
	{
...
#pragma warning( disable : 4146 ) // warning C4146: unary minus operator applied to unsigned type, result still unsigned
				...
				switch (quadrant)
				{
				case 0: tile.dx = diamond - s;
					tile.dy = s + 1;
					break;
				case 1: tile.dx = - s;
					tile.dy = diamond - s;
					break;
				case 2: tile.dx = -diamond + s + 1;
					tile.dy = - s;
					break;
				case 3: tile.dx = s + 1;
					tile.dy = -diamond + s + 1;
...
#pragma warning( pop )
				...
				tile.invRadius = i64Sqrt((uint64_t)2 * 65536 * 65536 / tileRadiusSq);
				// Result is at most 65536, inversely proportional to the radius.

				const int minCorner[4][2] = {{1, 0}, {1, 1}, {0, 1}, {0, 0}};
				const int mcdx = minCorner[quadrant][0], mcdy = minCorner[quadrant][1];
				// Corner of the tile which the minimum angle.
				RationalAngle minAngle = RationalAngle(tile.dx - 1 + mcdx, tile.dy - 1 + mcdy), maxAngle =
					              RationalAngle(tile.dx - mcdx, tile.dy - mcdy);
				if (maxAngle < minAngle)
				{
					maxAngle = RationalAngle(0, 0);
					// Special case, like RationalAngle(1, 0), except that it compares greater than all other angles instead of less than all other angles.
				}
	...
	for (auto& i : tiles)
	{
		i.angBegin = std::lower_bound(sortedAngles.begin(), sortedAngles.end(), unsortedAngles[i.angBegin]) -
			sortedAngles.begin();
		i.angEnd = std::lower_bound(sortedAngles.begin(), sortedAngles.end(), unsortedAngles[i.angEnd]) - sortedAngles.
			begin();
	}
