...
void gridReset()
{
	...
	{
          GameObject *start[3] = {(GameObject *)allDroidLists[player], (GameObject *)apsStructLists[player], (GameObject *)apsFeatureLists[player]};
		for (unsigned type = 0; type != sizeof(start) / sizeof(*start); ++type)
		{
			for (GameObject *psObj = start[type]; psObj != nullptr; psObj = psObj->psNext)
			{
				if (!psObj->deathTime)
				{
					gridPointTree->insert(psObj, psObj->position.x, psObj->position.y);
...
static GridList const &gridStartIterateFiltered(int32_t x, int32_t y, uint32_t radius, PointTree::Filter *filter, Condition const &condition)
{
	...
	for (i = w; i != gridPointTree->lastQueryResults.end(); ++i)
	{
		...
		else if (isInRadius(obj->position.x - x, obj->position.y - y, radius))  // Check that search result is less than radius (since they can be up to a factor of sqrt(2) more).
		{
			*w = *i;
			++w;
		}
	...
	*/
	...
	{
		gridList[n] = (GameObject *)gridPointTree->lastQueryResults[n];
	}
	return gridList;
}
...
struct ConditionDroidsByPlayer
{
	...
	bool test(GameObject *obj) const
	{
		return obj->type == OBJ_DROID && obj->owningPlayer == player;
	}
	int player;
};
...
GameObject **gridIterateDup()
{
	size_t bytes = gridPointTree->lastQueryResults.size() * sizeof(void *);
        GameObject **ret = (GameObject **)malloc(bytes);
