...
void scripting_engine::GROUPMAP::insertObjectIntoGroup(const BaseObject* psObj,
                                                       scripting_engine::GROUPMAP::groupID groupId)
{
	std::pair<ObjectToGroupMap::iterator, bool> result = m_map.insert(
		std::pair<const BaseObject *, scripting_engine::GROUPMAP::groupID>(psObj, groupId));
	if (result.second)
	{
...
std::vector<const BaseObject *> scripting_engine::GROUPMAP::getGroupObjects(groupID groupId) const
{
	std::vector<const BaseObject *> result;
	auto it = m_groups.find(groupId);
	if (it != m_groups.end())
	{
...
scripting_engine::timerNode::~timerNode()
{
	...
	{
		additionalTimerFuncParam->onTimerDelete(timerID, IdToObject(baseobj, player));
	}
}
...
uniqueTimerID scripting_engine::setTimer(wzapi::scripting_instance* caller, const TimerFunc& timerFunc, unsigned player,
                                         ...
                                         const BaseObject * obj /*= nullptr*/, timerType type /*= TIMER_REPEAT*/,
                                         std::unique_ptr<timerAdditionalData> additionalParam /*= nullptr*/)
{
	uniqueTimerID newTimerID = getNextAvailableTimerID();
	std::shared_ptr<timerNode> node = std::make_shared<timerNode>(caller, timerFunc, timerName, player, milliseconds,
...
{
	ASSERT(timerIDMap.count(node->timerID) == 0, "Duplicate timerID found: %s",
	       WzString::number(node->timerID).toUtf8().c_str());
	auto inserted_iter = timers.emplace(timers.end(), std::move(node));
...
bool scripting_engine::updateScripts()
{
	...
	// TODO - load balancing
	std::vector<std::shared_ptr<timerNode>> runlist;
	// make a new list here, since we might trample all over the timer list during execution
	...
	for (auto& node : runlist)
	{
		...
		node->function(node->timerID, IdToObject(node->baseobj, node->player),
		               node->additionalTimerFuncParam.get());
	}
...
wzapi::scripting_instance* scripting_engine::loadPlayerScript(const WzString& path, unsigned player, AIDifficulty difficulty)
{
	...
	if (!pNewInstance)
	{
		...
		debug(LOG_ERROR, "Failed to create new scripting instance: path:\"%s\", player: %d, difficulty: %d",
		      path.toUtf8().c_str(), player, static_cast<int>(difficulty));
		return nullptr;
	}
	...
	pNewInstance->setSpecifiedGlobalVariable("gameTime", gameTime,
	...
	pNewInstance->setSpecifiedGlobalVariables(
		globalVars, wzapi::GlobalVariableFlags::ReadOnly | wzapi::GlobalVariableFlags::DoNotSave);
	...
	pNewInstance->setSpecifiedGlobalVariable("Stats", wzapi::constructStatsObject(),
	                                         wzapi::GlobalVariableFlags::ReadOnly |
	                                         wzapi::GlobalVariableFlags::DoNotSave);

	// Register 'MapTiles' two-dimensional array. It is a read-only representation of static map tile states.
	pNewInstance->setSpecifiedGlobalVariable("MapTiles", wzapi::constructMapTilesArray(),
	                                         wzapi::GlobalVariableFlags::ReadOnly |
	                                         wzapi::GlobalVariableFlags::DoNotSave);

	// Set some useful constants
	pNewInstance->setSpecifiedGlobalVariables(wzapi::getUsefulConstants(),
	                                          wzapi::GlobalVariableFlags::ReadOnly |
	                                          wzapi::GlobalVariableFlags::DoNotSave);
	...
	//== be undefined.
	pNewInstance->setSpecifiedGlobalVariable("groupSizes", nlohmann::json::object(),
	                                         wzapi::GlobalVariableFlags::ReadOnly |
	                                         wzapi::GlobalVariableFlags::DoNotSave);

	// Static knowledge about players
	pNewInstance->setSpecifiedGlobalVariable("playerData", wzapi::constructStaticPlayerData(),
	                                         wzapi::GlobalVariableFlags::ReadOnly |
	                                         wzapi::GlobalVariableFlags::DoNotSave);

	// Static map knowledge about start positions
	pNewInstance->setSpecifiedGlobalVariable("derrickPositions", constructDerrickPositions(),
	                                         wzapi::GlobalVariableFlags::ReadOnly |
	                                         wzapi::GlobalVariableFlags::DoNotSave);
	pNewInstance->setSpecifiedGlobalVariable("startPositions", constructStartPositions(),
	                                         wzapi::GlobalVariableFlags::ReadOnly |
	                                         wzapi::GlobalVariableFlags::DoNotSave);
...
bool scripting_engine::saveScriptStates(const char* filename)
{
	...
	for (const auto& node : timers)
	{
		...
		nodeInfo["functionRestoreInfo"] = node->instance->saveTimerFunction(
		...
		if (instance && list[i].startsWith("triggers_"))
		{
			...
			try
			{
				...
				else
				{
					...
					{
						ASSERT(false,
						       "Invalid trigger in save (%s) - missing new functionRestoreInfo block, and old function parameter",
						       list[i].toUtf8().c_str());
						continue;
					}
...
//__
bool triggerEvent(SCRIPT_TRIGGER_TYPE trigger, BaseObject* psObj)
{
	...
	for (auto* instance : scripts)
	{
		if (psObj) {
			...
			if (player != psObj->playerManager->getPlayer() && !receiveAll) {
				continue;
			}
		}
...
//__
bool triggerEventDroidIdle(Droid const* psDroid)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	for (auto* instance : scripts)
	{
		unsigned player = instance->player();
		if (player == psDroid->playerManager->getPlayer()) {
...
bool triggerEventDroidBuilt(Droid const* psDroid, Structure const* psFactory)
{
	...
	for (auto* instance : scripts)
	{
		...
		bool receiveAll = instance->isReceivingAllEvents();
		if (player == psDroid->playerManager->getPlayer() || receiveAll) {
...
//__
bool triggerEventStructBuilt(Structure const* psStruct, Droid const* psDroid)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	...
	for (auto* instance : scripts)
	{
		auto player = instance->player();
		bool receiveAll = instance->isReceivingAllEvents();
		if (player == psStruct->playerManager->getPlayer() || receiveAll) {
...
//__
bool triggerEventStructDemolish(Structure const* psStruct, Droid const* psDroid)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	optional<const Droid*> opt_droid = (psDroid) ? optional<const Droid*>(psDroid) : nullopt;
	for (auto* instance : scripts)
...
//__
bool triggerEventStructureReady(Structure const* psStruct)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	for (auto* instance : scripts)
	{
...
//__
bool triggerEventStructureUpgradeStarted(Structure const* psStruct)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	for (auto* instance : scripts)
	{
...
//__
bool triggerEventAttacked(BaseObject const* psVictim, BaseObject const* psAttacker, int lastHit)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	if (!psAttacker) {
	...
	for (auto* instance : scripts)
	{
		auto player = instance->player();
		bool receiveAll = instance->isReceivingAllEvents();
		if (player == psVictim->playerManager->getPlayer() || receiveAll) {
...
//__
bool triggerEventResearched(ResearchStats* psResearch, Structure const* psStruct, unsigned player)
{
...
//__
bool triggerEventDestroyed(BaseObject * psVictim)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
...
//__ being seen.
//__
bool triggerEventSeen(BaseObject * psViewer, BaseObject * psSeen)
{
	return scripting_engine::instance().triggerEventSeen(psViewer, psSeen);
}

bool scripting_engine::triggerEventSeen(BaseObject * psViewer, BaseObject * psSeen)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
...
//__
bool triggerEventObjectTransfer(BaseObject * psObj, unsigned from)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	...
	for (auto* instance : scripts)
	{
		int me = instance->player();
		bool receiveAll = instance->isReceivingAllEvents();
		if (me == psObj->playerManager->getPlayer() || me == from || receiveAll) {
...
bool triggerEventGroupLoss(const BaseObject * psObj, int group, int size, wzapi::scripting_instance* instance)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
...
//__ cheating!
//__
bool triggerEventSyncRequest(int from, int req_id, int x, int y, BaseObject * psObj, BaseObject * psObj2)
{
	ASSERT(scriptsReady, "Scripts not initialized yet");
	for (auto* instance : scripts)
	{
...
std::vector<scripting_engine::LabelInfo> scripting_engine::debug_GetLabelInfo() const
{
	...
	for (const auto& label : labels)
	{
		...
		case SCRIPT_POSITION: c = "POSITION";
			break;
		case SCRIPT_AREA: c = "AREA";
			break;
		case SCRIPT_RADIUS: c = "RADIUS";
			break;
		case SCRIPT_GROUP: c = "GROUP";
			break;
		case SCRIPT_PLAYER:
		...
		switch (l.triggered)
		{
		case -1: labelInfo.trigger = "N/A";
			break;
		case 0: labelInfo.trigger = "Active";
			break;
		default: labelInfo.trigger = "Done";
			break;
		}
...
void scripting_engine::showLabel(const std::string& key, bool clear_old, bool jump_to)
{
	...
	if (l.type == SCRIPT_AREA || l.type == SCRIPT_POSITION)
	{
		...
		{
			for (int y = MAX(map_coord(l.p1.y - l.p2.x), 0); y < MIN(map_coord(l.p1.y + l.p2.x), mapHeight); y++)
			// l.p2.x is radius, not a bug
			{
		...
		auto psObj = IdToObject(l.id, l.player);
		if (psObj) 	{
			if (jump_to) {
				setViewPos(map_coord(psObj->getPosition().x), map_coord(psObj->getPosition().y), false); // move camera position
			}
			auto psTile = mapTile(map_coord(psObj->getPosition().x), map_coord(psObj->getPosition().y));
			psTile->tileInfoBits |= BITS_MARKED;
		}
	}
		...
		for (auto & group : groups)
		{
			...
			{
				if (iter.second == l.id) {
					const auto psObj = iter.first;
					if (!cameraMoved && jump_to) {
						setViewPos(map_coord(psObj->getPosition().x),
                       map_coord(psObj->getPosition().y), false); // move camera position
						cameraMoved = true;
					}
					auto psTile = mapTile(map_coord(psObj->getPosition().x), map_coord(psObj->getPosition().y));
					psTile->tileInfoBits |= BITS_MARKED;
...
std::pair<bool, int> scripting_engine::seenLabelCheck(wzapi::scripting_instance* instance, const BaseObject * seen,
                                                      const BaseObject * viewer)
{
	GROUPMAP* psMap = getGroupMap(instance);
	...
	{
		LABEL& l = it.second;
		if (l.triggered != 0 || !(l.subscriber == ALL_PLAYERS || l.subscriber == viewer->playerManager->getPlayer()))
		{
			continue;
		}
		...
		if (l.type != SCRIPT_GROUP && l.id == seen->getId())
		{
			l.triggered = viewer->getId(); // record who made the discovery
			foundObj = true;
		}
...
bool scripting_engine::areaLabelCheck(Droid* psDroid)
{
	...
	auto y = psDroid->getPosition().y;
	bool activated = false;
	for (auto & label : labels)
	{
		LABEL& l = label.second;
		if (l.triggered == 0 && (l.subscriber == ALL_PLAYERS || l.subscriber == psDroid->playerManager->getPlayer())
			&& ((l.type == SCRIPT_AREA && l.p1.x < x && l.p1.y < y && l.p2.x > x && l.p2.y > y)
				|| (l.type == SCRIPT_RADIUS && iHypot(l.p1 - psDroid->getPosition().xy()) < l.p2.x)))
		{
			// We're inside an untriggered area
			activated = true;
			l.triggered = psDroid->getId();
...
void scripting_engine::removeFromGroup(wzapi::scripting_instance* instance, GROUPMAP* psMap, const BaseObject * psObj)
{
	auto result = psMap->removeObjectFromGroup(psObj);
	if (result.has_value()) {
		auto groupId = result.value();
		const auto newGroupSize = psMap->groupSize(groupId);
		instance->updateGroupSizes(groupId, newGroupSize);
		triggerEventGroupLoss(psObj, groupId, newGroupSize, instance);
	}
}

void scripting_engine::groupRemoveObject(const BaseObject * psObj)
{
	for (auto & group : groups)
	{
...
bool scripting_engine::groupAddObject(const BaseObject * psObj, int groupId, wzapi::scripting_instance* instance)
{
	ASSERT_OR_RETURN(false, psObj && instance, "Bad parameter");
	auto psMap = getGroupMap(instance);
...
{
	auto psObj = IdToPointer(objId, ANYPLAYER);
...
bool scripting_engine::saveGroups(nlohmann::json& result, wzapi::scripting_instance* instance)
{
	...
	{
		const BaseObject * psObj = i.first;
		ASSERT(!psObj->damageManager->isDead(), "Wanted to save dead %s to savegame!", objInfo(psObj));
		std::vector<WzString> value = json_getValue(result, WzString::number(psObj->getId())).toWzStringList();
		value.push_back(WzString::number(i.second));
		result[WzString::number(psObj->getId()).toUtf8()] = value;
	}
	return true;
}
...
bool scripting_engine::loadLabels(const char* filename)
{
	...
	for (auto& i : list)
	{
		...
		else if (i.startsWith("group"))
		{
			...
			{
				int id = j.toInt();
        BaseObject * psObj = IdToPointer(id, p.player);
...
//--
wzapi::no_return_value scripting_engine::resetLabel(WZAPI_PARAMS(const std::string& labelName, optional<int> playerFilter))
{
...
generic_script_object generic_script_object::fromObject(const BaseObject * psObj)
{
	generic_script_object result;
	if (psObj == nullptr)
	{
	...
	result.id = psObj->getId();
	result.player = psObj->playerManager->getPlayer();
	return result;
}

BaseObject * generic_script_object::getObject() const
...
//--
wzapi::no_return_value scripting_engine::addLabel(
	WZAPI_PARAMS(generic_script_object object, const std::string& label, optional<int> _triggered))
{
...
//--
int scripting_engine::removeLabel(WZAPI_PARAMS(const std::string& label))
{
	LABELMAP& labels = scripting_engine::instance().labels;
...
//--
optional<std::string> scripting_engine::getLabel(WZAPI_PARAMS(const BaseObject *psObj))
{
...
generic_script_object scripting_engine::getObject(WZAPI_PARAMS(const wzapi::object_request& request))
{
	...
	else if (request.requestType == wzapi::object_request::RequestType::OBJECTID_REQUEST)
	{
		...
		return generic_script_object::fromObject(IdToObject(id, player));
	}
  ...
  BaseObject * psObj = nullptr;
	if (labels.count(label) > 0)
	{
		...
		case SCRIPT_RADIUS:
			...
			break;
		case SCRIPT_AREA:
			return generic_script_object::fromArea(map_coord(p.p1.x), map_coord(p.p1.y), map_coord(p.p2.x),
			                                       map_coord(p.p2.y));
			break;
		...
		case OBJ_FEATURE:
		case OBJ_STRUCTURE:
			psObj = IdToObject(p.id, p.player);
			if (!psObj)
			{
				return generic_script_object::Null();
			}
...
//--
std::vector<const BaseObject *> scripting_engine::enumAreaByLabel(
...
std::vector<const BaseObject *> scripting_engine::enumArea(
...
std::vector<const BaseObject *> scripting_engine::_enumAreaWorldCoords(
...
std::vector<const BaseObject *> scripting_engine::enumAreaJS(WZAPI_PARAMS(
...
//--
std::vector<const BaseObject *> scripting_engine::enumGroup(WZAPI_PARAMS(int groupId))
{
	std::vector<const BaseObject *> matches;
...
wzapi::no_return_value scripting_engine::groupAddArea(WZAPI_PARAMS(int groupId, int _x1, int _y1, int _x2, int _y2))
{
	...
	auto x1 = world_coord(_x1);
	auto y1 = world_coord(_y1);
...
//--
wzapi::no_return_value scripting_engine::groupAddDroid(WZAPI_PARAMS(int groupId, Droid const* psDroid))
{
...
//--
wzapi::no_return_value scripting_engine::groupAdd(WZAPI_PARAMS(int groupId, const BaseObject *psObj))
{
	...
	if (l.type == SCRIPT_AREA) {
		...
		for (int x = map_coord(l.p1.x - l.p2.x); x < map_coord(l.p1.x + l.p2.x); x++)
		{
			for (int y = map_coord(l.p1.y - l.p2.x); y < map_coord(l.p1.y + l.p2.x); y++)
			{
				if (x >= -1 && x < mapWidth + 1 &&
            y >= -1 && y < mapWidth + 1 &&
            iHypot(map_coord(l.p1) -
              Vector2i(x, y)) < map_coord(l.p2.x)) {
					auto psTile = mapTile(x, y);
					psTile->tileInfoBits |= BITS_MARKED;
				}
			}
...
void scripting_engine::logFunctionPerformance(wzapi::scripting_instance* instance, const std::string& function,
                                              int ticks)
{
	auto monitor = monitors.at(instance); // pick right one for this instance
	MONITOR_BIN m;
...
std::unordered_map<wzapi::scripting_instance*, nlohmann::json>
scripting_engine::DebugInterface::debug_GetGlobalsSnapshot()
{
	return scripting_engine::instance().debug_GetGlobalsSnapshot();
}

std::vector<scripting_engine::timerNodeSnapshot> scripting_engine::DebugInterface::debug_GetTimersSnapshot()
{
	return scripting_engine::instance().debug_GetTimersSnapshot();
}

std::vector<scripting_engine::LabelInfo> scripting_engine::DebugInterface::debug_GetLabelInfo()
{
...
/// Mark and show label
void scripting_engine::DebugInterface::showLabel(const std::string& key, bool clear_old /*= true*/,
                                                 bool jump_to /*= true*/)
{
