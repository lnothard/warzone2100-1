...
static bool serializeMultiplayerGame(PHYSFS_file* fileHandle, const MULTIPLAYERGAME* serializeMulti)
{
	...
	if (!PHYSFS_writeUBE8(fileHandle, static_cast<uint8_t>(serializeMulti->type))
		...
		|| !PHYSFS_writeUBE8(fileHandle, (int)serializeMulti->alliance)
...
static bool deserializeMultiplayerGame(PHYSFS_file* fileHandle, MULTIPLAYERGAME* serializeMulti)
{
	...
	if (!PHYSFS_readUBE8(fileHandle, reinterpret_cast<uint8_t*>(&serializeMulti->type))
		...
		|| !PHYSFS_readUBE8(fileHandle, (uint8_t*)&serializeMulti->alliance)
...
static bool serializePlayer(PHYSFS_file* fileHandle, PLAYER const* serializePlayer, unsigned player)
{
...
{
	uint32_t gameTime;
	uint32_t GameType; /* Type of game , one of the GAME_TYPE::GTYPE_... enums. */
...
static void loadSaveStructLimits(const char* pFileName);
static void writeStructLimitsFile(const char* pFileName);

static bool readFiresupportDesignators(const char* pFileName);
static void writeFiresupportDesignators(const char* pFileName);
...
bool loadMissionExtras(const char* pGameToLoad, LEVEL_TYPE levelType)
{
  ...
  if (saveGameVersion < VERSION_11) return true;

  //if user save game then load up the messages AFTER any droids or structures are loaded
  if (gameType == GAME_TYPE::GTYPE_SAVE_START || gameType == GAME_TYPE::GTYPE_SAVE_MIDMISSION) {
...
static void getIniBaseObject(WzConfig& ini, WzString const& key, BaseObject *& object)
{
	object = nullptr;
...
static void getIniStructureStats(WzConfig& ini, WzString const& key, StructureStats*& stats)
{
	stats = nullptr;
	if (ini.contains(key))
	{
...
{
	order.type = (ORDER_TYPE)ini.value(key + "/type", (int)ORDER_TYPE::NONE).toInt();
...
static void setIniBaseObject(nlohmann::json& json, WzString const& key, BaseObject const* object)
{
	if (object != nullptr && object->damageManager->getTimeOfDeath() <= 1)
	{
		...
		json[keyStr + "/type"] = getObjectType(object);
#ifdef DEBUG
...
static inline void setIniStructureStats(nlohmann::json& jsonObj, WzString const& key, StructureStats const* stats)
{
	if (stats != nullptr)
	{
...
bool loadGame(const char* pGameToLoad, bool keepObjects, bool freeMem, bool UserSaveGame)
{
	...
	if ((gameType == GAME_TYPE::GTYPE_SAVE_START) ||
		(gameType == GAME_TYPE::GTYPE_SAVE_MIDMISSION))
	{
	...
	if (gameType != GAME_TYPE::GTYPE_SCENARIO_EXPAND || UserSaveGame)
	{
		...
		for (player = 0; player < MAX_PLAYERS; ++player)
		{
			for (auto& psCurr : playerList[player].droids)
			{
				if (psCurr.getType() != DROID_TYPE::PERSON
					...
					&& psCurr.getPosition().x != INVALID_XY) {
	...
	if (gameType != GAME_TYPE::GTYPE_SCENARIO_EXPAND)
	{
		psMapTiles = nullptr;
		// load in the map file
		if (!data)
	...
	if (gameType == GAME_TYPE::GTYPE_SAVE_MIDMISSION && missionLimboExpand())
	{
	...
	if (gameType == GAME_TYPE::GTYPE_SAVE_MIDMISSION)
	{
...
bool saveGame(const char* aFileName, GAME_TYPE saveType)
{
	...
	/*Write the data to the file*/
	writeStructLimitsFile(CurrentFileName);
	...
	if (saveGameVersion < VERSION_25) {
		ASSERT(selectedPlayer < MAX_PLAYERS, "selectedPlayer is out of bounds: %" PRIu32 "", selectedPlayer);
		for (auto psDroid : apsLimboDroids[selectedPlayer])
		{
...
bool gameLoadV(PHYSFS_file* fileHandle, unsigned int version, nonstd::optional<nlohmann::json>& gamJson)
{
	...
	clearPlayerPower();
	//don't adjust any power if a camStart (gameType is set to GAME_TYPE::GTYPE_SCENARIO_START when a camChange saveGame is loaded)
	if (gameType != GAME_TYPE::GTYPE_SCENARIO_START)
	{
		//set the players power
		for (i = 0; i < MAX_PLAYERS; ++i)
		{
			//only overwrite selectedPlayer's power on a startMission save game
			if (gameType == GAME_TYPE::GTYPE_SAVE_MIDMISSION || i == selectedPlayer)
			{
...
static bool writeMainFile(const std::string& fileName, SDWORD saveType)
{
	ASSERT(saveType == GAME_TYPE::GTYPE_SAVE_START || saveType == GAME_TYPE::GTYPE_SAVE_MIDMISSION, "invalid save type");
...
//-- Write gam.json
//--
static bool writeGameFile(const char* fileName, GAME_TYPE saveType)
{
	GAME_SAVEHEADER fileHeader;
	SAVE_GAME saveGame;
	unsigned int i, j;
	...
	ASSERT(saveType == GAME_TYPE::GTYPE_SAVE_START ||
         saveType == GAME_TYPE::GTYPE_SAVE_MIDMISSION, "invalid save type");
...
static bool loadWzMapDroidInit(WzMap::Map& wzMap)
{
	...
	{
		unsigned player = RemapWzMapPlayerNumber(droid.playerManager->getPlayer());
		if (player >= MAX_PLAYERS)
		{
			player = MAX_PLAYERS - 1; // now don't lose any droids ... force them to be the last player
			NumberOfSkippedDroids++;
		}
		auto psTemplate = getTemplateFromTranslatedNameNoPlayer(droid.getName().c_str());
		if (psTemplate == nullptr)
		{
			debug(LOG_ERROR, "Unable to find template for %s for player %d -- unit skipped", droid.getName().c_str(),
			      player);
			continue;
		}
		turnOffMultiMsg(true);
		auto psDroid = reallyBuildDroid(psTemplate,
                                    Position(droid.getPosition().x,
                                             droid.getPosition().y, 0),
                                    player, false, {droid.getRotation().direction, 0, 0});
		turnOffMultiMsg(false);
		if (psDroid == nullptr) {
			debug(LOG_ERROR, "Failed to build unit %s", droid.getName().c_str());
			continue;
		}
		if (droid.getId()) {
			psDroid->setId(droid.getId() > 0 ? droid.getId() : 0xFEDBCA98); // hack to remove droid id zero
		}
		ASSERT(psDroid->getId() != 0, "Droid ID should never be zero here");
...
bool skipForDifficulty(WzConfig& ini, unsigned player)
{
  ...
  auto difficulty = ini.value("difficulty").toInt();
...
static bool loadSaveDroidPointers(const WzString& pFileName)
{
	...
	for (auto i = 0; i < list.size(); ++i)
	{
		...
		for (auto& psDroid : playerList[player].droids)
		{
      if (psDroid.getId() == id) break;

      // Check for droids in the transporter.
			if (isTransporter(psDroid) && psDroid.getGroup() != nullptr) {
				for (Droid* psTrDroid = psDroid->group->members; psTrDroid != nullptr; psTrDroid = psTrDroid->psGrpNext)
				{
	...
	foundDroid:
      ...
      BaseObject * psObj = getBaseObjFromData(tid, tplayer, ttype);
		...
		if (ini.contains("commander"))
		{
			...
			Droid* psCommander = (Droid*)getBaseObjFromData(tid, psDroid->playerManager->getPlayer(), OBJ_DROID);
			ASSERT(psCommander, "Failed to find droid commander");
			cmdDroidAddDroid(psCommander, psDroid);
		}
		getIniDroidOrder(ini, "order", psDroid->getOrder());
		ini.endGroup();
	}
	return true;
}
...
static void loadSaveObject(WzConfig& ini, BaseObject * psObj)
{
	psObj->damageManager->setTimeOfDeath(ini.value("died", 0).toInt());
	for (auto j = 0; j < game.maxPlayers; j++)
	{
		psObj->setVisibleToPlayer(j, ini.value("visible/" + WzString::number(j), 0).toInt());
	}
	psObj->damageManager->setPeriodicalDamage(ini.value("periodicalDamage", 0).toInt());
	psObj->damageManager->setPeriodicalDamageStartTime(ini.value("periodicalDamageStart", 0).toInt());
	psObj->timeAnimationStarted = ini.value("timeAnimationStarted", 0).toInt();
	psObj->animationEvent = ini.value("animationEvent", 0).toInt();
	psObj->damageManager->setTimeLastHit(ini.value("timeLastHit", UDWORD_MAX).toInt());
...
{
	ini.setValue("id", psObj->getId());
	setPlayer(ini, psObj->playerManager->getPlayer());
	ini.setValue("health", psObj->damageManager->getHp());
	ini.setVector3i("position", psObj->getPosition());
	ini.setVector3i("rotation", toVector(psObj->getRotation()));
	...
	ini.setValue("selected", psObj->damageManager->isSelected()); // third kind of group
	if (psObj->lastEmission)
	{
		ini.setValue("lastEmission", psObj->lastEmission);
	}
	if (psObj->damageManager->getPeriodicalDamageStartTime() > 0)
	{
		ini.setValue("periodicalDamageStart", psObj->damageManager->getPeriodicalDamageStartTime());
	}
	if (psObj->damageManager->getPeriodicalDamage() > 0)
	{
	...
	{
		ini.setValue("timeLastHit", psObj->damageManager->getTimeLastHit());
	}
	if (psObj->damageManager->isSelected())
	{
		ini.setValue("selected", psObj->damageManager->isSelected());
	}
	for (int i = 0; i < game.maxPlayers; i++)
	{
		if (psObj->isVisibleToPlayer(i)) {
			ini.setValue("visible/" + WzString::number(i), psObj->isVisibleToPlayer(i));
		}
	}
}

static void writeSaveObjectJSON(nlohmann::json& jsonObj, BaseObject const* psObj)
{
	jsonObj["id"] = psObj->getId();
	setPlayerJSON(jsonObj, psObj->playerManager->getPlayer());
	jsonObj["health"] = psObj->damageManager->getHp();
	jsonObj["position"] = psObj->getPosition();
	jsonObj["rotation"] = toVector(psObj->getRotation());
	if (psObj->timeAnimationStarted) {
	...
	jsonObj["selected"] = psObj->damageManager->isSelected(); // third kind of group
	if (psObj->lastEmission)
	{
	...
	if (psObj->damageManager->getPeriodicalDamageStartTime() > 0)
	{
		jsonObj["periodicalDamageStart"] = psObj->damageManager->getPeriodicalDamageStartTime();
	}
	if (psObj->damageManager->getPeriodicalDamage() > 0)
	{
	...
	{
		if (psObj->isVisibleToPlayer(i)) {
			jsonObj["visible/" + WzString::number(i).toStdString()] = psObj->isVisibleToPlayer(i);
		}
	}
}
...
*/
static nlohmann::json writeDroid(Droid const* psCurr, bool onMission, int& counter)
{
	nlohmann::json droidObj = nlohmann::json::object();
	droidObj["name"] = psCurr->getName();
	droidObj["originalBody"] = psCurr->damageManager->getOriginalHp();
	...
	for (auto i = 0; i < numWeapons(*psCurr); i++)
	{
		if (psCurr->asWeaps[i].nStat > 0)
		{
			...
			droidObj["ammo/" + numStr] = psCurr->weaponManager->weapons[i].ammo;
			droidObj["lastFired/" + numStr] = psCurr->weaponManager->weapons[i].timeLastFired;
			droidObj["shotsFired/" + numStr] = psCurr->weaponManager->weapons[i].shotsFired;
			droidObj["rotation/" + numStr] = toVector(psCurr->weaponManager->weapons[i].getRotation());
		}
	}
	...
	if (psCurr->getLastFrustratedTime() > 0) {
		droidObj["lastFrustratedTime"] = psCurr->getLastFrustratedTime();
	}
	...
	if (psCurr->damageManager->getTimeLastHit() != UINT32_MAX)
	{
		droidObj["timeLastHit"] = psCurr->damageManager->getTimeLastHit();
	}
  ...
  auto movement = psCurr->getMovementData();
	...
	{
		droidObj["attackRun/" + WzString::number(i).toStdString()] = psCurr->getWeapon(i)->getAmmoUsed();
	}
	droidObj["lastBump"] = movement->lastBump;
	droidObj["pauseTime"] = movement->pauseTime;
	droidObj["bumpPosition"] = movement->bumpPos.xy();
	droidObj["onMission"] = onMission;
	return droidObj;
}

static void writeDroidFile(const char* pFileName, bool onMission)
{
...
static bool loadWzMapStructure(WzMap::Map& wzMap)
{
	...
	for (auto& structure : *pStructures)
	{
		...
		if (psStats == asStructureStats + numStructureStats) {
			debug(LOG_ERROR, "Structure type \"%s\" unknown", structure.getName()->c_str());
			continue; // ignore this
		}
		//for modules - need to check the base structure exists
		if (IsStatExpansionModule(psStats)) {
			Structure* psStructure = getTileStructure(map_coord(structure.getPosition().x), map_coord(structure.getPosition().y));
			if (psStructure == nullptr) {
				debug(LOG_ERROR, "No owning structure for module - %s for player - %d",
              structure.getName()->c_str(), structure.playerManager->getPlayer());
				continue; // ignore this module
			}
		}
		//check not trying to build too near the edge
		if (map_coord(structure.getPosition().x) < TOO_NEAR_EDGE || map_coord(structure.getPosition().x) > mapWidth -
			TOO_NEAR_EDGE
			|| map_coord(structure.getPosition().y) < TOO_NEAR_EDGE || map_coord(structure.getPosition().y) > mapHeight -
			TOO_NEAR_EDGE)
		{
			debug(LOG_ERROR, "Structure %s, coord too near the edge of the map", structure.getName()->c_str());
			continue; // skip it
		}
		auto player = RemapWzMapPlayerNumber(structure.playerManager->getPlayer());
		if (player >= MAX_PLAYERS)
		{
			player = MAX_PLAYERS - 1;
			NumberOfSkippedStructures++;
		}
		Structure* psStructure = buildStructureDir(psStats, structure.getPosition().x, structure.getPosition().y,
                                               structure.getRotation().direction, player, true);
		if (psStructure == nullptr)
		{
    ...
    psStructure->setId(structure.getId() > 0 ? structure.getId() : 0xFEDBCA98);
    // hack to remove struct id zero
		...
		if (structure.getCapacity() > 0) {
			...
			{
				debug(LOG_ERROR, "Structure %s can't have modules.", structure.getName()->c_str());
				continue;
			}
			for (auto i = 0; i < structure.getCapacity(); ++i)
			{
				buildStructure(moduleStat, structure.getPosition().x, structure.getPosition().y, player, true);
			}
		}
		...
		for (auto& psCurr : apsStructLists[player])
		{
			ini.beginGroup(
				"structure_" + (WzString::number(counter++).leftPadToMinimumLength(
					WzUniCodepoint::fromASCII('0'), 10))); // Zero padded so that alphabetical sort works.
			ini.setValue("name", psCurr->getStats()->id);

			writeSaveObject(ini, psCurr.get());

			if (psCurr->damageManager->getResistance() > 0) {
				ini.setValue("resistance", psCurr->damageManager->getResistance());
			}
			if (psCurr->getState() != STRUCTURE_STATE::BUILT) {
				ini.setValue("status", psCurr->getState());
			}
			ini.setValue("weapons", numWeapons(*psCurr));
			for (auto j = 0; j < psCurr->numWeaps; j++)
			{
			...
			for (auto i = 0; i < numWeapons(*psCurr); i++)
			{
				if (psCurr->getTarget(i) && !psCurr->getTarget(i)->damageManager->isDead()) {
					ini.setValue("target/" + WzString::number(i) + "/id", psCurr->getTarget(i)->getId());
					ini.setValue("target/" + WzString::number(i) + "/player", psCurr->getTarget(i)->playerManager->getPlayer());
					ini.setValue("target/" + WzString::number(i) + "/type", getObjectType(psCurr->getTarget(i)));
#ifdef DEBUG
			...
			{
				if (psCurr->getStats()->type == STRUCTURE_TYPE::FACTORY || psCurr->getStats()->type == STRUCTURE_TYPE::CYBORG_FACTORY
					|| psCurr->getStats()->type == STRUCTURE_TYPE::VTOL_FACTORY)
				{
					Factory* psFactory = (Factory*)psCurr->pFunctionality;
					ini.setValue("modules", psCurr->getCapacity());
					...
					auto psFlag = ((Factory*)psCurr->pFunctionality)->psAssemblyPoint;
					if (psFlag != nullptr)
					{
				...
				else if (psCurr->getStats()->type == STRUCTURE_TYPE::RESEARCH)
				{
					ini.setValue("modules", psCurr->getCapacity());
				...
				else if (psCurr->getStats()->type == STRUCTURE_TYPE::POWER_GEN)
				{
					ini.setValue("modules", psCurr->getCapacity());
				}
				else if (psCurr->getStats()->type == STRUCTURE_TYPE::REPAIR_FACILITY)
				{
					...
					auto psFlag = psRepair->getDeliveryPoint();
					if (psFlag) {
				...
				else if (psCurr->getStats()->type == STRUCTURE_TYPE::REARM_PAD)
				{
				...
				else if (psCurr->getStats()->type == STRUCTURE_TYPE::WALL || psCurr->getStats()->type == STRUCTURE_TYPE::GATE)
				{
...
bool loadSaveStructurePointers(WzString const& filename)
{
    ...
    auto it = std::find_if(playerList[player].structures.begin(),
                           playerList[player].structures.end(),
                           [&id](auto const& psStruct) {
      return psStruct.getId() == id;
    });

		if (it == playerList[player].structures.end()) {
			ini.endGroup();
			continue;
		...
		for (auto j = 0; j < MAX_WEAPONS; j++)
		{
			...
			if (ini.contains("target/" + WzString::number(j) + "/id"))
			{
				...
				ASSERT(tid >= 0 && tplayer >= 0, "Bad ID");
				setStructureTarget(psStruct, getBaseObjFromData(tid, tplayer, ttype), j, TARGET_ORIGIN::UNKNOWN);
				ASSERT(psStruct->getTarget(j), "Failed to find target");
			}
		}
		if (ini.contains("Factory/commander/id"))
		{
			ASSERT(psStruct->getStats()->type == STRUCTURE_TYPE::FACTORY || psStruct->getStats()->type == STRUCTURE_TYPE::CYBORG_FACTORY
			       || psStruct->getStats()->type == STRUCTURE_TYPE::VTOL_FACTORY, "Bad type");
			Factory* psFactory = (Factory*)psStruct->pFunctionality;
			OBJECT_TYPE ttype = OBJECT_TYPE::DROID;
		...
		{
			ASSERT(psStruct->getStats()->type == STRUCTURE_TYPE::REPAIR_FACILITY, "Bad type");
		...
		{
			ASSERT(psStruct->getStats()->type == STRUCTURE_TYPE::REARM_PAD, "Bad type");
			auto psReArmPad = ((RearmPad*)psStruct->pFunctionality);
			auto ttype = OBJECT_TYPE::DROID; // always, for now
			auto tid = ini.value("Rearm/target/id", -1).toInt();
			auto tplayer = ini.value("Rearm/target/player", -1).toInt();
...
bool loadSaveFeature(char* pFileData, UDWORD filesize)
{
	...
	for (count = 0; count < psHeader->quantity; count ++, pFileData += sizeOfSaveFeature)
	{
		...
		if (pFeature->getStats()->subType == FEATURE_TYPE::OIL_RESOURCE)
		{
			scriptSetDerrickPos(pFeature->getPosition().x, pFeature->getPosition().y);
		}
		//restore values
		pFeature->setId(psSaveFeature->id);
		pFeature->rot.direction = DEG(psSaveFeature->direction);
		pFeature->damageManager->setPeriodicalDamage(psSaveFeature->periodicalDamage);
		if (psHeader->version >= VERSION_14)
		{
			for (i = 0; i < MAX_PLAYERS; i++)
			{
				pFeature->setVisibleToPlayer(i, psSaveFeature->visible[i]);
			}
		}
	}
...
static bool loadWzMapFeature(WzMap::Map& wzMap)
{
	...
	for (auto& feature : *pFeatures)
	{
		...
		{
			return stat.id.compare(feature.getName().c_str()) == 0;
		});
		if (psStats == asFeatureStats + numFeatureStats) {
			debug(LOG_ERROR, "Feature type \"%s\" unknown", feature.getName()->c_str());
			continue; // ignore this
		}
		// Create the Feature
		auto pFeature = buildFeature(psStats, feature.getPosition().x, feature.getPosition().y, true);
		if (!pFeature) {
		...
		//restore values
		pFeature.setId(feature.getId());
		pFeature->rot.direction = feature.getRotation().direction;
...
bool loadSaveFeature2(const char* pFileName)
{
	...
	for (auto i = 0; i < list.size(); ++i)
	{
		...
		{
			pFeature->setId(generateSynchronisedObjectId());
		}
		pFeature->setRotation(ini.vector3i("rotation"));
		pFeature->playerManager->setPlayer(ini.value("player", PLAYER_FEATURE).toInt());

		// common SimpleObject info
		loadSaveObject(ini, pFeature);

		pFeature->damageManager->setHp(healthValue(ini, pFeature->getStats()->body));
...
bool writeFeatureFile(const char* pFileName)
{
	...
	for (auto psCurr : apsFeatureLists[0])
	{
		ini.beginGroup(
...
bool loadSaveTemplate(const char* pFileName)
{
	...
	for (size_t i = 0; i < list.size(); ++i)
	{
		...
		{
			addTemplate(player, std::unique_ptr<DroidTemplate>(new DroidTemplate(loadTemplate())));
		}
		ini.endArray();
		ini.endGroup();
	}
	...
	else
	{
		...
		enumerateTemplates(selectedPlayer, [](DroidTemplate* psTempl)
		{
...
static nlohmann::json convGameTemplateToJSON(DroidTemplate* psCurr)
{
...
bool writeTemplateFile(const char* pFileName)
{
	...
	for (auto player = 0; player < MAX_PLAYERS; player++)
	{
		if (playerList[player].droids.empty() && playerList[player].droids.empty()) {
		...
		enumerateTemplates(player, [&templates_array](DroidTemplate* psTemplate)
		{
...
bool loadTerrainTypeMapOverride(TILE_SET tileSet)
{
  ...
  switch (tileSet) {
	...
	for (auto i = 0; i < list.size(); ++i)
	{
    if (list[i].compare(tileTypeKey) != 0) continue;
    ...
    {
      auto tileType = ini.value(keys.at(j)).toUInt();
...
static bool writeTerrainTypeMapFile(char const* pFileName)
{
	...
	for (auto i = 0; i < MAX_TILE_TEXTURES; i++)
	{
		auto& tType = terrainTypes[i];
		if (tType > TER_MAX) {
...
bool loadSaveCompList(const char* pFileName)
{
	...
	for (auto player = 0; player < MAX_PLAYERS; player++)
	{
		...
		for (auto i = 0; i < list.size(); ++i)
		{
			...
			auto state = ini.value(name, UNAVAILABLE).toInt();
			ComponentStats* psComp = getCompStatsFromName(name);
			ASSERT_OR_RETURN(false, psComp, "Bad component %s", name.toUtf8().c_str());
			ASSERT_OR_RETURN(false, (int)psComp->compType >= 0 && psComp->compType != COMPONENT_TYPE::COUNT, "Bad type %d",
			                 psComp->compType);
			...
			apCompLists[player][(int)psComp->compType][psComp->index] = state;
		}
		ini.endGroup();
	}
	return true;
...
static bool writeCompListFile(const char* pFileName)
{
	...
	{
		ini.beginGroup("player_" + WzString::number(player));
		for (auto i = 0; i < numBodyStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asBodyStats + i);
			const int state = apCompLists[player][(int)COMPONENT_TYPE::BODY][i];
			if (state != UNAVAILABLE)
			{
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numWeaponStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asWeaponStats + i);
			const int state = apCompLists[player][(int)COMPONENT_TYPE::WEAPON][i];
			if (state != UNAVAILABLE)
			{
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numConstructStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asConstructStats + i);
			const int state = apCompLists[player][(int)COMPONENT_TYPE::CONSTRUCT][i];
			if (state != UNAVAILABLE) {
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numECMStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asECMStats + i);
			const int state = apCompLists[player][(int)COMPONENT_TYPE::ECM][i];
			if (state != UNAVAILABLE) {
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numPropulsionStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asPropulsionStats + i);
			const auto state = apCompLists[player][(int)COMPONENT_TYPE::PROPULSION][i];
			if (state != UNAVAILABLE) {
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numSensorStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asSensorStats + i);
			const auto state = apCompLists[player][(int)COMPONENT_TYPE::SENSOR][i];
			if (state != UNAVAILABLE) {
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numRepairStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asRepairStats + i);
			const auto state = apCompLists[player][(int)COMPONENT_TYPE::REPAIR_UNIT][i];
			if (state != UNAVAILABLE) {
				ini.setValue(psStats->id, state);
			}
		}
		for (auto i = 0; i < numBrainStats; i++)
		{
			ComponentStats* psStats = (ComponentStats*)(asBrainStats + i);
			const int state = apCompLists[player][COMPONENT_TYPE::BRAIN][i];
			if (state != UNAVAILABLE)
			{
				ini.setValue(psStats->id, state);
			}
...
static bool loadSaveStructTypeList(const char* pFileName)
{
	...
	for (unsigned player = 0; player < MAX_PLAYERS; player++)
	{
		...
		for (size_t i = 0; i < list.size(); ++i)
		{
			...
			{
				StructureStats* psStats = asStructureStats + statInc;
...
bool loadSaveMessage(const char* pFileName, LEVEL_TYPE levelType)
{
	...
	else if (gameType == GAME_TYPE::GTYPE_SAVE_START)
	{
	...
	for (size_t i = 0; i < list.size(); ++i)
	{
		...
		if (type == MESSAGE_TYPE::MSG_PROXIMITY) {
			...
			if (type == MESSAGE_TYPE::MSG_RESEARCH || gameType == GAME_TYPE::GTYPE_SAVE_MIDMISSION) {
				MESSAGE* psMessage = addMessage(type, false, player);
	...
	for (auto player = 0; player < game.maxPlayers; player++)
	{
		...
		for (auto psMessage : apsMessages[player])
		{
			...
			if (psMessage.type == MESSAGE_TYPE::MSG_PROXIMITY) {
				...
				for (psProx = *apsProxDisp[player].begin(); psProx < apsProxDisp[player].end(); psProx)
				{
				...
				if (psProx && psProx->type == POSITION_TYPE::POS_PROXDATA) {
					...
					if (psMessage.dataType == MSG_DATA_BEACON) {
						auto viewData = (VIEW_PROXIMITY*)psMessage.pViewData->pData;
					...
					const auto psObj = psMessage.psObj;
					if (psObj) {
						ini.setValue("obj/id", psObj->getId());
						ini.setValue("obj/player", psObj->playerManager->getPlayer());
						ini.setValue("obj/type", getObjectType(psObj));
					}
					else {
...
void loadSaveStructLimits(const char* pFileName)
{
	WzConfig ini(pFileName, WzConfig::ReadOnly);
	...
	for (auto player = 0; player < game.maxPlayers; player++)
	{
		...
		{
			WzString name = list[i];
			auto limit = ini.value(name, 0).toInt();
			if (name.compare("@Droid") == 0) {
				...
				for (statInc = 0; statInc < numStructureStats; ++statInc)
				{
					...
					if (name.compare(psStats->id) == 0)
					{
						asStructureStats[statInc].upgraded_stats[player].limit = limit != 255 ? limit : LOTS_OF;
						break;
					}
				}
				ASSERT_OR_RETURN(, statInc != numStructureStats, "Did not find structure %s",
				                 name.toUtf8().c_str());
			}
		}
		ini.endGroup();
...
*/
void writeStructLimitsFile(const char* pFileName)
{
	WzConfig ini(pFileName, WzConfig::ReadAndWrite);
	...
	for (auto player = 0; player < game.maxPlayers; player++)
	{
		...
		StructureStats* psStats = asStructureStats;
		for (auto i = 0; i < numStructureStats; i++, psStats++)
		{
			const auto limit = MIN(asStructureStats[i].upgraded_stats[player].limit, 255);
			if (limit != 255) {
...
bool readFiresupportDesignators(const char* pFileName)
{
	...
	for (auto i = 0; i < list.size(); ++i)
	{
		auto id = ini.value("Player_" + WzString::number(i) + "/id", NULL_ID).toInt();
		if (id != NULL_ID) {
 ...
 */
void writeFiresupportDesignators(const char* pFileName)
{
	WzConfig ini(pFileName, WzConfig::ReadAndWrite);

	for (auto player = 0; player < MAX_PLAYERS; player++)
	{
		auto psDroid = cmdDroidGetDesignator(player);
		if (psDroid != nullptr) {
			ini.setValue("Player_" + WzString::number(player) + "/id", psDroid->getId());
		}
	}
}

// write the event state to a file on disk
static void writeScriptState(const char* pFileName)
{
...
void loadScriptState(char* pFileName)
{
