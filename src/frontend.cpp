...
template <typename T>
static T seqCycle(T value, T min, int inc, T max)
{
...
void startTitleMenu()
{
	...
	{
		addTextButton(FRONTEND_PLAYINTRO, FRONTEND_POS6X, FRONTEND_POS6Y, _("View Intro"),
		              WBUT_TXTCENTRE | WBUT_DISABLE);
	...
	{
		addTextButton(FRONTEND_CONTINUE, FRONTEND_POS7X, FRONTEND_POS7Y, _("Continue Last Save"),
		              WBUT_TXTCENTRE | WBUT_DISABLE);
	...
	W_BUTTON* pRightAlignedButton = addSmallTextButton(FRONTEND_DONATELINK, FRONTEND_POS9X + 360, FRONTEND_POS9Y,
	                                                   _("Donate: http://donations.wz2100.net/"), 0);
	...
	pRightAlignedButton = addSmallTextButton(FRONTEND_CHATLINK, FRONTEND_POS9X + 360, 0,
	                                         _("Chat with players on Discord or IRC"), 0);
	moveToParentRightEdge(pRightAlignedButton, 6);
	widgSetTip(psWScreen, FRONTEND_CHATLINK,
	           _("Connect to Discord or Freenode through webchat by clicking this link."));
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FRONTEND_UPGRDLINK, 7, 7, MULTIOP_BUTW, MULTIOP_BUTH,
	            _("Check for a newer version"), IMAGE_GAMEVERSION, IMAGE_GAMEVERSION_HI, true);
}
...
void startTutorialMenu()
{
	...
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FRONTEND_QUIT, 10, 10, 30, 29, P_("menu", "Return"), IMAGE_RETURN,
	            IMAGE_RETURN_HI, IMAGE_RETURN_HI);
}
...
void startSinglePlayerMenu()
{
	...
	if (!PHYSFS_exists("sequences/devastation.ogg"))
	{
		addSmallTextButton(FRONTEND_HYPERLINK, FRONTEND_POS9X, FRONTEND_POS9Y,
		                   _("Campaign videos are missing! Get them from http://wz2100.net"), 0);
		notifyAboutMissingVideos();
	}
}
...
void startCampaignSelector()
{
	...
	ASSERT(list.size() <= static_cast<size_t>(std::numeric_limits<UDWORD>::max()),
	       "list.size() (%zu) exceeds UDWORD max", list.size());
	for (UDWORD i = 0; i < static_cast<UDWORD>(list.size()); i++)
	{
		addTextButton(FRONTEND_CAMPAIGN_1 + i, FRONTEND_POS1X, FRONTEND_POS2Y + FRONTEND_BUTHEIGHT * i,
		              gettext(list[i].name.toUtf8().c_str()), WBUT_TXTCENTRE);
	}
...
void startMultiPlayerMenu()
{
	...
	addSmallTextButton(FRONTEND_HYPERLINK, FRONTEND_POS9X, FRONTEND_POS9Y,
	                   _("TCP port 2100 must be opened in your firewall or router to host games!"), 0);
}
	...
	case FRONTEND_HYPERLINK:
		if (!openFolderInDefaultFileManager(PHYSFS_getWriteDir()))
		{
			...
			std::string configErrorMessage =
				_("Failed to open configuration directory in system default file browser.");
			configErrorMessage += "\n\n";
			configErrorMessage += _("Configuration directory is reported as:");
			configErrorMessage += "\n";
			configErrorMessage += PHYSFS_getWriteDir();
			configErrorMessage += "\n\n";
			configErrorMessage += _(
				"If running inside a container / isolated environment, this may differ from the actual path on disk.");
			configErrorMessage += "\n";
...
void startGraphicsOptionsMenu()
{
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_FMVMODE_R, graphicsOptionsFmvmodeString(), WBUT_SECONDARY)));
	row.start++;

	// Scanlines
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_SCANLINES, _("Scanlines"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_SCANLINES_R, graphicsOptionsScanlinesString(), WBUT_SECONDARY)));
	row.start++;
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_SHADOWS_R, graphicsOptionsShadowsString(), WBUT_SECONDARY)));
	row.start++;

	// fog
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_FOG, _("Fog"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_FOG_R, graphicsOptionsFogString(), WBUT_SECONDARY)));
	row.start++;

	// Radar
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_RADAR, _("Radar"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_RADAR_R, graphicsOptionsRadarString(), WBUT_SECONDARY)));
	row.start++;

	// RadarJump
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_RADAR_JUMP, _("Radar Jump"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_RADAR_JUMP_R, graphicsOptionsRadarJumpString(), WBUT_SECONDARY)));
	row.start++;

	// screenshake
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_SSHAKE, _("Screen Shake"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_SSHAKE_R, graphicsOptionsScreenShakeString(), WBUT_SECONDARY)));
	row.start++;
...
void startAudioAndZoomOptionsMenu()
{
	...
	grid->place({1, 1, false}, row, addSliderWrap(makeFESlider(FRONTEND_FX_SL, FRONTEND_BOTFORM, AUDIO_VOL_MAX,
	                                                           static_cast<UDWORD>(sound_GetUIVolume() * 100.0f))));
	row.start++;

	// 3d audio
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_3D_FX, _("FX Volume"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row, addSliderWrap(makeFESlider(FRONTEND_3D_FX_SL, FRONTEND_BOTFORM, AUDIO_VOL_MAX,
	                                                           static_cast<UDWORD>(sound_GetEffectsVolume() *
		                                                           100.0f))));
	row.start++;

	// cd audio
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_MUSIC, _("Music Volume"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row, addSliderWrap(makeFESlider(FRONTEND_MUSIC_SL, FRONTEND_BOTFORM, AUDIO_VOL_MAX,
	                                                           static_cast<UDWORD>(sound_GetMusicVolume() * 100.0f))));
	row.start++;
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_SUBTITLES_R, graphicsOptionsSubtitlesString(), WBUT_SECONDARY)));
	row.start++;

	// HRTF
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_SOUND_HRTF, _("HRTF"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_SOUND_HRTF_R, audioAndZoomOptionsSoundHRTFMode(), WBUT_SECONDARY)));
	row.start++;
	if (sound_GetHRTFMode() == HRTFMode::Unsupported)
	{
		...
		widgSetTip(psWScreen, FRONTEND_SOUND_HRTF_R,
		           _("HRTF is not supported on your device / system / OpenAL library"));
	}

	// map zoom
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_MAP_ZOOM, _("Map Zoom"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_MAP_ZOOM_R, audioAndZoomOptionsMapZoomString(), WBUT_SECONDARY)));
	row.start++;

	// map zoom rate
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_MAP_ZOOM_RATE, _("Map Zoom Rate"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_MAP_ZOOM_RATE_R, audioAndZoomOptionsMapZoomRateString(),
	                                     WBUT_SECONDARY)));
	row.start++;
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_RADAR_ZOOM_R, audioAndZoomOptionsRadarZoomString(), WBUT_SECONDARY)));
	row.start++;
...
bool runAudioAndZoomOptionsMenu()
{
	...
	case FRONTEND_MAP_ZOOM_RATE_R:
		{
			war_SetMapZoomRate(seqCycle(war_GetMapZoomRate(), MAP_ZOOM_RATE_MIN, MAP_ZOOM_RATE_STEP,
			                            MAP_ZOOM_RATE_MAX));
...
static char const* videoOptionsResolutionGetReadOnlyTooltip()
{
	...
	case WINDOW_MODE::desktop_fullscreen:
		return _(
			"In Desktop Fullscreen mode, the resolution matches that of your desktop \n(or what the window manager allows).");
...
public:
	...
	void selectAt(size_t index) const
	{
		...
		ASSERT_OR_RETURN(, wzGetCurrentWindowMode() == WINDOW_MODE::fullscreen,
		                   "Attempt to change resolution in windowed-mode / desktop-fullscreen mode");
...
void refreshCurrentVideoOptionsValues()
{
	...
	{
		widgSetString(psWScreen, FRONTEND_RESOLUTION_READONLY,
		              ScreenResolutionsModel::currentResolutionString().c_str());
...
void startVideoOptionsMenu()
{
	...
	parent->attach(label);
	label->setGeometry(FRONTEND_POS1X + 48, FRONTEND_POS1Y, FRONTEND_BUTWIDTH - FRONTEND_POS1X - 48,
	                   FRONTEND_BUTHEIGHT);
	...
	// Fullscreen/windowed
	grid->place(
		{0}, row, addMargin(makeTextButton(FRONTEND_WINDOWMODE, videoOptionsWindowModeLabel(), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_WINDOWMODE_R, videoOptionsWindowModeString(), WBUT_SECONDARY)));
	row.start++;

	// Resolution
	auto resolutionReadonlyLabel = makeTextButton(FRONTEND_RESOLUTION_READONLY_LABEL, videoOptionsResolutionLabel(),
	                                              WBUT_SECONDARY | WBUT_DISABLE);
	...
	auto resolutionReadonlyValue = makeTextButton(FRONTEND_RESOLUTION_READONLY,
	                                              ScreenResolutionsModel::currentResolutionString(),
	                                              WBUT_SECONDARY | WBUT_DISABLE);
	...
	auto resolutionDropdownLabel = makeTextButton(FRONTEND_RESOLUTION_DROPDOWN_LABEL, videoOptionsResolutionLabel(),
	                                              WBUT_SECONDARY);
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_TEXTURESZ_R, videoOptionsTextureSizeString(), WBUT_SECONDARY)));
	row.start++;

	// Vsync
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_VSYNC, _("Vertical sync"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_VSYNC_R, videoOptionsVsyncString(), WBUT_SECONDARY)));
	row.start++;

	// Antialiasing
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_FSAA, _("Antialiasing*"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_FSAA_R, videoOptionsAntialiasingString(), WBUT_SECONDARY)));
	row.start++;

	auto antialiasing_label = std::make_shared<W_LABEL>();
	parent->attach(antialiasing_label);
	antialiasing_label->setGeometry(FRONTEND_POS1X + 48, FRONTEND_POS1Y - 18, FRONTEND_BUTWIDTH - FRONTEND_POS1X - 48,
	                                FRONTEND_BUTHEIGHT);
	...
	if (showDisplayScale)
	{
		grid->place({0}, row, addMargin(makeTextButton(FRONTEND_DISPLAYSCALE, videoOptionsDisplayScaleLabel(),
		                                               WBUT_SECONDARY)));
		grid->place({1, 1, false}, row,
		            addMargin(makeTextButton(FRONTEND_DISPLAYSCALE_R, videoOptionsDisplayScaleString(),
		                                     WBUT_SECONDARY)));
		row.start++;
	}
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_GFXBACKEND_R, videoOptionsGfxBackendString(), WBUT_SECONDARY)));
	row.start++;
...
void seqWindowMode()
{
	...
	do
	{
		currentFullscreenModePos = seqCycle(currentFullscreenModePos, static_cast<size_t>(0), 1,
		                                    supportedFullscreenModes.size() - 1);
	...
	if (currentFullscreenModePos == startingFullscreenModePos)
	{
		...
		wzDisplayDialog(Dialog_Warning, _("Unable to change Window Mode"),
		                _(
			                "Warzone failed to change the Window mode.\nYour system / drivers may not support other modes."));
	}
	else if (success)
	{
...
void seqVsyncMode()
{
	...
	do
	{
		currentVsyncMode = static_cast<gfx_api::context::swap_interval_mode>(seqCycle(
			static_cast<std::underlying_type<gfx_api::context::swap_interval_mode>::type>(currentVsyncMode),
			static_cast<std::underlying_type<gfx_api::context::swap_interval_mode>::type>(
				gfx_api::context::min_swap_interval_mode), 1,
			static_cast<std::underlying_type<gfx_api::context::swap_interval_mode>::type>(
				gfx_api::context::max_swap_interval_mode)));
	...
	if (currentVsyncMode == startingVsyncMode)
	{
		...
		wzDisplayDialog(Dialog_Warning, _("Unable to change Vertical Sync"),
		                _(
...
void startMouseOptionsMenu()
{
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_MFLIP_R, mouseOptionsMflipString(), WBUT_SECONDARY)));
	row.start++;

	// Cursor trapping
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_TRAP, _("Trap Cursor"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_TRAP_R, mouseOptionsTrapString(), WBUT_SECONDARY)));
	row.start++;
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_MBUTTONS_R, mouseOptionsMbuttonsString(), WBUT_SECONDARY)));
	row.start++;
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_MMROTATE_R, mouseOptionsMmrotateString(), WBUT_SECONDARY)));
	row.start++;
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_CURSORMODE_R, mouseOptionsCursorModeString(), WBUT_SECONDARY)));
	row.start++;
...
void startGameOptionsMenu()
{
	...
	addTextButton(FRONTEND_DIFFICULTY_R, FRONTEND_POS3M - 55, FRONTEND_POS3Y, gameOptionsDifficultyString(),
	              WBUT_SECONDARY);

	// Camera speed
	addTextButton(FRONTEND_CAMERASPEED, FRONTEND_POS4X - 35, FRONTEND_POS4Y, _("Camera Speed"), WBUT_SECONDARY);
	addTextButton(FRONTEND_CAMERASPEED_R, FRONTEND_POS4M - 55, FRONTEND_POS4Y, gameOptionsCameraSpeedString(),
	              WBUT_SECONDARY);
	...
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FE_P0, FRONTEND_POS6M + (0 * (w + 6)) - 20, FRONTEND_POS6Y, w, h, nullptr,
	            IMAGE_PLAYERN, IMAGE_PLAYERX, true, 0);
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FE_P4, FRONTEND_POS6M + (1 * (w + 6)) - 20, FRONTEND_POS6Y, w, h, nullptr,
	            IMAGE_PLAYERN, IMAGE_PLAYERX, true, 4);
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FE_P5, FRONTEND_POS6M + (2 * (w + 6)) - 20, FRONTEND_POS6Y, w, h, nullptr,
	            IMAGE_PLAYERN, IMAGE_PLAYERX, true, 5);
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FE_P6, FRONTEND_POS6M + (3 * (w + 6)) - 20, FRONTEND_POS6Y, w, h, nullptr,
	            IMAGE_PLAYERN, IMAGE_PLAYERX, true, 6);
	addMultiBut(psWScreen, FRONTEND_BOTFORM, FE_P7, FRONTEND_POS6M + (4 * (w + 6)) - 20, FRONTEND_POS6Y, w, h, nullptr,
	            IMAGE_PLAYERN, IMAGE_PLAYERX, true, 7);
	...
	for (int colour = -1; colour < MAX_PLAYERS_IN_GUI; ++colour)
	{
		...
		addMultiBut(psWScreen, FRONTEND_BOTFORM, FE_MP_PR + colour + 1, FRONTEND_POS7M + cellX * (w + 2) - 20,
		            FRONTEND_POS7Y + cellY * (h + 2) - 5, w, h, nullptr, IMAGE_PLAYERN, IMAGE_PLAYERX, true,
		            colour >= 0 ? colour : MAX_PLAYERS + 1);
	}
...
static std::shared_ptr<WIDGET> makeInactivityMinutesMPDropdown()
{
	...
	for (uint32_t inactivityMinutes = MIN_MPINACTIVITY_MINUTES; inactivityMinutes <= (MIN_MPINACTIVITY_MINUTES + 6);
	     inactivityMinutes++)
	{
	...
	dropdown->setOnChange([inactivityMinutesValues](DropdownWidget& dropdown)
	{
		if (auto selectedIndex = dropdown.getSelectedIndex())
		{
			ASSERT_OR_RETURN(, selectedIndex.value() < inactivityMinutesValues.size(), "Invalid selected index: %zu",
			                   selectedIndex.value());
...
static std::shared_ptr<WIDGET> makeLagKickDropdown()
{
	...
	{
		// add the current value, which must be a custom manual config edit
		addLagKickSecondsRow(war_getAutoLagKickSeconds(),
		                     astringf(_("%u seconds"), (unsigned)war_getAutoLagKickSeconds()));
	}
	...
	dropdown->setOnChange([lagKickSecondsValues](DropdownWidget& dropdown)
	{
		if (auto selectedIndex = dropdown.getSelectedIndex())
		{
			ASSERT_OR_RETURN(, selectedIndex.value() < lagKickSecondsValues.size(), "Invalid selected index: %zu",
			                   selectedIndex.value());
...
static std::shared_ptr<WIDGET> makeOpenSpectatorSlotsMPDropdown()
{
	...
	auto it = std::find(openSpectatorSlotsValues.begin(), openSpectatorSlotsValues.end(),
	                    war_getMPopenSpectatorSlots());
	...
	dropdown->setOnChange([openSpectatorSlotsValues](DropdownWidget& dropdown)
	{
		if (auto selectedIndex = dropdown.getSelectedIndex())
		{
			ASSERT_OR_RETURN(, selectedIndex.value() < openSpectatorSlotsValues.size(), "Invalid selected index: %zu",
			                   selectedIndex.value());
...
void startMultiplayOptionsMenu()
{
	...
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_GAME_PORT_R, std::to_string(NETgetGameserverPort()), WBUT_DISABLE)));
	// FUTURE TODO: Make this an input field or similar and allow editing (although reject ports <= 1024)
	row.start++;

	// Enable UPnP
	grid->place({0}, row, addMargin(makeTextButton(FRONTEND_UPNP, _("Enable UPnP"), WBUT_SECONDARY)));
	grid->place({1, 1, false}, row,
	            addMargin(makeTextButton(FRONTEND_UPNP_R, multiplayOptionsUPnPString(), WBUT_SECONDARY)));
	row.start++;

	// Inactivity Kick
	grid->place(
		{0}, row, addMargin(makeTextButton(FRONTEND_INACTIVITY_TIMEOUT, _("Inactivity Timeout"), WBUT_SECONDARY)));
...
static void displayTitleBitmap(WZ_DECL_UNUSED WIDGET* psWidget, WZ_DECL_UNUSED UDWORD xOffset,
                               WZ_DECL_UNUSED UDWORD yOffset)
{
	...
	cache.formattedVersionString.render(pie_GetVideoBufferWidth() - 9, pie_GetVideoBufferHeight() - 14, WZCOL_GREY,
	                                    270.f);
	...
	cache.formattedVersionString.render(pie_GetVideoBufferWidth() - 10, pie_GetVideoBufferHeight() - 15,
	                                    WZCOL_TEXT_BRIGHT, 270.f);
...
{
	iV_DrawImage2(WzString::fromUtf8("image_fe_logo.png"), xOffset + psWidget->x(), yOffset + psWidget->y(),
...
static void displayBigSlider(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset)
{
	...
	int sx = (Slider->width() - 3 - Slider->barSize) * Slider->pos / std::max<int>(Slider->numStops, 1);
	// determine pos.
...
void displayTextOption(WIDGET* psWidget, UDWORD xOffset, UDWORD yOffset)
{
	...
	else // available
	{
		...
		else if (psWidget->id == FRONTEND_HYPERLINK || psWidget->id == FRONTEND_DONATELINK || psWidget->id ==
			FRONTEND_CHATLINK) // special case for our hyperlink
		{
			colour = WZCOL_YELLOW;
		}
...
void addTopForm(bool wide)
{
	...
	if (wide)
	{
		topForm->setCalcLayout(LAMBDA_CALCLAYOUT_SIMPLE({
				psWidget->setGeometry(FRONTEND_TOPFORM_WIDEX, FRONTEND_TOPFORM_WIDEY, FRONTEND_TOPFORM_WIDEW,
					FRONTEND_TOPFORM_WIDEH);
				}));
	}
	else
	{
...
// ////////////////////////////////////////////////////////////////////////////
W_LABEL* addSideText(const std::shared_ptr<W_SCREEN>& psScreen, UDWORD formId, UDWORD id, UDWORD PosX, UDWORD PosY,
                     const char* txt)
{
...
void frontendScreenSizeDidChange(int oldWidth, int oldHeight, int newWidth, int newHeight)
{
	...
	if ((newWidth >= oldWidth && newHeight >= oldHeight)
		&& (lastProcessedScreenWidth != newWidth || lastProcessedScreenHeight != newHeight)
		// filter out duplicate events
		&& (lastProcessedDisplayScale == 0 || lastProcessedDisplayScale == currentDisplayScale)
		// filter out duplicate events & display-scale-only changes
		&& (newWidth >= WZ_SUGGESTED_MAX_LOGICAL_SCREEN_SIZE_DIMENSION || newHeight >=
			WZ_SUGGESTED_MAX_LOGICAL_SCREEN_SIZE_DIMENSION))
	{
		unsigned int suggestedDisplayScale = wzGetSuggestedDisplayScaleForCurrentWindowSize(
			WZ_SUGGESTED_MAX_LOGICAL_SCREEN_SIZE_DIMENSION);
			...
			&& !hasNotificationsWithTag(DISPLAY_SCALE_PROMPT_INCREASE_TAG, NotificationScope::DISPLAYED_ONLY))
		{
			...
			contextText += _(
				"With your current resolution & display scale settings, the game's user interface may appear small, and the game perspective may appear distorted.");
			contextText += "\n\n";
			contextText += _("You can fix this by increasing the game's Display Scale setting.");
			contextText += "\n\n";
			contextText += astringf(
				_("Would you like to increase the game's Display Scale to: %u%%?"), suggestedDisplayScale);
			...
			notification.action = WZ_Notification_Action(_("Increase Display Scale"), [](const WZ_Notification&)
			{
				...
				unsigned int desiredDisplayScale = wzGetSuggestedDisplayScaleForCurrentWindowSize(
					WZ_SUGGESTED_MAX_LOGICAL_SCREEN_SIZE_DIMENSION);
				...
				if (wzChangeDisplayScale(desiredDisplayScale))
				{
					...
					completed_notification.contentTitle = astringf(
						_("Display Scale Increased to: %u%%"), desiredDisplayScale);
					std::string contextText = _(
						"You can adjust the Display Scale at any time in the Video Options menu.");
			...
			notification.onDismissed = [](const WZ_Notification&, WZ_Notification_Dismissal_Reason reason)
			{
				...
				dismissed_notification.displayOptions = WZ_Notification_Display_Options::makeOneTime(
					"displayscale::tip");
			...
			notification.displayOptions = WZ_Notification_Display_Options::makeIgnorable(
				"displayscale::prompt_increase", 4);
