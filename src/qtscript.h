...
bool triggerEventSyncRequest(unsigned from, int req_id, int x, int y, BaseObject * psObj, BaseObject * psObj2);
...
void jsDebugSelected(const BaseObject * psObj);
...
public:
	...
	static generic_script_object fromObject(const BaseObject * psObj);
public:
...
public:
	...
	[[nodiscard]] BaseObject * getObject() const; // if type == OBJ_DROID, OBJ_FEATURE, OBJ_STRUCTURE, returns the game object
public:
...
public:
	...
	{
		typedef int groupID;
		typedef std::unordered_map<const BaseObject *, groupID> ObjectToGroupMap;
	private:
		ObjectToGroupMap m_map;
		typedef std::unordered_set<const BaseObject *> GroupSet;
	...
	public:
		groupID newGroupID();
		void insertObjectIntoGroup(const BaseObject * psObj, groupID groupId);
		[[nodiscard]] inline const ObjectToGroupMap& map() const { return m_map; }
		[[nodiscard]] size_t groupSize(groupID groupId) const;
		optional<groupID> removeObjectFromGroup(const BaseObject * psObj);
		[[nodiscard]] std::vector<const BaseObject *> getGroupObjects(groupID groupId) const;
	};
...
public:
	uniqueTimerID setTimer(wzapi::scripting_instance* caller, const TimerFunc& timerFunc, unsigned player, int milliseconds,
                         const std::string& timerName = "", const BaseObject * obj = nullptr, timerType type = TIMER_REPEAT,
...
public:
	static bool triggerEventSeen(BaseObject * psViewer, BaseObject * psSeen);
...
public:
	...
	static optional<std::string> getLabel(WZAPI_PARAMS(const BaseObject *psObj));
...
public:
	...
	static std::vector<const BaseObject *> enumAreaByLabel(
		WZAPI_PARAMS(std::string label, optional<int> playerFilter, optional<bool> seen));
	static std::vector<const BaseObject *> enumArea(
		WZAPI_PARAMS(scr_area area, optional<int> playerFilter, optional<bool> seen));
private:
	static std::vector<const BaseObject *> _enumAreaWorldCoords(
...
public:
	...
	static std::vector<const BaseObject *> enumAreaJS(WZAPI_PARAMS(area_by_values_or_area_label_lookup area_lookup,
                                                                      optional<int> playerFilter, optional<bool> seen));

	// Group functions
	static std::vector<const BaseObject *> enumGroup(WZAPI_PARAMS(int groupId));
	...
	static wzapi::no_return_value groupAdd(WZAPI_PARAMS(int groupId, const BaseObject *psObj));
...
protected:
	...
	friend void scriptRemoveObject(const BaseObject * psObj);
	void groupRemoveObject(const BaseObject * psObj);

private:
	std::pair<bool, int> seenLabelCheck(wzapi::scripting_instance* instance, const BaseObject * seen,
	                                    const BaseObject * viewer);
	static void removeFromGroup(wzapi::scripting_instance* instance, GROUPMAP* psMap, const BaseObject * psObj);
	bool groupAddObject(const BaseObject * psObj, int groupId, wzapi::scripting_instance* instance);
};
