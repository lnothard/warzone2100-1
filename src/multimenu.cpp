...
static const unsigned M_REQUEST_NP[] = {
	M_REQUEST_2P, M_REQUEST_3P, M_REQUEST_4P, M_REQUEST_5P, M_REQUEST_6P, M_REQUEST_7P, M_REQUEST_8P, M_REQUEST_9P,
	M_REQUEST_10P, M_REQUEST_11P, M_REQUEST_12P, M_REQUEST_13P, M_REQUEST_14P, M_REQUEST_15P, M_REQUEST_16P
};
	...
	if (mapData != nullptr)
	{
		...
		if (!hash.isZero())
		{
			if (hash != data.cache.hash)
			{
				...
				while ((int)iV_GetTextWidth(butString, font_small) > psWidget->width() - 10 - (8 + mapData->players *
					6))
				{
					butString[strlen(butString) - 1] = '\0';
				}
 ...
 */
void addMultiRequest(const char* searchDir, const char* fileExtension, UDWORD mode, UBYTE numPlayers,
...
private:
	...
	void initialize()
	{
		...
		{
			if (isHumanPlayer(player) || (game.type == LEVEL_TYPE::SKIRMISH && player < game.maxPlayers && NetPlay.
				players[player].difficulty != AIDifficulty::DISABLED))
			{
	...
	void addPlayerWidgets(uint32_t player, uint32_t row)
	{
		...
		if (alliancesCanGiveAnything(game.alliance) && player != selectedPlayer && player < MAX_PLAYERS && !NetPlay.
	...
	void updatePlayersWidgets()
	{
		...
		{
			const auto color = GetPlayerTextColor(
				(selectedPlayer < MAX_PLAYERS && playerWidget.player < MAX_PLAYERS)
					? alliances[selectedPlayer][playerWidget.player]
					: 0, playerWidget.player);
			const bool isHuman = isHumanPlayer(playerWidget.player);
			const bool isAlly = (selectedPlayer < MAX_PLAYERS && playerWidget.player < MAX_PLAYERS) && aiCheckAlliances(
				selectedPlayer, playerWidget.player);
			...
			{
				ssprintf(unitsString, "%d",
				         getNumDroids(playerWidget.player) + getNumTransporterDroids(playerWidget.player));
			}
			...
			//Won't pass this when in both release and multiplayer modes
			{
				// Player power
...
public:
	void display(int xOffset, int yOffset) override
	{
		...
		for (unsigned q = 0; q < 2; ++q)
		{
			...
			ssprintf(str, _("Traf: %lu/%lu"), NETgetStatistic(NetStatisticRawBytes, true, isTotal),
			         NETgetStatistic(NetStatisticRawBytes, false, isTotal));
			iV_DrawText(str, x0 + xPos, y0 + height() + yPos, font_small);
			xPos += iV_GetTextWidth(str, font_small) + 20;

			ssprintf(str, _("Uncompressed: %lu/%lu"), NETgetStatistic(NetStatisticUncompressedBytes, true, isTotal),
			         NETgetStatistic(NetStatisticUncompressedBytes, false, isTotal));
			iV_DrawText(str, x0 + xPos, y0 + height() + yPos, font_small);
			xPos += iV_GetTextWidth(str, font_small) + 20;

			ssprintf(str, _("Pack: %lu/%lu"), NETgetStatistic(NetStatisticPackets, true, isTotal),
			         NETgetStatistic(NetStatisticPackets, false, isTotal));
			iV_DrawText(str, x0 + xPos, y0 + height() + yPos, font_small);
		}
#endif
	}
	...
	if (id >= MULTIMENU_CHANNEL && id < MULTIMENU_CHANNEL + MAX_CONNECTED_PLAYERS)
	{
		...
		if (mouseDown(MOUSE_RMB) && NetPlay.isHost) // both buttons....
		{
                                                                           ...
                                                                           && NetPlay.players[i].ai != AI_OPEN)))
			{
				...
				ssprintf(buf, _("kicked %s : %s from the game, and added them to the banned list!"),
				         getPlayerName((unsigned int)i), NetPlay.players[i].IPtextAddress);
