...
/// Initialisation function for states & globals in this module
void Edit3DInitVars()
{
...
void adjustTileHeight(Tile* psTile, int adjust)
{
	auto newHeight = psTile->height + adjust;

	if (newHeight >= TILE_MIN_HEIGHT &&
      newHeight <= TILE_MAX_HEIGHT) {
...
void init3DBuilding(BaseStats* psStats, BUILDCALLBACK CallBack, void* UserData)
{
	...
	if (psStats->hasType(STAT_STRUCTURE)) {
		sBuildDetails.width = ((StructureStats*)psStats)->base_width;
		sBuildDetails.height = ((StructureStats*)psStats)->base_breadth;
...
bool process3DBuilding()
{
	...
	if (buildState != BUILD3D_FINISHED) {
        ...
        canLineBuild()) {
			...
			// why must this be done here? If not doing it here, dragging works
      // almost normally, except it suddenly stops working if the drag becomes invalid.
...
bool found3DBuildLocTwo(Vector2i& pos, Vector2i& pos2)
{
	...
	LineBuild lb = calcLineBuild(stats, getBuildingDirection(),
                               wallDrag.pos, wallDrag.pos2);
	pos = lb.begin;
	pos2 = lb.back();
	...
	if (ctrlShiftDown()) {
		quickQueueMode = true;
		init3DBuilding(sBuildDetails.psStats,
                   sBuildDetails.CallBack,
...
/// @return `true` if the build state is not equal to BUILD3D_NONE
