...
struct Droid::Impl
{
  ...
  /// Bit set telling which tiles block this type of droid
...
void Droid::upgradeHitPoints() {
  ...
  auto hp = MIN(damageManager->getOriginalHp(),
                damageManager->getHp() * increase / factor + 1);
...
bool Droid::hasStandardSensor() const
{
  ...
  return sensor != nullptr && pimpl->type == DROID_TYPE::SENSOR &&
         (sensor->type == SENSOR_TYPE::VTOL_INTERCEPT ||
          sensor->type == SENSOR_TYPE::STANDARD || sensor->type == SENSOR_TYPE::SUPER);
}
...
bool Droid::hasCbSensor() const
{
  ...
  return sensor != nullptr && pimpl->type == DROID_TYPE::SENSOR &&
         (sensor->type == SENSOR_TYPE::VTOL_CB || sensor->type == SENSOR_TYPE::INDIRECT_CB);
}
...
void Droid::actionSanity()
{
  ...
  bool avoidOverkill = pimpl->order->type != ORDER_TYPE::ATTACK &&
                       (pimpl->action == ACTION::ATTACK || pimpl->action == ACTION::MOVE_FIRE ||
  ...
  for (auto i = 0; i < MAX_WEAPONS; i++)
  {
    ...
    if (!pimpl->actionTargets[i] || !(avoidOverkill
    ...
    if (!isVtol()) continue;
    if ((pimpl->order->type == ORDER_TYPE::PATROL || pimpl->order->type == ORDER_TYPE::CIRCLE) &&
        (!vtolEmpty(*this) || (pimpl->secondaryOrder & DSS_ALEV_MASK) == DSS_ALEV_NEVER)) {
      // Back to the patrol.
      newAction(this, ACTION::MOVE, pimpl->order->pos);
      continue;
    }
...
void Droid::actionDroidBase(Action const* psAction)
{
  ...
  switch (psAction->action) {
    using enum ACTION;
    case NONE:
      // clear up whatever the droid was doing before if necessary
      if (!isStationary()) {
        moveStopDroid();
      }
    ...
    case ACTION::ATTACK:
      if (numWeapons(*this) == 0 || isTransporter(*this) ||
          psAction->targetObject == this || !hasValidWeapon) {
        break;
      }

      if (hasElectronicWeapon() && dynamic_cast<Structure *>(psAction->targetObject) &&
      ...
      // note the droid's current pos so that scout & patrol orders
      // know how far the droid has gone during an attack
      pimpl->actionPos = Vector2i().xy();
      setActionTarget(psAction->targetObject, 0);

      if ((pimpl->order->type == ORDER_TYPE::ATTACK_TARGET || pimpl->order->type == ORDER_TYPE::NONE ||
           pimpl->order->type == ORDER_TYPE::HOLD || pimpl->order->type == ORDER_TYPE::GUARD && hasCommander() ||
           pimpl->order->type == ORDER_TYPE::FIRE_SUPPORT) && secHoldActive ||
          !isVtol() && orderStateObj(this, ORDER_TYPE::FIRE_SUPPORT) != nullptr) {
        pimpl->action = ATTACK; // holding, try attacking straight away
        break;
      }

      // too close?
      if (!targetInsideFiringDistance(this, psAction->targetObject, psWeapStats)) {
        if (pimpl->order->type == ORDER_TYPE::HOLD)
          break;

        if (secondaryGetState(SECONDARY_ORDER::HALT_TYPE) == DSS_HALT_HOLD) {
          pimpl->action = ATTACK;
          break;
        }
      ...
      if (proj_Direct(psWeapStats) && pimpl->order->type != ORDER_TYPE::HOLD &&
        ...
        pimpl->action = MOVE_TO_ATTACK;
        auto fallbackpos = getFallbackPosition(this, psAction->targetObject);
        moveDroidTo(this, fallbackpos);
        break;
      }
    ...
    case MOVE_TO_REARM:
      ...
      if (!findVtolLandingPosition(this, &pos)) {
        ...
        orderDroid(this, ORDER_TYPE::RETURN_TO_BASE);
        break;
      }
    ...
    case REPAIR:
      ...
      else if (pimpl->order->type == ORDER_TYPE::REPAIR) {
        pimpl->action = MOVE_TO_REPAIR;
        moveDroidTo(this, psAction->location);
      }
    ...
    case OBSERVE:
      ...
      if (secondaryGetState(SECONDARY_ORDER::HALT_TYPE) != DSS_HALT_GUARD &&
        ...
        pimpl->action = visibleObject(
                this, pimpl->actionTargets[0], false)
                        ? OBSERVE : NONE;
        break;
      }
      if (hasCbSensor() || !(!secHoldActive && pimpl->order->type != ORDER_TYPE::HOLD ||
  ...
  switch (pimpl->order->type) {
    ...
    case HOLD:
      ...
      if (hasCommander() && pimpl->order->type != HOLD &&
        ...
        orderDroidObj(this, GUARD, pimpl->commander);
      }
      else if (isTransporter(*this)) break;
      // default to guarding
    ...
    case RTR_SPECIFIED:
      if (pimpl->order->target == nullptr) {
        ...
        pimpl->order = std::make_unique<Order>(NONE);
        orderDroid(this, RETURN_TO_REPAIR);
        break;
      }
      if (pimpl->action != ACTION::NONE) break;
    ...
    case GUARD:
      ...
      if (pimpl->commander->getAction() == ACTION::ATTACK &&
        ...
        psObj = pimpl->commander->pimpl->actionTargets[0];
  ...
  switch (pimpl->action) {
    ...
    case WAIT_DURING_REPAIR:
      ...
      if (pimpl->order->type == ORDER_TYPE::RETURN_TO_REPAIR &&
        ...
        if (isStationary() &&
            !adjacentToBuildSite(this, dynamic_cast<Structure*>(pimpl->order->target)->getStats(),
                                 {pimpl->order->target->getPosition().x, pimpl->order->target->getPosition().y},
                                 dynamic_cast<Structure*>(pimpl->order->target)->getRotation().direction)) {
        ...
        if (isStationary() && pimpl->action != ATTACK) {
          ...
          if (pimpl->order->type == ORDER_TYPE::ATTACK_TARGET && secHoldActive) {
            ...
            if (proj_Direct(psWeapStats) && pimpl->order->type != ORDER_TYPE::HOLD) {
              ...
              auto pos = getFallbackPosition(this, pimpl->actionTargets[0]);
              moveDroidTo(this, pos);
            }
            else {
              if (psWeapStats->rotate) {
                pimpl->action = ATTACK;
    ...
    case RESTORE:
      ...
      if (isStationary() && !adjacentToBuildSite(this, pimpl->order->structure_stats.get(),
                                                 pimpl->actionPos, pimpl->actionTargets[0]->getRotation().direction)) {
        if (pimpl->order->type != ORDER_TYPE::HOLD &&
            (!secHoldActive || secHoldActive && pimpl->order->type != ORDER_TYPE::NONE)) {
      ...
      if (!isStationary() && pimpl->movement->status != MOVE_STATUS::TURN_TO_TARGET &&
          pimpl->movement->status != MOVE_STATUS::SHUFFLE &&
          adjacentToBuildSite(this, pimpl->order->structure_stats.get(), pimpl->actionPos,
    ...
    case MOVE_TO_REPAIR_POINT:
    {
      if (pimpl->order->rtrType == RTR_DATA_TYPE::REPAIR_FACILITY) {
        ...
        if (adjacentToBuildSite(this, dynamic_cast<Structure*>(pimpl->actionTargets[0])->getStats(),
                                {pimpl->actionTargets[0]->getPosition().x, pimpl->actionTargets[0]->getPosition().y},
                                pimpl->actionTargets[0]->getRotation().direction)) {
      ...
      if (pimpl->order->rtrType != RTR_DATA_TYPE::DROID)
        break;
    ...
    case OBSERVE:
    {
      ...
      auto const xdiff1 = getPosition().x - pimpl->actionTargets[0]->getPosition().x;
      auto const ydiff1 = getPosition().y - pimpl->actionTargets[0]->getPosition().y;
      auto const rangeSq1 = droidSensorRange(this) * droidSensorRange(this);

      if (visibleObject(this, pimpl->actionTargets[0], false) &&
          xdiff1 * xdiff1 + ydiff1 * ydiff1 < rangeSq1)
        break;
    ...
    case MOVE_TO_OBSERVE:
      ...
      if (visibleObject(this, pimpl->actionTargets[0], false)) {
        ...
        auto rangeSq = droidSensorRange(this) * droidSensorRange(this);

        if (xdiff * xdiff + ydiff * ydiff < rangeSq && !isStationary()) {
    ...
    case FIRE_SUPPORT:
    {
      ...
      ASSERT_OR_RETURN(, (dynamic_cast<Droid*>(pimpl->order->target) || dynamic_cast<Structure*>(pimpl->order->target)) &&
      ...
      auto diff1 = (getPosition() - pimpl->order->target->getPosition()).xy();
      //Consider .shortRange here
      auto rangeSq1 = weaponManager->weapons[0].stats->upgraded[playerManager->getPlayer()].maxRange / 2;
      // move close to sensor
      rangeSq1 *= rangeSq1;

      if (isStationary()) {
        if (dot(diff1, diff1) < rangeSq1)
          break;
        ...
        diff1 = pimpl->order->target->getPosition().xy() - pimpl->movement->destination;
        if (dot(diff1, diff1) == rangeSq1)
          break;
    ...
    case MOVE_TO_DROID_REPAIR:
    {
      ...
      ASSERT_OR_RETURN(, actionTargetObj != nullptr && dynamic_cast<Droid*>(actionTargetObj), "unexpected repair target");
    ...
    case DROID_REPAIR:
    {
      ...
      // Just self-repairing.
      // See if there's anything to shoot.
      else if (numWeapons(*this) > 0 && !isVtol() && (pimpl->order->type == ORDER_TYPE::NONE ||
                pimpl->order->type == ORDER_TYPE::HOLD || pimpl->order->type == ORDER_TYPE::RETURN_TO_REPAIR)) {
        for (auto i = 0; i < numWeapons(*this); ++i)
        {
          if (!nonNullWeapon[i])
            continue;
          ...
          BaseObject*psTemp1 = nullptr;
          auto const psWeapStats1 = weaponManager->weapons[i].stats;
          if (psWeapStats1->rotate && secondaryGetState(SECONDARY_ORDER::ATTACK_LEVEL) == DSS_ALEV_ALWAYS &&
              aiBestNearestTarget(&psTemp1, i) >= 0 && psTemp1) {
            pimpl->action = ATTACK;
            setActionTarget(psTemp1, 0);
            break;
          }
        }
      }
    ...
    case CLEAR_REARM_PAD:
      ...
      // Droid has cleared the rearm pad without getting rearmed. One way this can happen
      // if a rearming pad was built under the VTOL while it was waiting for a pad
      if (!vtolHappy(*this)) {
    ...
    case MOVE_TO_REARM:
    {
      ...
      auto pos1 = pimpl->actionTargets[0]->getPosition().xy();
      if (!findVtolLandingPosition(this, &pos1)) {
        // totally bunged up - give up
      ...
      objTrace(getId(), "moving to rearm pad at %d,%d (%d,%d)", (int) pos1.x, (int) pos1.y,
               (int)(pos1.x/TILE_UNITS), (int)(pos1.y/TILE_UNITS));

      moveDroidToDirect(pos1);
    }
    default:
...
void Droid::setUpBuildModule()
{
  ...
  else if (nextModuleToBuild(psStruct, -1) > 0 && droidStartBuild()) {
    pimpl->action = ACTION::BUILD;
    return;
  }
  cancelBuild();
                       ...
                       bool isDamagePerSecond, int minDamage)
{
  ...
  auto relativeDamage = objDamage(this, damage, damageManager->getOriginalHp(),
                                  weaponClass, weaponSubClass, isDamagePerSecond, minDamage);
  ...
  else if (selectedPlayer < MAX_PLAYERS && !aiCheckAlliances(playerManager->getPlayer(), selectedPlayer)) {
    scoreUpdateVar(WD_UNITS_KILLED);
  }

  // Do we have a dying animation?
  if (getDisplayData()->imd_shape->objanimpie[ANIM_EVENT_DYING] && getAnimationEvent() != ANIM_EVENT_DYING) {
  ...
  if (getAnimationEvent() == ANIM_EVENT_DYING)
    return relativeDamage;

  debug(LOG_DEATH, "%s droid %d (%p) is toast", objInfo(this), (int)getId(), static_cast<void*>(this));
...
{
  if (!pimpl || damageManager->isDead() ||
      pimpl->type != DROID_TYPE::SENSOR && numWeapons(*this) == 0)
    return;
  ...
  if (aiChooseTarget(this, &psTarget, 0, true, nullptr)) {
    if (!orderState(this, ORDER_TYPE::HOLD) &&
        secondaryGetState(SECONDARY_ORDER::HALT_TYPE) == DSS_HALT_PURSUE) {
...
{
  ASSERT_OR_RETURN(false, pimpl != nullptr, "Droid is null");
  ...
  if (game.maxPlayers > 0 && isTransporter(*this)) {
    fpathSetDirectRoute(location);
  }
  else {
    retVal = fpathDroidRoute(this, location, moveType);
  }
  ...
  if (retVal == OK) {
    ...
    auto pos = Vector2i{pimpl->movement->destination.x, pimpl->movement->destination.y};
    objTrace(getId(), "unit %d: path ok - base Speed %u, speed %d, target(%u|%d, %u|%d)",
             getId(), pimpl->baseSpeed, pimpl->movement->speed,
             pos.x, map_coord(pos.x), pos.y, map_coord(pos.y));
  ...
  if (retVal == WAIT) {
    ...
    pimpl->movement->destination = location;
    return true;
  }

  objTrace(getId(), "Path to (%d, %d) failed for droid %d", (int)location.x, (int)location.y, (int)getId());
...
{
  ASSERT_OR_RETURN(, pimpl != nullptr, "Droid is null");
...
void Droid::moveTurnDroid(Vector2i location)
{
  ASSERT_OR_RETURN(, pimpl != nullptr, "Droid is null");
  auto moveDir = calcDirection(getPosition().x, getPosition().y,
                               location.x, location.y);

  if (getRotation().direction == moveDir)
    return;

  pimpl->movement->target = location;
  pimpl->movement->status = MOVE_STATUS::TURN_TO_TARGET;
}
...
void Droid::moveShuffleDroid(Vector2i s)
{
  ...
  {
    auto psCurr = dynamic_cast<Droid const*>(gi);
...
bool Droid::droidSensorDroidWeapon(const BaseObject* psObj) const
{
  ...
  if (!dynamic_cast<const Droid*>(psObj) && !dynamic_cast<const Structure*>(psObj)) {
    return false;
  }
  ...
  // - check the right droid/sensor combination
  // - check vtol droid with commander
  if ((isVtol() || !proj_Direct(weaponManager->weapons[0].stats.get())) &&
      psDroid != nullptr && psDroid->getType() == DROID_TYPE::COMMAND) {
    return true;
  }
  ...
  return (psStats->type == STANDARD || psStats->type == INDIRECT_CB || psStats->type == SUPER) &&
         !proj_Direct(weaponManager->weapons[0].stats.get()) || isVtol() &&
         (psStats->type == VTOL_INTERCEPT || psStats->type == VTOL_CB || psStats->type == SUPER);
}
 ...
 */
bool Droid::secondarySetState(SECONDARY_ORDER sec, SECONDARY_STATE state)
{
 ...
 * Called everytime to send a direct order to a droid
 */
void Droid::orderDroidBase(Order* psOrder)
{
  ...
  Vector3i const rPos(psOrder->pos, 0);
  ...
  using enum ORDER_TYPE;
  if (psOrder->type != TRANSPORT_IN && psOrder->target == nullptr &&
  ...
  if (!isTransporter(*this) && psOrder->type != NONE &&
    ...
    unsigned const oldState = pimpl->secondaryOrder;
  ...
  if (pimpl->type == DROID_TYPE::COMMAND && pimpl->group != nullptr &&
      pimpl->group->isCommandGroup() && psOrder->type != GUARD &&
      psOrder->type != RETURN_TO_REPAIR && psOrder->type != RECYCLE) {

    if (psOrder->type == ATTACK) {
      // change to attack target so that the group members guard order does not get canceled
    ...
    if (psOrder->type == RECOVER && !pimpl->group->getMembers()->empty()) {
      psOrder->type = NONE;
    }
  }
  ...
  switch (psOrder->type) {
    ...
    case MOVE:
    case SCOUT:
      // - cannot move vtols to blocking tiles
      // - in multiplayer, cannot move Transporter to blocking tile either
      if (isVtol() && fpathBlockingTile(map_coord(psOrder->pos), psPropStats->propulsionType) ||
          game.type == LEVEL_TYPE::SKIRMISH && isTransporter(*this) &&
    ...
    case TRANSPORT_OUT:
    case TRANSPORT_RETURN:
      // - tell a (transporter) droid to leave home base for the off-world mission
    ...
    case ATTACK_TARGET:
      ...
      if (pimpl->order->type == GUARD && psOrder->type == ATTACK_TARGET) {
      ...
      if (!psOrder->target || psOrder->target->damageManager->isDead() ||
          game.type == LEVEL_TYPE::SKIRMISH && hasElectronicWeapon() &&
      ...
      if (isVtol() || withinRange(this, psOrder->target, 0) ||
          (psOrder->type == ATTACK_TARGET || psOrder->type == ATTACK) &&
          secondaryGetState(SECONDARY_ORDER::HALT_TYPE) == DSS_HALT_HOLD) {
    ...
    case LINE_BUILD:
      ...
      objTrace(getId(), "Starting new construction effort of %s", psOrder->structure_stats
    ...
    case BUILD_MODULE:
      ...
      if (!isConstructionDroid(this) || psOrder->index < nextModuleToBuild(
              dynamic_cast<Structure*>(psOrder->target), -1)) {
        break;
      }
    ...
    case HELP_BUILD:
      ...
      if (pimpl->action == ACTION::BUILD &&
          psOrder->target == pimpl->actionTargets[0] &&
      ...
      newAction(this, ACTION::BUILD, pimpl->order->pos);
      objTrace(getId(), "Helping construction of %s", psOrder->structure_stats
                                                      ? getStatsName(pimpl->order->structure_stats)
                                                      : "NULL");
      break;
    ...
    case COMMANDER_SUPPORT:
      ...
      if (cmdDroidAddDroid(dynamic_cast<Droid*>(psOrder->target), this) &&
          playerManager->isSelectedPlayer()) {
        orderPlayFireSupportAudio(psOrder->target);
        break;
      }
    ...
    case RETURN_TO_BASE: {
      for (auto const &psStruct: playerList[playerManager->getPlayer()].structures)
      {
        if (psStruct.getStats()->type != STRUCTURE_TYPE::HQ)
          continue;

        auto pos1 = psStruct.getPosition().xy();
        pimpl->order = std::make_unique<Order>(*psOrder);
        ...
        if (isVtol() || game.type == LEVEL_TYPE::SKIRMISH && isTransporter(*this)) {
          findVtolLandingPosition(this, &pos1);
        }
        newAction(this, ACTION::MOVE, {pos1.x, pos1.y});
        break;
      }
      ...
      if (getLandingX(playerManager->getPlayer()) &&
          getLandingY(playerManager->getPlayer())) {
        pimpl->order = std::make_unique<Order>(*psOrder);
        newAction(this, ACTION::MOVE, {getLandingX(playerManager->getPlayer()), getLandingY(playerManager->getPlayer())});
        break;
      }
    ...
    case RTR_SPECIFIED:
    {
      ...
      if (psOrder->rtrType == RTR_DATA_TYPE::NO_RESULT || !psOrder->target) {
      ...
      if (rtrData.type == RTR_DATA_TYPE::REPAIR_FACILITY) {
        ...
        newAction(this, ACTION::MOVE, rtrData.target, pimpl->order->pos);
        break;
      }

      /* give repair order if repair droid found */
      if (rtrData.type == RTR_DATA_TYPE::DROID && !isTransporter(*this)) {
        pimpl->order = std::make_unique<Order>(psOrder->type, Vector2i(rtrData.target->getPosition().x,
                                                        rtrData.target->getPosition().y), RTR_DATA_TYPE::DROID);
    ...
    case EMBARK:
    {
      ...
      // require a transporter for embarking
      // nor can a transporter load another transporter
      if (isTransporter(*this) || embarkee == nullptr || !isTransporter(*embarkee)) {
    ...
    case DISEMBARK:
      ...
      if (!bMultiPlayer || !isTransporter(*this))
        break;
    ...
    case RECYCLE:
      ...
      for (auto& psStruct : playerList[playerManager->getPlayer()].structures)
      {
        ...
        if (psStruct.getStats()->type != FACTORY && psStruct.getStats()->type != CYBORG_FACTORY &&
            psStruct.getStats()->type != VTOL_FACTORY && psStruct.getStats()->type != REPAIR_FACILITY) {
          continue;
        }
      ...
      setTarget(psFactory);
      newAction(this, ACTION::MOVE, psFactory, pimpl->order->pos);
      break;
    ...
    case RESTORE:
      if (!dynamic_cast<Structure*>(psOrder->target) || !hasElectronicWeapon()) {
...
{
  ASSERT_OR_RETURN(false, pimpl != nullptr, "Droid object is null");
  if (isRepairLikeAction(pimpl->action))
  ...
  switch (pimpl->type) {
    ...
    case CYBORG_CONSTRUCT:
    {
      ...
      if (damaged.second != ACTION::BUILD)
        return true;
      ...
      newAction(this, damaged.second,
                {damaged.first->getPosition().x, damaged.first->getPosition().y});
...
std::unique_ptr<Droid> Droid::reallyBuildDroid(DroidTemplate const* pTemplate, Position pos,
                                               unsigned player, bool onMission, Rotation rot)
{
  ...
  psDroid->pimpl->type = droidTemplateType(pTemplate);
  ...
  if (!gameTimeIsStopped() && psDroid->getType() != DROID_TYPE::CONSTRUCT &&
      psDroid->getType() != DROID_TYPE::CYBORG_CONSTRUCT && psDroid->getType() != DROID_TYPE::REPAIRER &&
      psDroid->getType() != DROID_TYPE::CYBORG_REPAIR && !isTransporter(*psDroid) &&
  ...
  ASSERT(psDroid->damageManager->getHp() > 0, "Invalid number of hit points");

  psDroid->damageManager->setOriginalHp(psDroid->damageManager->getHp());
  psDroid->setImdShape(dynamic_cast<BodyStats const*>(
                               getComponent(COMPONENT_TYPE::BODY))->pIMD.get());
  ...
  if (isTransporter(*psDroid)) {
    ...
    if (player == selectedPlayer && !missionCanReEnforce()) {
      intAddTransporterLaunch(psDroid.get());
    }
  ...
  debug(LOG_LIFE, "created droid for player %d, droid = %p,"
        " id=%d (%s): position: x(%d)y(%d)z(%d)",
        player, static_cast<void*>(psDroid.get()), (int)psDroid->getId(),
...
std::unique_ptr<Droid> Droid::giftSingleDroid(unsigned to, bool electronic)
{
  ...
  if (bMultiPlayer && ( (int)getNumDroids(to) >= getMaxDroids(to) ||
                        (getType() == DROID_TYPE::CYBORG_CONSTRUCT || getType() == DROID_TYPE::CONSTRUCT) &&
  ...
  if (electronic || !bMultiPlayer) {
    ...
    sTemplate.name = WzString::fromUtf8(getName()->c_str()); // copy the name across

    // update score
    if (playerManager->getPlayer() == selectedPlayer && to != selectedPlayer && !bMultiPlayer) {
      scoreUpdateVar(WD_UNITS_LOST);
    }
    ...
    ASSERT_OR_RETURN(nullptr, psNewDroid != nullptr, "Unable to build unit");
  ...
  for (auto& psCurr : playerList[playerManager->getPlayer()].droids)
  {
    auto psLeader = psCurr.hasCommander()
                    ? psCurr.pimpl->commander
                    : orderStateObj(&psCurr, ORDER_TYPE::FIRE_SUPPORT);
  ...
  for (auto i = 0; i < MAX_PLAYERS; ++i)
  {
    ...
    {
      if (psCurr.getOrder()->target == this || psCurr.getTarget(0) == this) {
        orderDroid(&psCurr, ORDER_TYPE::STOP, ModeQueue);
        break;
      }
  ...
  for (auto i = 0; i < MAX_PLAYERS; ++i)
  {
    ...
    for (auto& psStruct : playerList[i].structures)
    {
      if (psStruct.getTarget(0) == this) {
        psStruct.setTarget(nullptr, 0, TARGET_ORIGIN::UNKNOWN);
      }
    }
  }

  triggerEventObjectTransfer(this, playerManager->getPlayer());
  return std::make_unique<Droid>(*this);
}

// Set the asBits in a Droid structure given its template.
void Droid::droidSetBits(DroidTemplate const* pTemplate)
{
  ASSERT_OR_RETURN(, pimpl != nullptr, "Droid object is undefined");
...
{
  ASSERT_OR_RETURN(, pimpl != nullptr, "Droid object is undefined");
  ASSERT(psNewTarget == nullptr || !psNewTarget->damageManager->isDead(),
...
{
  ASSERT_OR_RETURN(, pimpl != nullptr, "Droid object is undefined");
  ASSERT(psNewBase == nullptr || !psNewBase->damageManager->isDead(),
...
int droidReloadBar(Droid const& psDroid, Weapon const* psWeap, int weapon_slot)
{
	if (!psDroid.weaponManager || psDroid.weaponManager->weapons.empty()) {
		return -1;
	}

  // justifiable on when greater than a one second reload or intra salvo time
	auto const psStats = psWeap->stats;
  auto const player = psDroid.playerManager->getPlayer();
	auto const bSalvo = psStats->upgraded[player].numRounds > 1;
  ...
  auto firingStage = psDroid.isVtol()
                     ? getNumAttackRuns(&psDroid, weapon_slot) - psDroid.weaponManager->weapons[weapon_slot].ammoUsed
                     : gameTime - psWeap->timeLastFired;

  auto interval = psDroid.isVtol()
                  ? getNumAttackRuns(&psDroid, weapon_slot)
                  : bSalvo
                    ? weaponReloadTime(psStats.get(), player)
                    : weaponFirePause(psStats.get(), player);

  return firingStage < interval && interval > 0
         ? PERCENT(firingStage, interval)
         : 100;
}
...
static void removeDroidFX(Droid const* psDel, unsigned impactTime)
{
...
void destroyDroid(Droid* psDel, unsigned impactTime)
{
  ...
  // darken tile if las-sat
  if (psDel->damageManager->getLastHitWeapon() != WEAPON_SUBCLASS::LAS_SAT) {
  ...
  auto const mapX = map_coord(psDel->getPosition().x);
  auto const mapY = map_coord(psDel->getPosition().y);
...
{
	auto const pos = map_coord(psDroid->getPosition());
	auto const minY = std::max(pos.y - 1, 0);
	auto const minX = std::max(pos.x - 1, 0);
	auto const maxX = std::min(pos.x + 1, mapWidth);
	auto const maxY = std::min(pos.y + 1, mapHeight);
...
{
  return !psDroid->damageManager->isDead() &&
...
{
  audio_PlayStaticTrack(iVecEffect.x, iVecEffect.z,
                        ID_SOUND_CONSTRUCTION_1 + rand() % 4);
}

static void addConstructorEffect(Structure const* psStruct)
{
  if (!ONEINTEN || !psStruct->isVisibleToSelectedPlayer() || !(rand() % 2))
    return;

  auto const size = psStruct->getSize() * TILE_UNITS / 4;
  auto temp = Vector3i{psStruct->getPosition().x + (rand() % (2 * size.x) - size.x),
                       map_TileHeight(map_coord(psStruct->getPosition().x), map_coord(psStruct->getPosition().y))
                           + psStruct->getDisplayData()->imd_shape->max.y / 6,
                       psStruct->getPosition().y + (rand() % (2 * size.y) - size.y)};

  droidAddWeldSound(temp);
}

void Droid::droidUpdateRepair()
{
  ASSERT_OR_RETURN(, pimpl != nullptr , "Null object");
	ASSERT_OR_RETURN(, pimpl->action == ACTION::REPAIR, "unit does not have repair order");

	auto psStruct = dynamic_cast<Structure*>(pimpl->actionTargets[0]);
  auto construct = dynamic_cast<ConstructStats const*>(getComponent(COMPONENT_TYPE::CONSTRUCT));

  auto iRepairRate = construct != nullptr
                     ? constructorPoints(construct, playerManager->getPlayer())
                     : -1;
...
{
  auto repairStats = dynamic_cast<RepairStats const*>(psRepairDroid->getComponent(COMPONENT_TYPE::REPAIR_UNIT));
  ...
  auto iVecEffect = (psDroidToRepair->getPosition() + Vector3i(
...
bool vtolRearming(Droid const& droid)
{
  ...
  using enum ACTION;
  return action == MOVE_TO_REARM || action == WAIT_FOR_REARM ||
...
unsigned getNumAttackRuns(Droid const* psDroid, int weapon_slot)
{
  ...
  auto const& stats = psDroid->weaponManager->weapons[weapon_slot].stats;

  // if weapon is a salvo weapon, then number of shots that can be fired = vtolAttackRuns * numRounds
  return stats->upgraded[psDroid->playerManager->getPlayer()].reloadTime
         ? stats->upgraded[psDroid->playerManager->getPlayer()].numRounds * stats->vtolAttackRuns
         : stats->vtolAttackRuns;
}

bool vtolHappy(Droid const& droid)
{
  return droid.isVtol() && !droid.isDamaged() &&
         (droid.getType() != DROID_TYPE::WEAPON || hasFullAmmo(droid));
}
...
void updateVtolAttackRun(Droid& droid, int weapon_slot)
{
  ...
  if (weapon.stats->vtolAttackRuns == 0) {
    return;
  }
  ++droid.weaponManager->weapons[weapon_slot].ammoUsed;

  if (droid.weaponManager->weapons[weapon_slot].ammoUsed ==
        getNumAttackRuns(&droid, weapon_slot)) {
    droid.weaponManager->weapons[weapon_slot].ammo = 0;
  }
}
...
bool checkValidWeaponForProp(DroidTemplate const* psTemplate)
{
  ...
  if (psTemplate->weapons.empty() || psTemplate->weapons[0].stats->vtolAttackRuns ||
      asPropulsionTypes[(int) psPropStats->propulsionType].travel == TRAVEL_MEDIUM::AIR &&
      (!proj_Direct(psTemplate->weapons[0].stats.get()) || !psTemplate->weapons[0].stats.get()->vtolAttackRuns)) {
    return false;
  }

	// also checks that there is no other system component
  return psTemplate->getComponent(COMPONENT_TYPE::BRAIN) == nullptr ||
         psTemplate->weapons[0].stats->weaponSubClass == WEAPON_SUBCLASS::COMMAND;
}

// Check if a droid can be selected.
bool isSelectable(Droid const* psDroid)
{
  return !isTransporter(*psDroid) || bMultiPlayer ||
...
{
	if (psDroid == nullptr || psDroid->damageManager->isDead()) {
		return;
	}
	psDroid->setAudioId(NO_SOUND);
}

bool isCyborg(Droid const& droid)
{
  using enum DROID_TYPE;
  return droid.getType() == CYBORG || droid.getType() == CYBORG_CONSTRUCT ||
         droid.getType() == CYBORG_REPAIR || droid.getType() == CYBORG_SUPER;
}

bool isBuilder(Droid const& droid)
{
  using enum DROID_TYPE;
  return droid.getType() == CONSTRUCT || droid.getType() == CYBORG_CONSTRUCT;
}

bool droidOnMap(Droid const* psDroid)
{
  return psDroid->damageManager->getTimeOfDeath() == NOT_CURRENT_LIST ||
         isTransporter(*psDroid) || psDroid->getPosition().x == INVALID_XY ||
         psDroid->getPosition().y == INVALID_XY || missionIsOffworld() || mapHeight == 0 ||
         worldOnMap(psDroid->getPosition().x, psDroid->getPosition().y);
}
...
int droidSqDist(Droid const* psDroid, BaseObject const* psObj)
{
  ...
  if (psPropStats == nullptr) return -1;

  return !fpathCheck(psDroid->getPosition(), psObj->getPosition(), psPropStats->propulsionType)
         ? -1
         : objectPositionSquareDiff(psDroid->getPosition(), psObj->getPosition());
}

unsigned calculateMaxRange(Droid const& droid)
{
  if (numWeapons(droid) == 0)
    return 0;

  return droid.getType() == DROID_TYPE::SENSOR
         ? droid.calculateSensorRange()
         : getMaxWeaponRange(droid);
}

bool transporterIsFlying(Droid const& transporter)
{
  ASSERT_OR_RETURN(false, isTransporter(transporter), "Non-transport droid given to transporter function");
...
bool stillBuilding(Droid const& droid)
{
  return !droid.damageManager->isDead() && droid.getAction() == ACTION::BUILD;
}

bool canAssignFireSupport(Droid const& droid, Structure const& structure)
{
  return numWeapons(droid) > 0 && structure.hasSensor() &&
         (droid.isVtol() && (structure.hasVtolInterceptSensor() || structure.hasVtolCbSensor())) ||
         (hasArtillery(droid) && (structure.hasStandardSensor() || structure.hasCbSensor()));
}

bool vtolCanLandHere(Vector2i position)
{
  if (!tileOnMap(position)) {
    return false;
  }

  auto const tile = mapTile(position);
  return !(tile->tileInfoBits & AUXBITS_BLOCKING) && !TileIsOccupied(tile) &&
...
{
  auto start_pos = map_coord(position);
  setBlockingFlags(vtol, AUXBITS_BLOCKING);
  ...
  clearBlockingFlags(vtol, AUXBITS_BLOCKING);
  return landing_tile;
}
...
void setBlockingFlags(Droid const& droid, uint8_t flag)
{
  auto const& droids = playerList[droid.playerManager->getPlayer()].droids;

  std::for_each(droids.begin(), droids.end(), [&](auto const& other_droid) {
...
void clearBlockingFlags(Droid const& droid, uint8_t flag)
{
  auto const& droids = playerList[droid.playerManager->getPlayer()].droids;
...
bool tileOccupiedByDroid(Vector2i position)
{
  for (auto& player : playerList)
  {
    auto tileOccupied = std::any_of(player.droids.begin(), player.droids.end(),
                    [&position](auto const& droid) {
        return map_coord(droid.getPosition().x) == position.x &&
               map_coord(droid.getPosition().y == position.y);
    });
...
bool Droid::loadSaveDroid(const char* pFileName)
{
  ...
  for (auto& i : list)
  {
    ...
    switch (droidType) {
      ...
      case DROID_TYPE::COMMAND:
        ...
        if (!missionList || !getDroidsToSafetyFlag()) {
          ++priority;
        }
      default:
        break;
    }
    sortedList.emplace_back(-priority, i);
    ini.endGroup();
  }
  ...
  for (auto& pair : sortedList)
  {
    ...
    if (skipForDifficulty(ini, getPlayer(ini))) {
      ini.endGroup();
      continue;
    }
    ...
    if (ini.contains("template")) {
      ...
      if (psTemplate == nullptr) {
        debug(LOG_ERROR, "Unable to find template for %s for player %d -- unit skipped",
              templName.toUtf8().c_str(), getPlayer(ini));
        ini.endGroup();
        continue;
      }
    }
    ...
    auto psDroid = reallyBuildDroid(psTemplate, pos, getPlayer(ini), onMission, rot).get();
    ...
    if (psDroid->pimpl->movement->status == MOVE_STATUS::WAIT_FOR_ROUTE) {
      ...
      auto dr = fpathDroidRoute(psDroid, psDroid->getMovementData()->destination, FPATH_MOVETYPE::FMT_MOVE);
    ...
    auto startpos = getPlayerStartPosition(getPlayer(ini));
...
int Droid::aiBestNearestTarget(BaseObject** ppsObj, int weapon_slot, int extraRange) const
{
  ...
  if (proj_Direct(weaponManager->weapons[weapon_slot].stats.get()) && targetStructure &&
      ...
      asStructStrengthModifier[weaponEffect][targetStructure->getStats()->strength]
        >= MIN_STRUCTURE_BLOCK_STRENGTH) {
...
BaseObject* Droid::findTarget() const
{
  ASSERT_OR_RETURN(nullptr, pimpl != nullptr, "Null object");
  ASSERT_OR_RETURN(nullptr, pimpl->type == DROID_TYPE::SENSOR,
  ...
  if (pimpl->action != ACTION::OBSERVE ||
      sensor && sensor->type == SENSOR_TYPE::VTOL_CB ||
      sensor->type == SENSOR_TYPE::VTOL_INTERCEPT || objRadarDetector(this)) {
    return nullptr;
  }
  return pimpl->actionTargets[0];
}
