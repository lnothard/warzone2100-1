...
void Droid::upgradeHitPoints() {
  ...
  auto prev = damageManager->getOriginalHp();
  damageManager->setOriginalHp(calcDroidBaseBody(this));
  auto increase = damageManager->getOriginalHp() * factor / prev;
...
void Droid::updateExpectedDamage(unsigned damage, bool isDirect) noexcept {
  if (isDirect) {
    damageManager->setExpectedDamageDirect(
            damageManager->getExpectedDamageDirect() + damage);
  }
  else {
    damageManager->setExpectedDamageIndirect(
            damageManager->getExpectedDamageIndirect() + damage);
  }
}
...
void Droid::orderCheckGuardPosition(int range)
{
  ...
  if (xdiff * xdiff + ydiff * ydiff <= range * range) return;
...
void Droid::actionDroidBase(Action* psAction)
{
  ...
  ASSERT_OR_RETURN(, psAction->psObj == nullptr ||
                     !psAction->psObj->damageManager->isDead(),
                    "Droid is dead");
...
void Droid::orderUpdateDroid()
{
  ...
  // clear the target if it has died
  if (pimpl->order->target &&
      pimpl->order->target->damageManager->isDead()) {
    setTarget(nullptr);
  ...
  if (pimpl->associatedStructure &&
      pimpl->associatedStructure->damageManager->isDead()) {
    setBase(nullptr);
  ...
  if (damageManager->isDead()) return;
  ...
  switch (pimpl->order->type) {
    ...
    case TRANSPORT_OUT:
      ...
      if (playerManager->getPlayer() != selectedPlayer) break;
    ...
    case RTR_SPECIFIED:
      if (pimpl->order->target == nullptr) {
        ...
        if (objectPositionSquareDiff(getPosition(),
                                     pimpl->order->target->getPosition()) < (TILE_UNITS * 8) * (TILE_UNITS * 8)) {
    ...
    case LINE_BUILD:
    {
      ...
      auto lb = calcLineBuild(pimpl->order->structure_stats.get(),
    ...
    case GUARD:
      ...
      if (hasCommander() && (numWeapons(*this) > 0)) {
        ...
        if (psObj == nullptr ||
            psObj != dynamic_cast<BaseObject const*>(pimpl->group->getCommander())) {
          orderDroidObj(this, GUARD, pimpl->group->getCommander(), ModeImmediate);
        }
      }
  ...
  switch (pimpl->action) {
      ...
      if (bHasTarget) {
        ...
        for (auto i = 0; i < numWeapons(*this); ++i)
        {
          ...
          if (proj_Direct(psStats) && blockingWall) {
            ...
            if (!wallBlocked &&
                actionTargetTurret(this, psActionTarget1, pimpl->weapons[i])) {
              // In range - fire !!!
              combFire(pimpl->weapons[i], this, psActionTarget1, i);
            }
        }
    ...
    case FIRE_SUPPORT:
      ...
      if (!isVtol() && !dynamic_cast<Structure*>(pimpl->order->target)) {
        ...
        auto rangeSq = pimpl->weapons[0].getStats()->upgraded[playerManager->getPlayer()].maxRange / 2;
...
bool Droid::droidUpdateRestore()
{
  ...
  psStruct->damageManager->setResistance(
          psStruct->damageManager->getResistance() + (pointsToAdd - pimpl->actionPointsDone)));
...
void Droid::droidUpdate()
{
  ...
  if (pimpl->type == DROID_TYPE::REPAIRER || pimpl->type == DROID_TYPE::CYBORG_REPAIR) {
    for (auto& psOther : apsDroidLists[playerManager->getPlayer()])
    {
  ...
  switch (sec) {
    ...
    case ASSIGN_VTOL_PRODUCTION:
      ...
      if (pimpl->type == DROID_TYPE::COMMAND) {
        ...
        for (auto& psStruct : apsStructLists[playerManager->getPlayer()])
        {
...
RtrBestResult Droid::decideWhereToRepairAndBalance()
{
  ...
  if (pimpl->type != DROID_TYPE::REPAIRER &&
    ...
    auto psdroidList = !(apsDroidLists[playerManager->getPlayer()].empty())
                         ? apsDroidLists[playerManager->getPlayer()]
                         : mission.apsDroidLists[playerManager->getPlayer()];
...
void Droid::droidSetBits(const DroidTemplate* pTemplate)
{
  ...
  for (auto inc = 0; inc < MAX_WEAPONS; inc++)
  {
    ...
    if (inc < pTemplate->weaponCount) {
      pimpl->weapons[inc].ammo = pimpl->weapons[inc].getStats()->upgraded_stats[playerManager->getPlayer()].numRounds;
    }
...
int droidReloadBar(BaseObject const& psObj, Weapon const* psWeap, int weapon_slot)
{
  ...
  firingStage = getNumAttackRuns(psDroid, weapon_slot)
          - psDroid->getWeapon(weapon_slot)->ammoUsed;
...
bool removeDroidBase(Droid* psDel)
{
	...
	if (isTransporter(*psDel)) {
		if (psDel->getGroup()) {
			//free all droids associated with this Transporter
			for (auto psCurr : psDel->group->members)
			{
	...
	if (psDel->getType() == DROID_TYPE::COMMAND) {
		for (auto& psStruct : apsStructLists[psDel->playerManager->getPlayer()])
		{
	...
	if (psDel->playerManager->getPlayer() == selectedPlayer &&
      psDel->damageManager->isSelected() && isConstructionDroid(psDel)) {
		// If currently trying to build, kill off the placement
		if (tryingToGetLocation()) {
			auto numSelectedConstructors = 0;
			for (auto& psDroid : apsDroidLists[psDel->playerManager->getPlayer()]) {
...
DROID_TYPE droidTemplateType(DroidTemplate const* psTemplate)
{
  using enum DROID_TYPE;
	...
	else if (psTemplate->asParts[COMPONENT_TYPE::BRAIN] != 0)
	{
		type = COMMAND;
	}
	else if ((asSensorStats + psTemplate->asParts[COMPONENT_TYPE::SENSOR])->location == LOC::TURRET)
	{
		type = SENSOR;
	}
	else if ((asECMStats + psTemplate->asParts[COMPONENT_TYPE::ECM])->location == LOC::TURRET)
	{
		type = ECM;
	}
	else if (psTemplate->asParts[COMPONENT_TYPE::CONSTRUCT] != 0)
	{
		type = CONSTRUCT;
	}
	else if ((asRepairStats + psTemplate->asParts[COMPONENT_TYPE::REPAIR_UNIT])->location == LOC::TURRET)
	{
		type = REPAIRER;
	}
                               ...
                               F func, G propulsionFunc)
{
	...
	unsigned sum =
          func(asBrainStats[asParts[COMPONENT_TYPE::BRAIN]].upgraded[player]) +
          func(asSensorStats[asParts[COMPONENT_TYPE::SENSOR]].upgraded[player]) +
          func(asECMStats[asParts[COMPONENT_TYPE::ECM]].upgraded[player]) +
          func(asRepairStats[asParts[COMPONENT_TYPE::REPAIR_UNIT]].upgraded[player]) +
          func(asConstructStats[asParts[COMPONENT_TYPE::CONSTRUCT]].upgraded[player]) +
          propulsionFunc(asBodyStats[asParts[COMPONENT_TYPE::BODY]].upgraded[player],
		               asPropulsionStats[asParts[COMPONENT_TYPE::PROPULSION]].upgraded[player]);
	for (int i = 0; i < numWeaps; ++i)
	{
...
{
	auto speed = asPropulsionTypes[asPropulsionStats[psTemplate->asParts[COMPONENT_TYPE::PROPULSION]].propulsionType].
		powerRatioMult *
		bodyPower(&asBodyStats[psTemplate->asParts[COMPONENT_TYPE::BODY]], player) / MAX(1, weight);

	// reduce the speed of medium/heavy VTOLs
	if (asPropulsionStats[psTemplate->asParts[COMPONENT_TYPE::PROPULSION]].propulsionType == PROPULSION_TYPE::LIFT) {
		if (asBodyStats[psTemplate->asParts[COMPONENT_TYPE::BODY]].size == BODY_SIZE::HEAVY) {
			speed /= 4;
		}
		else if (asBodyStats[psTemplate->asParts[COMPONENT_TYPE::BODY]].size == BODY_SIZE::MEDIUM) {
			speed = speed * 3 / 4;
		}
	}

	// applies the engine output bonus if output > weight
	if (asBodyStats[psTemplate->asParts[COMPONENT_TYPE::BODY]].base.power > weight) {
		speed = speed * 3 / 2;
	}
	return speed;
}
...
{
  auto const& droids = apsDroidLists[player];
...
static bool oneDroidMax(unsigned x, unsigned y)
{
  ...
  bool bFound = false;
	for (auto i = 0; i < MAX_PLAYERS; i++)
	{
		for (auto const& pD : apsDroidLists[i])
		{
...
bool being_repaired(Droid const& droid)
{
  ...
  const auto& droids = apsDroidLists[droid.playerManager->getPlayer()];
...
void updateVtolAttackRun(Droid& droid, int weapon_slot)
{
  ...
  auto weapon = droid.getWeapon(weapon_slot);
...
bool checkValidWeaponForProp(DroidTemplate const* psTemplate)
{
	...
	if (psTemplate->asParts[COMPONENT_TYPE::BRAIN] != 0
...
{
  return !droid.damageManager->isDead() &&
...
Droid* find_nearest_droid(unsigned x, unsigned y, bool selected)
{
  ...
  std::for_each(droids.begin(), droids.end(), [&](auto& droid) {
      ...
      if (selected && !droid.damageManager->isSelected()) {
        return;
      }
...
{
  auto const& droids = apsDroidLists[droid.playerManager->getPlayer()];
  std::for_each(droids.begin(), droids.end(),
                [&droid](const auto &other_droid) {
      Vector2i tile{0, 0};
      if (other_droid.isStationary()) {
...
void Droid::setSaveDroidTarget(BaseObject* psNewTarget)
{
  ASSERT_OR_RETURN(, pimpl != nullptr, "Droid object is undefined");
  pimpl->order->target = psNewTarget;
}
...
{
  return interpolateRot(psDroid->getWeapon(weaponSlot)->getPreviousRotation(),
