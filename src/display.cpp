...
void processMouseClickInput()
{
	...
	else if (selection != SC_INVALID)
	{
		...
		if (item != MT_NOTARGET) {
			...
			else if (specialOrderKeyDown() && ObjUnderMouse != nullptr &&
			         ObjUnderMouse->player == selectedPlayer) {
			...
			else if (selection == SC_DROID_REPAIR)
			{
				...
				{
					if (psCurr.selected) {
						if (ObjUnderMouse != nullptr && ObjUnderMouse->player == selectedPlayer &&
                psCurr.id == ObjUnderMouse->id) {
							item = MT_BLOCKING;
						}
						break;
					}
...
BASE_OBJECT *mouseTarget()
{
	...
	for (int i = 0; i < MAX_PLAYERS; i++)
	{
      ...
      if (psReturn) break;
          ...
          mouseInBox(dispX - dispR, dispY - dispR, dispX + dispR, dispY + dispR)) {
...
bool deliveryReposValid()
{
	...
	auto map = map_coord(flagPos.coords.xy());
	...
	{
		auto flagTile = map_coord(psCurrFlag.coords.xy());
		if (flagTile == map) {
			return false;
		}
	}

  return !fpathBlockingTile(map.x, map.y, PROPULSION_TYPE_WHEELED);
}
	...
	else if (psDroid->droidType == DROID_SENSOR)
	{
		...
		for (auto& psCurr : apsDroidLists[selectedPlayer])
		{
          ...
          (!proj_Direct(asWeaponStats + psCurr.asWeaps[0].nStat) ||
           isVtolDroid(&psCurr)) &&
          droidSensorDroidWeapon(psDroid, &psCurr))
			{
				bSensorAssigned = true;
...
bool	anyDroidSelected(UDWORD player)
{
  ...
  return std::any_of(apsDroidLists[player].begin(),
                     apsDroidLists[player].end(),
                     [](auto const& psCurr) {
    return psCurr.selected;
  });
}
...
void clearSelection()
{
	...
	for (auto& psCurrDroid : apsDroidLists[selectedPlayer])
	{
