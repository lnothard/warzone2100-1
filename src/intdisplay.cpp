...
static void StatGetResearchImage(BASE_STATS* psStat, Image* image, iIMDShape** Shape, BASE_STATS** ppGraphicData,
                                 bool drawTechIcon);
...
void PowerBar::display(int xOffset, int yOffset)
{
	...
	displayPower = desiredPower + (displayPower - desiredPower) * exp((realTime - lastRealTime) / -80.);
	// If realTime < lastRealTime, then exp() returns 0 due to unsigned overflow.
	...
	if (textWidth > 0)
	{
		iV_DrawImageRepeatX(IntImages, IMAGE_PBAR_EMPTY, x0 - 1, y0, textWidth + 1, defaultProjectionMatrix(), true,
		                    &imageDrawBatch); // Overdraw by 1 to reduce seam with left-beginning-piece when scaling
		x0 += textWidth;
	}
	...
	if (Empty > 0)
	{
		iV_DrawImageRepeatX(IntImages, IMAGE_PBAR_EMPTY, x0, y0, Empty + 1, defaultProjectionMatrix(), true,
		                    &imageDrawBatch); // Overdraw by 1 to reduce seam with right-end-piece when scaling
		x0 += Empty;
	}
	...
	if (showNeedMessage)
	{
		...
		pie_UniTransBoxFill(textX - 3, y0 + 1, textX + needTextWidth + 3, y0 + this->height() - 1,
		                    WZCOL_TRANSPARENT_BOX);
...
void IntFancyButton::displayIfHighlight(int xOffset, int yOffset)
{
	if (isHighlighted())
	{
		iV_DrawImage(IntImages, buttonType == TOPBUTTON ? IMAGE_BUT_HILITE : IMAGE_BUTB_HILITE, xOffset + x(),
		             yOffset + y());
	}
}
...
void IntFormAnimated::display(int xOffset, int yOffset)
{
	...
	{
	case 1: FormOpenCount = 0;
		break;
	case 4: FormCloseCount = 0;
		break;
	}
	switch (currentAction)
	{
...
void drawRadarBlips(int radarX, int radarY, float pixSizeH, float pixSizeV, const glm::mat4& modelViewProjection)
{
	...
	static const uint16_t imagesBurningResource[] = {
		IMAGE_RAD_BURNRESREAD, IMAGE_RAD_BURNRES1, IMAGE_RAD_BURNRES2, IMAGE_RAD_BURNRES3, IMAGE_RAD_BURNRES4,
		IMAGE_RAD_BURNRES5, IMAGE_RAD_BURNRES6
	};
			...
			if (psProxDisp->psMessage->dataType == MSG_DATA_BEACON)
			{
				...
				if (pViewData->type == VIEW_BEACON)
				{
					...
					if (pViewData->pData != nullptr && (((VIEW_PROXIMITY*)pViewData->pData)->timeAdded + 60000) <=
						gameTime)
					{
						debug(LOG_MSG, "blip timeout for %d, from %d", i,
						      (((VIEW_PROXIMITY *)pViewData->pData)->sender));
	...
	/* Go through all the proximity Displays */
	for (psProxDisp = (selectedPlayer < MAX_PLAYERS) ? apsProxDisp[selectedPlayer] : nullptr; psProxDisp != nullptr;
