...
struct PathNonblockingArea
{
	...
	PathNonblockingArea(StructureBounds const& st) : x1(st.map.x), x2(st.map.x + st.size.x), y1(st.map.y),
	                                                 y2(st.map.y + st.size.y)
	{
	}
...
void fpathSetBlockingMap(PATHJOB* psJob)
{
	...
	// Find the map.
	auto i = std::find_if(fpathBlockingMaps.begin(), fpathBlockingMaps.end(),
	                      [&](std::shared_ptr<PathBlockingMap> const& ptr)
	                      {
		                      return *ptr == type;
	                      });
	if (i == fpathBlockingMaps.end())
	{
		...
		syncDebug("blockingMap(%d,%d,%d,%d) = %08X %08X", gameTime, psJob->propulsion, psJob->owner, psJob->moveType,
		          checksumMap, checksumDangerMap);
