...
void Structure::aiUpdateStructure(bool isMission)
{
  ...
  if (getTime() == gameTime) {
    ...
    for (auto& s : apsStructLists[damageManager->getPlayer()])
    {
      count1 += s == this;
    }
    for (auto s : mission.apsStructLists[damageManager->getPlayer()])
    {
      count2 += s == this;
    }
    ...
    if (psDroid->getAction() == ACTION::WAIT_DURING_REARM &&
      ...
      if (pointsToAdd1 >= psDroid->getWeight()) // amount required is a factor of the droid weight
      {
        ...
        for (auto i1 = 0; i1 < numWeapons(*psDroid); i1++)
        {
          ...
          psDroid->weapons[i1].ammo = psDroid->getWeapons()[i1].getStats()->
        ...
        for (unsigned i2 = 0; i2 < numWeapons(*psDroid); i2++) // rearm one weapon at a time
        {
          ...
          if (psDroid->getWeapons()[i2].ammoUsed > 0 &&
              psDroid->getWeapons()[i2].getStats()->upgraded[
...
void Structure::buildingComplete()
{
  ...
  if (pimpl->prebuiltImd != nullptr) {
    ...
    auto imdIndex = std::min<int>(numStructureModules(this) * 2, IMDs.size() - 1);
    ...
    if (numWeapons(*pStructureType) > 0) {
      ...
      for (weapon = 0; weapon < pStructureType->numWeaps; weapon++)
      {
        if (pStructureType->psWeapStat[weapon]) {
          ...
          psBuilding->pimpl->weapons[weapon].ammo = psBuilding->pimpl->weapons[weapon].getStats()->upgraded[psBuilding->getPlayer()].numRounds;
        }
      }
    }
    else {
...
void Factory::refundBuildPower()
{
  ...
  if (pimpl->buildPointsRemaining < (int)calcTemplateBuild(pimpl->psSubject.get())) {
    ...
    addPower(getPlayer(), calcTemplatePower(pimpl->psSubject.get()));
  }
}
...
bool electronicDamage(BaseObject* psTarget, unsigned damage, uint8_t attackPlayer)
{
	...
	if (auto psStructure = dynamic_cast<Structure*>(psTarget)) {
		...
		if (psStructure->getResistance() < 0) {
			...
			psStructure->setResistance((SWORD)(psStructure->getResistance() - damage));
	...
	//droid electronic damage
		...
		if (psDroid->getResistance() == ACTION_START_TIME) {
			...
			psDroid->setResistance(droidResistance(psDroid));
		}
		...
		if (psDroid->getResistance() < 0) {
			...
			psDroid->setResistance((SWORD)(psDroid->getResistance() - damage));
...
void resetResistanceLag(Structure const* psBuilding)
{
  ...
  switch (psBuilding->getStats()->type) {
    ...
    case CYBORG_FACTORY:
      {
        auto psFactory = dynamic_cast<Factory const*>(psBuilding);
