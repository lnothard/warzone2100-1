...
static inline bool _isTransporter(DROID_TYPE type)
{
  return type == DROID_TYPE::TRANSPORTER || type == DROID_TYPE::SUPERTRANSPORTER;
}
...
{
  DROID_TYPE type = DROID_TYPE::DEFAULT;

  if (droidType == DROID_TYPE::PERSON ||
      droidType == DROID_TYPE::CYBORG ||
      droidType == DROID_TYPE::CYBORG_SUPER ||
      droidType == DROID_TYPE::CYBORG_CONSTRUCT ||
      droidType == DROID_TYPE::CYBORG_REPAIR ||
      droidType == DROID_TYPE::TRANSPORTER ||
      droidType == DROID_TYPE::SUPERTRANSPORTER)
  {
    type = droidType;
  }
  else if (asParts[COMP_BRAIN] != 0)
  {
    type = DROID_TYPE::COMMAND;
  }
  else if ((asSensorStats + asParts[COMP_SENSOR])->location == LOC_TURRET)
  {
    type = DROID_TYPE::SENSOR;
  }
  ...
  {
    type = DROID_TYPE::REPAIR;
  }
  else if (asWeaps[0] != 0 || numWeaps > 1)
  {
...
bool DroidStats::isTransporter() const
{
  return _isTransporter(droidType);
}

Droid::Droid(uint32_t id, unsigned player)
    : Unit(OBJECT_TYPE::DROID, id, player)
      , droidType(ANY)
      , psGroup(nullptr)
      , psGrpNext(nullptr)
      ...
      , actionPos(0, 0)
{
  ...
  activeOrder.type = DROID_ORDER::NONE;
  activeOrder.pos = Vector2i(0, 0);
  activeOrder.pos2 = Vector2i(0, 0);
  activeOrder.direction = 0;
  activeOrder.psObj = nullptr;
  activeOrder.psStats = nullptr;
...
{
  int32_t longRange = aiDroidRange(weapon_slot);
...
{
  if (weaponList[0].nStat == 0) return false;
  ...
  const int dx = (SDWORD)position.x - (SDWORD)targetObj.getPosition().x;
  const int dy = (SDWORD)position.y - (SDWORD)targetObj.getPosition().y;
...
bool Droid::actionInsideMinRange(const GameObject& targetObj, const WEAPON_STATS* droidWeapStats) const
{
  ...
  const int dx = position.x - targetObj.getPosition().x;
  const int dy = position.y - targetObj.getPosition().y;
  const int radSq = dx * dx + dy * dy;
...
void Droid::actionAddVtolAttackRun(const GameObject& targetDroid)
{
  ...
  Vector2i delta = (targetDroid.getPosition() - position).xy();
  ...
  Vector2i dest = targetDroid.getPosition().xy() + delta * VTOL_ATTACK_LENGTH / dist;
...
bool Droid::actionVTOLLandingPos(Vector2i *p)
{
  ...
  {
    Vector2i t(0, 0);
    if (droid.isStationary())
    {
      t = map_coord(((const GameObject&) droid).getPosition().xy());
    }
    else
    {
...
{
  if (!isVtolDroid()) return;

  //if droid is already returning - ignore
  if (vtolRearming()) return;
  ...
  if (psStruct)
  {
    ...
    if (activeOrder.type == DROID_ORDER_TYPE::NONE)
    {
      // no order set - use the rearm order to ensure the unit goes back
      // to the landing pad
      orderDroidObj(this, DROID_ORDER_TYPE::REARM, psStruct, ModeImmediate);
    }
    else
    {
      actionDroid(DROID_ACTION::MOVETOREARM, dynamic_cast<GameObject*>(psStruct));
    }
  }
  else
  {
    ...
    sendOrder(DROID_ORDER_TYPE::RTB, ModeImmediate);
  }
}
...
void Droid::actionSanity()
{
  ...
  bool avoidOverkill = activeOrder.type != DROID_ORDER_TYPE::ATTACK &&
    ...
    if (psActionTarget[i] && (avoidOverkill ? aiObjectIsProbablyDoomed(psActionTarget[i], bDirect) : psActionTarget[i]->deathTime))
    {
      ...
      if (i == 0)
      {
            ...
            action != DROID_ACTION::TRANSPORTOUT)
        {
          ...
          if (!isVtolDroid()) continue;
          if ((activeOrder.type == DROID_ORDER_TYPE::PATROL || activeOrder.type == DROID_ORDER_TYPE::CIRCLE) && (!vtolEmpty() || (secondaryOrder & DSS_ALEV_MASK) == DSS_ALEV_NEVER))
          {
            // Back to the patrol.
            actionDroid(DROID_ACTION::MOVE, activeOrder.pos.x, activeOrder.pos.y);
          }
          else
          {
            moveToRearm();
...
{
  if (order.type == DROID_ORDER_TYPE::NONE || order.type == DROID_ORDER_TYPE::PATROL || order.type == DROID_ORDER_TYPE::HOLD || order.type == DROID_ORDER_TYPE::SCOUT || order.type == DROID_ORDER_TYPE::GUARD)
  {
...
void Droid::setPosition(int x, int y)
{
  position.x = x;
  position.y = y;
  position.z = map_Height(position.x, position.y);
  initDroidMovement();
...
void Droid::actionUpdateVtolAttack()
{
  ...
  if (numWeapons > 0 && weaponList[0].nStat > 0 && vtolEmpty())
  {
    moveToRearm();
    return;
  }
  ...
  if (sMove.Status == MOVEHOVER && !isCyborg())
  {
...
{
  if (deathTime == NOT_CURRENT_LIST || isTransporter()
      ||
      position.x == INVALID_XY || position.y == INVALID_XY || missionIsOffworld()
      || mapHeight == 0)
  {
    // Off world or on a transport or is a transport or in mission list, or on a mission, or no map - ignore
    return true;
  }
  return worldOnMap(position.x, position.y);
}

bool Droid::isConstructionDroid()
{
  return droidType == CONSTRUCT || droidType == CYBORG_CONSTRUCT;
}

/*returns true if droid type is one of the Cyborg types*/
bool Droid::isCyborg()
{
  return (droidType == CYBORG
...
SWORD Droid::calculateResistance() const
{
...
{
  if (droidType != SENSOR) return false;
...
void Droid::updateVtolAttackRun(int weaponSlot)
{
  if (!isVtolDroid()) return;
  if (numWeapons <= 0) return;
  if (asWeaponStats[weaponList[weaponSlot].nStat].vtolAttackRuns > 0)
  {
    ++weaponList[weaponSlot].usedAmmo;
    if (weaponList[weaponSlot].usedAmmo == getNumAttackRuns(weaponSlot))
    {
      weaponList[weaponSlot].ammo = 0;
    }
    //quick check doesn't go over limit
    ASSERT(weaponList[weaponSlot].usedAmmo < UWORD_MAX, "too many attack runs");
  }
}
...
{
  ASSERT_OR_RETURN(false, isVtolDroid(), "not a VTOL droid");

  // VTOLs with less health than their original aren't happy
  if (hitPoints < originalBody) return false;

  // Not an armed droid, so don't check the (non-existent) weapons
  if (droidType != WEAPON) return true;
  ...
  {
    if (asWeaponStats[weaponList[i].nStat].vtolAttackRuns > 0
        &&
        weaponList[i].usedAmmo != 0)
    {
      return false;
    }
  }
...
UWORD Droid::getNumAttackRuns(int weapon_slot) const
{
  ASSERT_OR_RETURN(0, isVtolDroid(), "not a VTOL Droid");
  // if weapon is a salvo weapon, then number of shots that can be fired = vtolAttackRuns * numRounds
  if (asWeaponStats[weaponList[weapon_slot].nStat].upgrade[owningPlayer].reloadTime)
  {
    return asWeaponStats[weaponList[weapon_slot].nStat].upgrade[owningPlayer].numRounds
           * asWeaponStats[weaponList[weapon_slot].nStat].vtolAttackRuns;
  }
  return asWeaponStats[weaponList[weapon_slot].nStat].vtolAttackRuns;
}
...
{
  //what about cyborgs?
  if (!(droidType == WEAPON || droidType == CYBORG ||
        droidType == CYBORG_SUPER))
  {
    return false;
  }
...
{
  if (!isVtolDroid()) return false;
  if (droidType != WEAPON) return false;
...
void Droid::droidSetBits(const DroidStats *pTemplate)
{
  ...
  for (int inc = 0; inc < MAX_WEAPONS; inc++)
  {
    ...
    weaponList[inc].lastFired = 0;
    weaponList[inc].shotsFired = 0;
    // no weapon (could be a construction droid for example)
    // this is also used to check if a droid has a weapon, so zero it
    weaponList[inc].nStat = 0;
    weaponList[inc].ammo = 0;
    weaponList[inc].rot.direction = 0;
    weaponList[inc].rot.pitch = 0;
    weaponList[inc].rot.roll = 0;
    weaponList[inc].prevRot = weaponList[inc].rot;
    weaponList[inc].origin = ORIGIN_UNKNOWN;
    if (inc < pTemplate->numWeaps)
    {
      weaponList[inc].nStat = pTemplate->asWeaps[inc];
      weaponList[inc].ammo = (asWeaponStats + weaponList[inc].nStat)->upgrade[owningPlayer].numRounds;
    }
    weaponList[inc].usedAmmo = 0;
  }
...
bool Droid::idfDroid()
{
  ...
  return !proj_Direct(weaponList[0].nStat + asWeaponStats);
}
...
bool Droid::droidUpdateRepair()
{
  ...
  ASSERT_OR_RETURN(false, psStruct->getType() == OBJECT_TYPE::STRUCTURE, "target is not a structure");
...
bool Droid::droidUpdateRestore()
{
  ...
  ASSERT_OR_RETURN(false, psStruct->getType == OBJECT_TYPE::STRUCTURE, "Target is not a structure");
  ASSERT_OR_RETURN(false, weaponList[0].nStat > 0, "Droid doesn't have any weapons");

  unsigned compIndex = weaponList[0].nStat;
...
bool Droid::droidUpdateBuild()
{
  ASSERT_OR_RETURN(false, action == DROID_ACTION::BUILD, "%s (order %s) has wrong action for construction: %s",
                   droidGetName(psDroid), getDroidOrderName(activeOrder.type), getDroidActionName(action));

  auto psStruct = castStructure(activeOrder.psObj);
...
int32_t Droid::droidDamage(unsigned damage, WEAPON_CLASS weaponClass, WEAPON_SUBCLASS weaponSubClass, unsigned impactTime, bool isDamagePerSecond, int minDamage)
{
  ...
  if ((isVtolDroid() || (isTransporter() && bMultiPlayer)) && (sMove.Status == MOVEINACTIVE))
  {
    damage *= 3;
  }
...
void Droid::droidUpdate()
{
  ...
  if (owningPlayer == selectedPlayer && (droidType == SENSOR || droidType == COMMAND))
  {
    /* If we're attacking or sensing (observing), then... */
    if ((psBeingTargetted = orderStateObj(psDroid, DROID_ORDER_TYPE::ATTACK))
        || (psBeingTargetted = orderStateObj(psDroid, DROID_ORDER_TYPE::OBSERVE)))
    {
      psBeingTargetted->flags.set(TARGETED, true);
             ...
             (action == DROID_ACTION::ATTACK ||
              ...
              orderState(psDroid, DROID_ORDER_TYPE::HOLD)))
    {
  ...
  Droid *psOther;
  if (droidType == REPAIR || droidType == CYBORG_REPAIR)
  {
    for (psOther = allDroidLists[owningPlayer]; psOther; psOther = psOther->psNext)
    {
      // unlike repair facility, no droid  can have DROID_ORDER_TYPE::RTR_SPECIFIED with another droid as target, so skip that check
      if (psOther->order.type == DROID_ORDER_TYPE::RTR &&
          ...
          psOther->action != DROID_ACTION::WAITDURINGREPAIR)
      {
        if (psOther->hitPoints >= psOther->originalBody)
        {
          ...
          if (hasCommander(psOther))
          {
            ...
            orderDroidObj(psOther, DROID_ORDER_TYPE::GUARD, psCommander, ModeImmediate);
          }
          continue;
        }
      }
               ...
               && psOther->order.psObj == psDroid)
      {
        ...
        {
          actionDroid(psOther, DROID_ACTION::MOVE, psDroid, position.x,
                      position.y);
        }
  ...
  if (!isVtolDroid() && hitPoints < originalBody && asBits[COMP_REPAIRUNIT] != 0 && selfRepairEnabled(owningPlayer))
  {
  ...
  if ((id + gameTime) / 833 != (id + gameTime - deltaGameTime) / 833)
  {
    ...
    if (resistance && resistance < calculateResistance(psDroid))
    {
...
DroidStartBuild Droid::droidStartBuild()
{
  ...
  if (order.psObj == nullptr &&
      (order.type == DROID_ORDER_TYPE::BUILD ||
  ...
  if (psStruct)
  {
    if (psStruct->status != Structure::BUILT && aiCheckAlliances(psStruct->owningPlayer, owningPlayer))
    {
...
bool Droid::droidUpdateDemolishing()
{
  ...
  auto psStruct = (Structure*) activeOrder.psObj;
  ASSERT_OR_RETURN(false, psStruct->getType == OBJECT_TYPE::STRUCTURE, "target is not a structure");
...
{
  Vector2i tile = map_coord(activeOrder.pos);

  //check not another Truck started
  Structure *psStruct = getTileStructure(tile.x, tile.y);
  if (psStruct)
  {
    // if a droid is currently building, or building is in progress of being built/upgraded the droid's order should be DROID_ORDER_TYPE::HELPBUILD
    if (checkDroidsBuilding(psStruct) || !psStruct->status)
    {
      //set up the help build scenario
      activeOrder.type = DROID_ORDER_TYPE::HELPBUILD;
...
bool Droid::isElectronic() const
{
  CHECK_DROID(psDroid);

  //use slot 0 for now
  if (numWeapons > 0 && asWeaponStats[weaponList[0].nStat].weaponSubClass == WSC_ELECTRONIC)
  {
    return true;
  }
  ...
  if (droidType == DROID_COMMAND && psGroup && psGroup->psCommander == psDroid)
  {
    ...
    {
      if (psDroid != psCurr && isElectronic(psCurr))
      {
        return true;
      }
    }
...
bool Droid::isUnderRepair() const
{
  CHECK_DROID(psDroid);
...
bool Droid::vtolEmpty() const
{
  ...
  {
    if (asWeaponStats[weaponList[i].nStat].vtolAttackRuns > 0 &&
        weaponList[i].usedAmmo < getNumAttackRuns(i))
    {
      return false;
    }
  }
...
bool Droid::vtolFull() const
{
  ...
  if (droidType != DROID_TYPE::WEAPON) return false;
...
void Droid::aiUpdateDroid()
{
  ...
  if (orderState(DROID_ORDER_TYPE::NONE) ||
      orderState(DROID_ORDER_TYPE::GUARD) ||
      orderState(DROID_ORDER_TYPE::HOLD))
  {
    lookForTarget = true;
  }
  ...
  if (orderState(DROID_ORDER_TYPE::GUARD) && action != DROID_ACTION::NONE && droidType != DROID_SENSOR)
  {
    lookForTarget = false;
  }
  ...
  if ((orderState(DROID_ORDER_TYPE::OBSERVE) || orderState(DROID_ORDER_TYPE::ATTACKTARGET)) &&
  ...
  if (orderState(DROID_ORDER_TYPE::ATTACK) && psActionTarget[0] == order.psObj)
  {
    updateTarget = false;
  }
  ...
  if (bMultiPlayer && isVtolDroid() && isHumanPlayer(owningPlayer))
  {
    lookForTarget = false;
    updateTarget = false;
  }
  ...
  if (droidType == DROID_SENSOR && !hasStandardSensor(psDroid))
  {
    lookForTarget = false;
    updateTarget = false;
  }
  ...
  if (lookForTarget && !updateTarget)
  {
    ...
    if (droidType == DROID_SENSOR)
    {
      ...
      {
        if (!orderState(DROID_ORDER_TYPE::HOLD)
            && secondaryGetState(psDroid, DSO_HALTTYPE) == DSS_HALT_PURSUE)
        {
          order = DroidOrder(DROID_ORDER_TYPE::OBSERVE, psTarget);
        }
        actionDroid(DROID_ACTION::OBSERVE, psTarget);
      }
    }
    else
    {
      if (aiChooseTarget((GameObject *)psDroid, &psTarget, 0, true, nullptr))
      {
        if (!orderState(DROID_ORDER_TYPE::HOLD)
            && secondaryGetState(psDroid, DSO_HALTTYPE) == DSS_HALT_PURSUE)
        {
          order = DroidOrder(DROID_ORDER_TYPE::ATTACK, psTarget);
        }
        actionDroid(DROID_ACTION::ATTACK, psTarget);
      }
    }
  }
}
...
{
  DROID_ORDER order = DROID_ORDER_TYPE::NONE;
  PROPULSION_TYPE propulsion = getPropulsionStats(psDroid)->propulsionType;

  if (isTransporter() && game.type == LEVEL_TYPE::CAMPAIGN)
  {
    // transports can't be controlled in campaign
    return DROID_ORDER_TYPE::NONE;
  }
...
void Droid::orderDroidAdd(DROID_ORDER_DATA *psOrder)
{
  ...
  if (listSize <= 1 &&
      (order.type == DROID_ORDER_TYPE::NONE ||
       order.type == DROID_ORDER_TYPE::GUARD ||
       order.type == DROID_ORDER_TYPE::PATROL ||
       order.type == DROID_ORDER_TYPE::CIRCLE ||
       order.type == DROID_ORDER_TYPE::HOLD))
  {
    orderDroidList();
  }
}
...
bool Droid::orderStateStatsLoc(DROID_ORDER order, StructureStats **ppsStats)
{
  ...
  case DROID_ORDER_TYPE::BUILD:
  case DROID_ORDER_TYPE::LINEBUILD:
    if (order.type == DROID_ORDER_TYPE::BUILD ||
        order.type == DROID_ORDER_TYPE::LINEBUILD)
    {
      match = true;
    }
...
void Droid::orderUpdateDroid()
{
  ...
  case DROID_ORDER_TYPE::HOLD:
    ...
    else if (hasCommander(psDroid) && order.type != DROID_ORDER_TYPE::HOLD
             && order.psStats != structGetDemolishStat())  // stop the constructor auto repairing when it is about to demolish
    {
      orderDroidObj(psDroid, DROID_ORDER_TYPE::GUARD, psGroup->psCommander, ModeImmediate);
    }
    else if (isTransporter() && !bMultiPlayer)
    {
    ...
    // default to guarding
             ...
             && !isVtolDroid())
    {
      orderDroidLoc(DROID_ORDER_TYPE::GUARD, position.x, position.y,
                    ModeImmediate);
    }
    break;
    ...
    if (action == DROID_ACTION::NONE)
    {
      if (owningPlayer == selectedPlayer)
      {
        if (getDroidsToSafetyFlag())
        {
          ...
          sendOrder(DROID_ORDER_TYPE::TRANSPORTIN, ModeImmediate);
          /* set action transporter waits for timer */
          actionDroid(DROID_ACTION::TRANSPORTWAITTOFLYIN);
          ...
          //orderDroidLoc( psDroid, DROID_ORDER_TYPE::TRANSPORTIN,
  ...
  case DROID_ORDER_TYPE::RECOVER:
    ...
    else if (action == DROID_ACTION::NONE)
    {
      ...
      actionDroid(DROID_ACTION::MOVE, order.psObj->position.x,
                  order.psObj->position.y);
    }
    break;
  case DROID_ORDER_TYPE::SCOUT:
  case DROID_ORDER_TYPE::PATROL:
    ...
    if (action == DROID_ACTION::MOVE || action == DROID_ACTION::MOVEFIRE || (action == DROID_ACTION::NONE && isVtolDroid(psDroid)))
    {
      ...
      if (isVtolDroid() && order.type == DROID_ORDER_TYPE::PATROL)
      {
        ...
        if (delta == Vector2i(0, 0))
        {
          tooFarFromPath = false;
                   ...
                   MIN(order.pos.x, order.pos2.x) - SCOUT_DIST <=
                       position.x &&
                   position.x <=
                       MAX(order.pos.x, order.pos2.x) + SCOUT_DIST)
        {
          tooFarFromPath = (abs((position.x - order.pos.x) * delta.y / delta.x +
                                order.pos.y - position.y) > SCOUT_DIST);
        }
        else if (abs(delta.x) <= abs(delta.y) &&
                   MIN(order.pos.y, order.pos2.y) - SCOUT_DIST <=
                       position.y &&
                   position.y <=
                       MAX(order.pos.y, order.pos2.y) + SCOUT_DIST)
        {
      ...
      if (!tooFarFromPath)
      {
        ...
        case COMMAND:
          if (attack)
          {
            actionDroid(DROID_ACTION::ATTACK, psObj);
          }
          break;
        case SENSOR:
          if (attack)
          {
            actionDroid(DROID_ACTION::OBSERVE, psObj);
          }
          break;
        default:
    ...
    if (action == DROID_ACTION::NONE)
    {
      ...
      {
        if (order.type == DROID_ORDER_TYPE::PATROL)
        {
          ...
          if (isVtolDroid() && !vtolFull() && (secondaryOrder & DSS_ALEV_MASK) != DSS_ALEV_NEVER)
          {
            moveToRearm();
            break;
          }
          ...
          actionDroid(DROID_ACTION::MOVE, order.pos.x, order.pos.y);
        }
        else
        {
          order = DroidOrder(DROID_ORDER_TYPE::NONE);
    ...
    if (order.type == DROID_ORDER_TYPE::PATROL && isVtolDroid(psDroid) && vtolEmpty(psDroid) && (secondaryOrder & DSS_ALEV_MASK) != DSS_ALEV_NEVER)
    {
  ...
  case DROID_ORDER_TYPE::CIRCLE:
    ...
    else if (action == DROID_ACTION::NONE || action == DROID_ACTION::MOVE)
    {
      ...
      if (action != DROID_ACTION::MOVE || dot(edgeDiff, edgeDiff) <= TILE_UNITS * 4 * TILE_UNITS * 4)
      {
        ...
        actionDroid(DROID_ACTION::MOVE, order.pos.x + xoffset, order.pos.y + yoffset);
      }
             ...
             secondaryGetState(psDroid, DSO_HALTTYPE) != DSS_HALT_PURSUE)
    {
      ...
      if (xdiff * xdiff + ydiff * ydiff > 2000 * 2000)
      {
        ...
        actionDroid(DROID_ACTION::RETURNTOPOS, order.pos.x, order.pos.y);
      }
    }
    break;
  case DROID_ORDER_TYPE::HELPBUILD:
  ...
  case DROID_ORDER_TYPE::ATTACKTARGET:
             ...
             actionVisibleTarget(psDroid, order.psObj, 0) && !isVtolDroid(psDroid))
    {
      ...
      actionDroid(DROID_ACTION::ATTACK, order.psObj);
    }
             ...
             && !aiCheckAlliances(psWall->owningPlayer, owningPlayer))
    {
      ...
      actionDroid(DROID_ACTION::ATTACK, psWall);
    }
    else if ((action == DROID_ACTION::NONE) ||
             (action == DROID_ACTION::CLEARREARMPAD))
    {
      if ((order.type == DROID_ORDER_TYPE::ATTACKTARGET || order.type == DROID_ORDER_TYPE::ATTACK)
          && secondaryGetState(psDroid, DSO_HALTTYPE) == DSS_HALT_HOLD
          && !actionInRange(psDroid, order.psObj, 0))
      {
      ...
      else if (!isVtolDroid() || allVtolsRearmed())
      {
        actionDroid(DROID_ACTION::ATTACK, order.psObj);
      }
    }
  ...
  case DROID_ORDER_TYPE::EMBARK:
  {
    ...
    auto *temp = (Droid *)order.psObj;	// NOTE: It is possible to have a NULL here

    if (temp && temp->droidType == DROID_TRANSPORTER && !isCyborg())
    {
      order = DroidOrder(DROID_ORDER_TYPE::NONE);
      actionDroid(DROID_ACTION::NONE);
      if (owningPlayer == selectedPlayer)
    ...
    else
    {
      ...
      else if (abs((SDWORD)position.x -
                     (SDWORD)order.psObj->position.x) < TILE_UNITS
               && abs((SDWORD)position.y -
                     (SDWORD)order.psObj->position.y) < TILE_UNITS)
      {
        ...
        setDroidTarget(psDroid, nullptr);
        order.psObj = nullptr;
        secondarySetState(DSO_RETURN_TO_LOC, DSS_NONE);
  ...
  case DROID_ORDER_TYPE::RTR_SPECIFIED:
    if (order.psObj == nullptr)
    {
      ...
      order = DroidOrder(DROID_ORDER_TYPE::NONE);
      sendOrder(DROID_ORDER_TYPE::RTR, ModeImmediate);
    }
    else if (action == DROID_ACTION::NONE)
    {
      ...
      if (objPosDiffSq(position, order.psObj->position) < (TILE_UNITS * 8) * (TILE_UNITS * 8))
      {
        ...
        actionDroid(DROID_ACTION::WAITFORREPAIR);
      }
      else
      {
  ...
  case DROID_ORDER_TYPE::LINEBUILD:
        ...
        (action == DROID_ACTION::BUILD && order.psObj == nullptr))
    {
      ...
      setDroidTarget(psDroid, nullptr);
      actionDroid(DROID_ACTION::BUILD, order.pos.x, order.pos.y);
      //intRefreshScreen();
    }
    break;
  case DROID_ORDER_TYPE::FIRESUPPORT:
    ...
    else
    {
      ...
      if (order.psObj->getType == OBJ_DROID)
      {
        auto *psSpotter = (Droid *)order.psObj;
      ...
      else if (order.psObj->getType == OBJ_STRUCTURE)
      {
        auto *psSpotter = (Structure *)order.psObj;
      ...
      if (psFireTarget && !psFireTarget->deathTime && checkAnyWeaponsTarget(psDroid, psFireTarget))
      {
             ...
             psFireTarget != psActionTarget[0]))
        {
          ...
          actionDroid(DROID_ACTION::ATTACK, psFireTarget);
        }
      }
      else if (isVtolDroid() &&
               !vtolFull() &&
      ...
      {
        actionDroid(DROID_ACTION::FIRESUPPORT, order.psObj);
      }
    }
    break;
  case DROID_ORDER_TYPE::RECYCLE:
    ...
    else if (actionReachedBuildPos(
                   psDroid, order.psObj->position.x,
                   order.psObj->position.y,
                   ((Structure *)order.psObj)->rotation.direction,
  ...
  case DROID_ORDER_TYPE::GUARD:
             ...
             (action == DROID_ACTION::MOVEFIRE))
    {
      ...
      if ((!(droidType == DROID_REPAIR || droidType == DROID_CYBORG_REPAIR))
          && order.psObj != nullptr && order.psObj->getType == OBJ_DROID
    ...
    else
    {
      ...
      if (!vtolRearming())
      {
        ...
        if (order.psObj != nullptr && order.psObj->getType == OBJ_DROID &&
            ((Droid *)order.psObj)->droidType == DROID_COMMAND)
        {
          // guarding a commander, allow more space
    ...
    if (hasCommander(psDroid) && (numWeapons > 0))
    {
      ...
      psObj = orderStateObj(psDroid, DROID_ORDER_TYPE::GUARD);	// find out who is being guarded by the droid
      if (psObj == nullptr
  ...
  if (order.type == DROID_ORDER_TYPE::NONE && vtolRearming()
      && (psActionTarget[0] == nullptr || !psActionTarget[0]->deathTime))
  {
    order = DroidOrder(DROID_ORDER_TYPE::REARM, psActionTarget[0]);
  }
...
Droid* Droid::checkForRepairRange()
{
  ...
  ASSERT(droidType == REPAIR || droidType == CYBORG_REPAIR, "Invalid droid getType");
  ...
  for (GameObject *object :
       gridStartIterate(position.x, position.y, radius))
  {
...
bool Droid::tryDoRepairlikeAction()
{
  ...
  case DROID_CYBORG_REPAIR:
    ...
    if (Droid *repairTarget = checkForRepairRange())
    {
      actionDroid(DROID_ACTION::DROIDREPAIR, repairTarget);
    }
    break;
  ...
  case DROID_CYBORG_CONSTRUCT:
  {
    ...
    {
      actionDroid(damaged.second, damaged.first);
    }
    else if (damaged.second == DROID_ACTION::BUILD)
    {
      ...
      actionDroid(damaged.second, damaged.first->position.x,
                  damaged.first->position.y);
    }
    break;
  }
  default:
...
void Droid::orderDroidBase(DROID_ORDER_DATA *psOrder)
{
  ...
  if (!isTransporter() && psOrder->type != DROID_ORDER_TYPE::NONE && psOrder->type != DROID_ORDER_TYPE::STOP && psOrder->type != DROID_ORDER_TYPE::GUARD)
  {
  ...
  case DROID_ORDER_TYPE::SCOUT:
    ...
    order = *psOrder;
    actionDroid(DROID_ACTION::MOVE, psOrder->pos.x, psOrder->pos.y);
    break;
  case DROID_ORDER_TYPE::PATROL:
    order = *psOrder;
  ...
  case DROID_ORDER_TYPE::RECOVER:
    order = *psOrder;
    actionDroid(DROID_ACTION::MOVE, psOrder->psObj->position.x,
                psOrder->psObj->position.y);
    break;
  case DROID_ORDER_TYPE::TRANSPORTOUT:
    // tell a (transporter) droid to leave home base for the offworld mission
    order = *psOrder;
    actionDroid(DROID_ACTION::TRANSPORTOUT, psOrder->pos.x, psOrder->pos.y);
    break;
  case DROID_ORDER_TYPE::TRANSPORTRETURN:
  ...
  case DROID_ORDER_TYPE::TRANSPORTIN:
    ...
    order = *psOrder;
    actionDroid(DROID_ACTION::TRANSPORTIN, psOrder->pos.x, psOrder->pos.y);
    break;
  case DROID_ORDER_TYPE::ATTACK:
  case DROID_ORDER_TYPE::ATTACKTARGET:
    if (numWeapons == 0
        || weaponList[0].nStat == 0
        || isTransporter())
    {
      break;
    }
    else if (order.type == DROID_ORDER_TYPE::GUARD && psOrder->type == DROID_ORDER_TYPE::ATTACKTARGET)
    {
      ...
      actionDroid(DROID_ACTION::ATTACK, psOrder->psObj);
    }
    else if (psOrder->psObj && !psOrder->psObj->deathTime)
    {
      //cannot attack a Transporter with EW in multiPlayer
      // FIXME: Why not ?
      if (game.type == LEVEL_TYPE::SKIRMISH && isElectronic()
          &&
          psOrder->psObj->getType() == OBJECT_TYPE::DROID && isTransporter((Droid *)psOrder->psObj))
      {
        break;
      }
      order = *psOrder;
  ...
  case DROID_ORDER_TYPE::LINEBUILD:
    ...
    ASSERT_OR_RETURN(, isConstructionDroid(), "%s cannot construct things!", objInfo(psDroid));
    ...
    actionDroid(DROID_ACTION::BUILD, psOrder->pos.x, psOrder->pos.y);
  ...
  case DROID_ORDER_TYPE::BUILDMODULE:
    ...
    if (!isConstructionDroid() || psOrder->index < nextModuleToBuild((Structure *)psOrder->psObj, -1))
    {
      break;
    }
    order = DroidOrder(DROID_ORDER_TYPE::BUILD, getModuleStat((Structure *)psOrder->psObj),
                       psOrder->psObj->position.xy(), 0);
    ASSERT_OR_RETURN(, order.psStats != nullptr, "should have found a module stats");
    ASSERT_OR_RETURN(, !order.psStats || order.psStats->type != REF_DEMOLISH, "Cannot build demolition");
    actionDroid(DROID_ACTION::BUILD, psOrder->psObj->position.x,
                psOrder->psObj->position.y);
  ...
  case DROID_ORDER_TYPE::HELPBUILD:
    ...
    ASSERT_OR_RETURN(, isConstructionDroid(), "Not a constructor droid");
  ...
  case DROID_ORDER_TYPE::DEMOLISH:
    ...
    order = *psOrder;
    order.pos = psOrder->psObj->position.xy();
    actionDroid(DROID_ACTION::DEMOLISH, psOrder->psObj);
    break;
  case DROID_ORDER_TYPE::REPAIR:
    if (!(droidType == DROID_CONSTRUCT || droidType == DROID_CYBORG_CONSTRUCT))
    {
    ...
    order = *psOrder;
    order.pos = psOrder->psObj->position.xy();
    actionDroid(DROID_ACTION::REPAIR, psOrder->psObj);
    break;
  case DROID_ORDER_TYPE::DROIDREPAIR:
    if (!(droidType == DROID_REPAIR || droidType == DROID_CYBORG_REPAIR))
    {
    ...
    order = *psOrder;
    actionDroid(DROID_ACTION::DROIDREPAIR, psOrder->psObj);
    break;
  case DROID_ORDER_TYPE::OBSERVE:
    // keep an object within sensor view
    order = *psOrder;
    actionDroid(DROID_ACTION::OBSERVE, psOrder->psObj);
    break;
  case DROID_ORDER_TYPE::FIRESUPPORT:
    if (isTransporter())
    {
    ...
    if (!isVtolDroid())
    {
      actionDroid(DROID_ACTION::FIRESUPPORT, psOrder->psObj);
    }
  ...
  case DROID_ORDER_TYPE::RTB:
    for (psStruct = apsStructLists[owningPlayer]; psStruct; psStruct = psStruct->psNext)
    {
      if (psStruct->stats->type == REF_HQ)
      {
        ...
        if (isVtolDroid() || (game.type == LEVEL_TYPE::SKIRMISH && isTransporter()))
        {
          actionVTOLLandingPos(&pos);
        }
        actionDroid(DROID_ACTION::MOVE, pos.x, pos.y);
        break;
      }
    }
    ...
    if (order.type != DROID_ORDER_TYPE::RTB)
    {
      ...
      {
        order = *psOrder;
        actionDroid(DROID_ACTION::MOVE, iDX, iDY);
      }
      else
      {
  ...
  case DROID_ORDER_TYPE::RTR_SPECIFIED:
  {
    ...
    if (rtrData.getType == RTR_TYPE_REPAIR_FACILITY)
    {
      ...
      else
      {
        ...
        actionDroid(DROID_ACTION::MOVE, rtrData.psObj, order.pos.x, order.pos.y);
      }
    }
    /* give repair order if repair droid found */
    else if (rtrData.getType == RTR_TYPE_DROID && !isTransporter(psDroid))
    {
      order = DroidOrder(psOrder->type,
          Vector2i(rtrData.psObj->position.x, rtrData.psObj->position.y), RTR_TYPE_DROID);
      order.pos = rtrData.psObj->position.xy();
    ...
    else
    {
      ...
      if (!bMultiPlayer && selectedPlayer == 0)
      {
        ...
        sendOrder(psDroid, DROID_ORDER_TYPE::RTB, ModeImmediate);
      }
    }
  }
  break;
  ...
  case DROID_ORDER_TYPE::RECYCLE:
    ...
    if (psFactory != nullptr)
    {
      ...
      actionDroid(DROID_ACTION::MOVE, psFactory, order.pos.x, order.pos.y);
    }
    break;
  case DROID_ORDER_TYPE::GUARD:
    order = *psOrder;
  ...
  case DROID_ORDER_TYPE::RESTORE:
    ...
    if (psOrder->psObj->getType != OBJ_STRUCTURE)
    {
      ASSERT(false, "orderDroidBase: invalid object getType for Restore order");
      break;
    }
    order = *psOrder;
    order.pos = psOrder->psObj->position.xy();
    actionDroid(DROID_ACTION::RESTORE, psOrder->psObj);
    break;
  case DROID_ORDER_TYPE::REARM:
    ...
    {
      order = *psOrder;
      actionDroid(DROID_ACTION::MOVETOREARM, psOrder->psObj);
...
void Droid::sendOrder(DROID_ORDER order, QUEUE_MODE mode)
{
...
bool Droid::secondarySetState(SECONDARY_ORDER sec, SECONDARY_STATE State, QUEUE_MODE mode)
{
      ...
      if (psGroup != nullptr)
      {
        if (droidType == COMMAND)
        {
          ...
          for (psCurr = psGroup->psList; psCurr; psCurr = psNext)
          {
            ...
            sendOrder(psCurr, DROID_ORDER_TYPE::STOP, ModeImmediate);
          }
        }
  ...
  case DSO_HALTTYPE:
    ...
    case DSS_HALT_GUARD:
      ...
      orderDroidLoc(psDroid, DROID_ORDER_TYPE::GUARD, position.x,
...
{
  if (getType != OBJ_DROID) {
    ASSERT(false, "%c Broken getType %u!", ch, getType);
    syncDebug("Broken getType %u!", getType);
  }
  int list[] =
      {
          ch,
...
bool Droid::droidUpdateDroidRepair()
{
  ...
  ASSERT_OR_RETURN(false, psDroidToRepair->getType == OBJECT_TYPE::DROID, "Target is not a unit");
  bool needMoreRepair = droidUpdateDroidRepairBase(psRepairDroid, psDroidToRepair);
  if (needMoreRepair && psDroidToRepair->order.type == DROID_ORDER_TYPE::RTR && psDroidToRepair->order.rtrType == RTR_TYPE_DROID && psDroidToRepair->action == DACTION_NONE)
  {
    psDroidToRepair->action = DROID_ACTION::WAITDURINGREPAIR;
  }
  if (!needMoreRepair && psDroidToRepair->order.type == DROID_ORDER_TYPE::RTR && psDroidToRepair->order.rtrType == RTR_TYPE_DROID)
  {
    // if psDroidToRepair has a commander, commander will call him back anyway
    // if no commanders, just DROID_ORDER_TYPE::GUARD the repair turret
    orderDroidObj(psDroidToRepair, DROID_ORDER_TYPE::GUARD, psRepairDroid, ModeImmediate);
...
void Droid::actionUpdateDroid()
{
  ...
  case DROID_ACTION::WAITFORREPAIR:
    ...
    if (numWeapons > 0 && !isVtolDroid(psDroid)
        && (order->type == DROID_ORDER_TYPE::NONE || order->type == DROID_ORDER_TYPE::HOLD || order->type == DROID_ORDER_TYPE::RTR || order->type == DROID_ORDER_TYPE::GUARD))
    {
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        if (nonNullWeapon[i])
        {
          ...
          WEAPON_STATS *const psWeapStats = &asWeaponStats[weaponList[i].nStat];
          if (weaponList[i].nStat > 0
              && psWeapStats->rotate
  ...
  case DROID_ACTION::WAITDURINGREPAIR:
    ...
    if (order->type == DROID_ORDER_TYPE::RTR && order->rtrType == RTR_TYPE_REPAIR_FACILITY)
    {
      // move back to the repair facility if necessary
      if (isStationary() &&
          !actionReachedBuildPos(
              psDroid, order->psObj->position.x, order->psObj->position.y,
              ((Structure *)order->psObj)->rotation.direction,
              ((Structure *)order->psObj)->stats))
      {
        moveDroidToNoFormation(psDroid, order->psObj->position.x,
                               order->psObj->position.y);
      }
    }
    else if (order->type == DROID_ORDER_TYPE::RTR && order->rtrType == RTR_TYPE_DROID && isStationary(psDroid)) {
  ...
  case DROID_ACTION::MOVEFIRE:
    ...
    for (unsigned i = 0; i < numWeapons; ++i)
    {
      ...
      if (psActionTarget[i] != nullptr)
      {
        ...
        else if (isElectronic(psDroid) && owningPlayer == psActionTarget[i]->owningPlayer)
        {
          setDroidActionTarget(psDroid, nullptr, i);
        }
    ...
    if (bHasTarget)
    {
      ...
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        const unsigned compIndex = weaponList[i].nStat;
        const WEAPON_STATS *psStats = asWeaponStats + compIndex;
        wallBlocked = false;
        ...
        if (psActionTarget[i] != nullptr && validTarget(psDroid, psActionTarget[i], i))
        {
          ...
          if (targetVisibile[i] && nonNullWeapon[i]) //to fix a AA-weapon attack ground unit exploit
          {
            ...
            if (!wallBlocked && actionTargetTurret(psDroid, psActionTarget, &weaponList[i]))
            {
              // In range - fire !!!
              combFire(&weaponList[i], psDroid, psActionTarget, i);
            }
          }
        }
      }
  ...
  case DROID_ACTION::ROTATETOATTACK:
    ...
    if (isElectronic(psDroid) && owningPlayer == psActionTarget[0]->owningPlayer)
    {
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        setDroidActionTarget(psDroid, nullptr, i);
    ...
    for (unsigned i = 0; i < numWeapons; ++i)
    {
      ...
      if (i > 0)
      {
        ...
        if ((order->type == DROID_ORDER_TYPE::ATTACK || order->type == DROID_ORDER_TYPE::ATTACKTARGET) &&
          ...
          && actionInRange(psDroid, psActionTarget, i))
      {
        ...
        if (validTarget(psDroid, psActionTarget, i) && !wallBlocked)
        {
          ...
          if (!psWeapStats->rotate)
          {
            ...
            const uint16_t targetDir = calcDirection(
                position.x, position.y, psActionTarget->position.x,
                psActionTarget->position.y);
          ...
          if (dirDiff > FIXED_TURRET_DIR)
          {
            ...
            {
              action = DROID_ACTION::ROTATETOATTACK;
              moveTurnDroid(psDroid, psActionTarget->position.x,
                            psActionTarget->position.y);
            }
          }
          else if (!psWeapStats->rotate ||
                   actionTargetTurret(psDroid, psActionTarget, &weaponList[i]))
          {
            /* In range - fire !!! */
            combFire(&weaponList[i], psDroid, psActionTarget, i);
          }
    ...
    {
      GameObject *psTarget;
      bool supportsSensorTower = !isVtolDroid() && (psTarget = orderStateObj(psDroid, DROID_ORDER_TYPE::FIRESUPPORT)) &&
          psTarget->getType == OBJ_STRUCTURE;

      if (secHoldActive && (order->type == DROID_ORDER_TYPE::ATTACKTARGET || order->type == DROID_ORDER_TYPE::FIRESUPPORT))
      {
        action = DROID_ACTION::NONE; // secondary holding, cancel the order.
               ...
               !supportsSensorTower &&
               !(order->type == DROID_ORDER_TYPE::HOLD ||
                 order->type == DROID_ORDER_TYPE::RTR))
      {
      ...
      else if (secHoldActive && order->type == DROID_ORDER_TYPE::GUARD && hasCommander(psDroid))
      {
        Droid *commander = psGroup->psCommander;

        if (commander->order.type == DROID_ORDER_TYPE::ATTACKTARGET ||
            commander->order.type == DROID_ORDER_TYPE::FIRESUPPORT ||
            commander->order.type == DROID_ORDER_TYPE::ATTACK)
        {
          action = DROID_ACTION::MOVETOATTACK;
        }
        else
  ...
  case DROID_ACTION::VTOLATTACK:
  {
        ...
        targetIsValid)
    {
      ...
      if ((vtolEmpty()) ||
          (psActionTarget[0] == nullptr) ||
          //check the target hasn't become one the same player ID - Electronic Warfare
          (isElectronic() && (owningPlayer == psActionTarget[0]->owningPlayer)) ||
      ...
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        ...
        {
          //I moved psWeapStats flag update there
          psWeapStats = &asWeaponStats[weaponList[i].nStat];
          if (actionVisibleTarget(psDroid, psActionTarget[0], i))
          {
            if (actionInRange(psDroid, psActionTarget[0], i))
            {
              ...
              if (actionTargetTurret(psDroid, psActionTarget[0], &weaponList[i]))
              {
                // In range - fire !!!
                combFire(&weaponList[i], psDroid,
                         psActionTarget[0], i);
              }
            }
            else
            {
              actionTargetTurret(psDroid, psActionTarget[0], &weaponList[i]);
            }
          }
        }
      }
    ...
    if (isStationary(psDroid) || dot(attackRunDelta, attackRunDelta) < TILE_UNITS * TILE_UNITS)
    {
      actionAddVtolAttackRun();
    }
             ...
             targetIsValid)
    {
      ...
      else if (psWeapStats)
      {
        ...
        if (rangeSq > maxRange * maxRange)
        {
          ...
          if (dot(diff, diff) > VTOL_ATTACK_TARDIST * VTOL_ATTACK_TARDIST)
          {
            moveDroidToDirect(psDroid, psActionTarget[0]->position.x,
                              psActionTarget[0]->position.y);
          }
        }
      }
  ...
  case DROID_ACTION::MOVETOATTACK:
    ...
    if ((isElectronic() && owningPlayer == psActionTarget[0]->owningPlayer) || !hasValidWeapon)
    {
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        setDroidActionTarget(psDroid, nullptr, i);
    ...
    {
      if (actionVisibleTarget(psActionTarget[0], 0))
      {
        for (unsigned i = 0; i < numWeapons; ++i)
        {
          if (nonNullWeapon[i]
              && validTarget(psDroid, psActionTarget[0], i)
              && actionVisibleTarget(psActionTarget[0], i))
          {
            ...
            if (!isVtolDroid(psDroid) &&
                psActionTarget[0]->getType == OBJ_DROID &&
            ...
            if (actionInRange(psActionTarget[0], i) && !chaseBloke)
            {
              ...
              {
                if (actionInRange(psActionTarget[0], i, false))
                {
                  moveStopDroid(psDroid);
                }
            ...
            else if (actionInRange(psActionTarget[0], i))
            {
              ...
              if ((blockingWall = visGetBlockingWall(psDroid, psActionTarget[0])) && proj_Direct(psWeapStats))
              {
                                                                ...
                                                                ->strength] >= MIN_STRUCTURE_BLOCK_STRENGTH)
                {
                  ...
                  combFire(&weaponList[i], psDroid, (GameObject
                                         *)blockingWall, i);
                }
              }
              else
              {
                combFire(&weaponList[i], psDroid, psActionTarget[0], i);
              }
            }
          }
        }
      ...
      else
      {
        for (unsigned i = 0; i < numWeapons; ++i)
        {
          if ((weaponList[i].rot.direction != 0) ||
      ...
      if (isStationary(psDroid) && action != DROID_ACTION::ATTACK)
      {
        ...
        WEAPON_STATS *const psWeapStats = &asWeaponStats[weaponList[0].nStat];

        if (order->type == DROID_ORDER_TYPE::ATTACKTARGET && secHoldActive)
        {
          action = DROID_ACTION::NONE; // on hold, give up.
        }
        else if (actionInsideMinRange(psActionTarget[0], psWeapStats))
        {
          if (proj_Direct(psWeapStats) && order->type != DROID_ORDER_TYPE::HOLD)
          {
            SDWORD pbx, pby;
        ...
        else if (order->type != DROID_ORDER_TYPE::HOLD) // approach closer?
        {
          // try to close the range
          moveDroidTo(psDroid, psActionTarget[0]->position.x,
                      psActionTarget[0]->position.y);
        }
      }
    }
  ...
  case DROID_ACTION::MOVETOBUILD:
    ...
    else
    {
      ...
      if (nullptr != structureAtBuildPosition)
      {
        ...
        if (droidCannotBuild)
        {
          if (order->type == DROID_ORDER_TYPE::LINEBUILD && map_coord(order.pos) != map_coord(order.pos2))
          {
    ...
    if (actionReachedBuildPos(psDroid, actionPos.x, actionPos.y, order->direction, order->psStats))
    {
      ...
      if (order->type == DROID_ORDER_TYPE::BUILD && order->psObj == nullptr)
      {
      ...
      else if (order->type == DROID_ORDER_TYPE::LINEBUILD || order->type == DROID_ORDER_TYPE::BUILD)
      {
        // building a wall.
...
# pragma GCC diagnostic pop
  ...
  case DROID_ACTION::BUILD:
    ...
    {
      actionTargetTurret(psDroid, psActionTarget[0], &weaponList[0]);
    }
    break;
  ...
  case DROID_ACTION::MOVETORESTORE:
    ...
    else
    {
      ...
      else if (order->type != DROID_ORDER_TYPE::RESTORE)
      {
        ...
        else if (order->type != DROID_ORDER_TYPE::DEMOLISH && structureAtPos->hitPoints == structureBody(structureAtPos))
        {
          cantDoRepairLikeAction = true;
        }
        else if (order->type == DROID_ORDER_TYPE::DEMOLISH && structureAtPos->owningPlayer != owningPlayer)
        {
          cantDoRepairLikeAction = true;
        }
  ...
  case DROID_ACTION::RESTORE:
    ...
    if (isStationary(psDroid) && !actionReachedBuildPos(psDroid, actionPos.x, actionPos.y, ((Structure *)psActionTarget[0])->rotation.direction, order->psStats))
    {
      if (order->type != DROID_ORDER_TYPE::HOLD && (!secHoldActive || (secHoldActive && order->type != DROID_ORDER_TYPE::NONE)))
      {
  ...
  case DROID_ACTION::MOVETOREPAIRPOINT:
    if (order.rtrType == RTR_TYPE_REPAIR_FACILITY)
    {
      ...
      if (actionReachedBuildPos(
              psDroid, psActionTarget[0]->position.x,
              psActionTarget[0]->position.y,
              ((Structure *)psActionTarget[0])->rotation.direction,
              ((Structure *)psActionTarget[0])->stats))
      {
    ...
    } else if (order.rtrType == RTR_TYPE_DROID)
    {
      ...
      if (reached)
      {
        if (hitPoints >= originalBody)
        {
          ...
          orderDroidObj(psDroid, DROID_ORDER_TYPE::GUARD, order.psObj, ModeImmediate);
        }
        else
        {
  ...
  case DROID_ACTION::OBSERVE:
    ...
    if (!hasCBSensor())
    {
      ...
      const int xdiff =
          (SDWORD)position.x - (SDWORD)psActionTarget[0]->position.x;
      const int ydiff =
          (SDWORD)position.y - (SDWORD)psActionTarget[0]->position.y;
      int rangeSq = droidSensorRange(psDroid);
      rangeSq = rangeSq * rangeSq;
      if (!visibleObject(psDroid, psActionTarget[0], false)
          || xdiff * xdiff + ydiff * ydiff >= rangeSq)
      {
        if (secondaryGetState(psDroid, DSO_HALTTYPE) != DSS_HALT_GUARD && (order->type == DROID_ORDER_TYPE::NONE || order->type == DROID_ORDER_TYPE::HOLD))
        {
          action = DROID_ACTION::NONE;
        }
        else if ((!secHoldActive && order->type != DROID_ORDER_TYPE::HOLD) || (secHoldActive && order->type == DROID_ORDER_TYPE::OBSERVE))
        {
          action = DROID_ACTION::MOVETOOBSERVE;
          moveDroidTo(psDroid, psActionTarget[0]->position.x,
                      psActionTarget[0]->position.y);
  ...
  case DROID_ACTION::MOVETOOBSERVE:
    ...
    if (visibleObject(psDroid, psActionTarget[0], false))
    {
      ...
      if ((xdiff * xdiff + ydiff * ydiff < rangeSq) &&
          !isStationary(psDroid))
      {
        action = DROID_ACTION::OBSERVE;
        moveStopDroid(psDroid);
      }
    }
    if (isStationary(psDroid) && action == DROID_ACTION::MOVETOOBSERVE)
    {
      moveDroidTo(psDroid, psActionTarget[0]->position.x,
                  psActionTarget[0]->position.y);
    }
  ...
  case DROID_ACTION::FIRESUPPORT:
    ...
    ASSERT_OR_RETURN(, (order->psObj->getType == OBJ_DROID ||
         order->psObj->getType == OBJ_STRUCTURE)
                           && aiCheckAlliances(order->psObj->owningPlayer, owningPlayer), "DROID_ACTION::FIRESUPPORT: incorrect target getType");

    //don't move VTOL's
    // also don't move closer to sensor towers
    if (!isVtolDroid(psDroid) && order->psObj->getType != OBJ_STRUCTURE)
    {
      Vector2i diff = (position - order->psObj->position).xy();
      //Consider .shortRange here
      int rangeSq = asWeaponStats[weaponList[0].nStat].upgrade[owningPlayer].maxRange / 2; // move close to sensor
      rangeSq = rangeSq * rangeSq;
      ...
      else
      {
        ...
        if (isStationary(psDroid) || dot(diff, diff) > rangeSq)
        {
          if (secHoldActive)
          {
          ...
          {
            // move in range
            moveDroidTo(psDroid, order->psObj->position.x,
                        order->psObj->position.y);
          }
        }
      }
  ...
  case DROID_ACTION::MOVETODROIDREPAIR:
  {
    ...
    assert(actionTarget->getType() == OBJ_DROID)
    ...
    if (!psActionTarget[0] ||  // Target missing.
        (order.type != DROID_ORDER_TYPE::DROIDREPAIR && dot(diff, diff) > 2 * REPAIR_MAXDIST * REPAIR_MAXDIST))  // Target farther then 1.4142 * REPAIR_MAXDIST and we aren't ordered to follow.
    {
      action = DROID_ACTION::NONE;
      return;
    }
  ...
  case DROID_ACTION::DROIDREPAIR:
  {
    ...
    // See if there's anything to shoot.
             ...
             && (order->type == DROID_ORDER_TYPE::NONE || order->type == DROID_ORDER_TYPE::HOLD || order->type == DROID_ORDER_TYPE::RTR))
    {
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        if (nonNullWeapon[i])
        {
          ...
          WEAPON_STATS *const psWeapStats = &asWeaponStats[weaponList[i].nStat];
          if (weaponList[i].nStat > 0 && psWeapStats->rotate
              && secondaryGetState(psDroid, DSO_ATTACK_LEVEL) == DSS_ALEV_ALWAYS
    ...
    {
      if (order->type == DROID_ORDER_TYPE::DROIDREPAIR)
      {
    ...
    else
    {
      if (!droidUpdateDroidRepair(psDroid))
      {
        ...
        {
          sendOrder(psDroid, DROID_ORDER_TYPE::RTR, ModeImmediate);
        }
      }
      else
      {
  ...
  case DROID_ACTION::WAITFORREARM:
    ...
    if (isStationary(psDroid) && vtolHappy(psDroid))
    {
  ...
  case DROID_ACTION::MOVETOREARM:
    ...
    if (isStationary(psDroid) || action == DROID_ACTION::WAITFORREARM)
    {
      ...
      action != DROID_ACTION::MOVETOOBSERVE)
  {
    ...
    if (numWeapons == 0)
    {
      if (weaponList[0].rot.direction != 0 || weaponList[0].rot.pitch != 0)
      {
    ...
    else
    {
      for (unsigned i = 0; i < numWeapons; ++i)
      {
        if (weaponList[i].rot.direction != 0 || weaponList[i].rot.pitch != 0)
        {
          actionAlignTurret(psDroid, i);
        }
      }
...
void Droid::actionDroidBase(DROID_ACTION_DATA *psAction)
{
  ...
  case DROID_ACTION::ATTACK:
    if (weaponList[0].nStat == 0 || isTransporter() || psAction->targetObj == psDroid)
    {
      break;
    }
    if (!hasValidWeapon)
    ...
    if (isElectronic())
    {
      ...
      if (psAction->targetObj->getType == OBJ_STRUCTURE
      ...
      if (bMultiPlayer
          && psAction->targetObj->getType == OBJ_DROID
    ...
    if (((order->type == DROID_ORDER_TYPE::ATTACKTARGET
          || order->type == DROID_ORDER_TYPE::NONE
          || order->type == DROID_ORDER_TYPE::HOLD
          || (order->type == DROID_ORDER_TYPE::GUARD && hasCommander(psDroid))
          || order->type == DROID_ORDER_TYPE::FIRESUPPORT)
         && secHoldActive)
        || (!isVtolDroid(psDroid) && (orderStateObj(psDroid, DROID_ORDER_TYPE::FIRESUPPORT) != nullptr)))
    {
    ...
    else if (actionInsideMinRange(psAction->targetObj, psWeapStats)) // too close?
    {
      ...
      else if (order->type != DROID_ORDER_TYPE::HOLD && secondaryGetState(psDroid, DSO_HALTTYPE) != DSS_HALT_HOLD)
      {
        int pbx = 0;
        int pby = 0;
    ...
    else if (order->type != DROID_ORDER_TYPE::HOLD && secondaryGetState(psDroid, DSO_HALTTYPE) != DSS_HALT_HOLD) // approach closer?
    {
      action = DROID_ACTION::MOVETOATTACK;
      turnOffMultiMsg(true);
      moveDroidTo(psDroid, psAction->targetObj->position.x,
                  psAction->targetObj->position.y);
      turnOffMultiMsg(false);
    }
    else if (order->type != DROID_ORDER_TYPE::HOLD && secondaryGetState(psDroid, DSO_HALTTYPE) == DSS_HALT_HOLD)
    {
  ...
  case DROID_ACTION::BUILD:
    ...
    //ASSERT_OR_RETURN(, order->type == DROID_ORDER_TYPE::BUILD || order->type == DROID_ORDER_TYPE::HELPBUILD || order->type == DROID_ORDER_TYPE::LINEBUILD, "cannot start build action without a build order");
    ...
    break;
  case DROID_ACTION::DEMOLISH:
    ASSERT_OR_RETURN(, order->type == DROID_ORDER_TYPE::DEMOLISH, "cannot start demolish action without a demolish order");
    action = DROID_ACTION::MOVETODEMOLISH;
    actionPos.x = psAction->x;
    actionPos.y = psAction->y;
    ASSERT_OR_RETURN(, (order->psObj != nullptr) && (order->psObj->getType == OBJ_STRUCTURE), "invalid target for demolish order");
  ...
  case DROID_ACTION::REPAIR:
    ...
    setDroidActionTarget(psDroid, psAction->targetObj, 0);
    ASSERT_OR_RETURN(, (psActionTarget[0] != nullptr) && (psActionTarget[0]->getType == OBJ_STRUCTURE),
                     "invalid target for repair order");
    order->psStats = ((Structure *)psActionTarget[0])->stats;
    if (secHoldActive && (order->type == DROID_ORDER_TYPE::NONE || order->type == DROID_ORDER_TYPE::HOLD))
    {
      action = DROID_ACTION::REPAIR;
    }
    else if ((!secHoldActive && order->type != DROID_ORDER_TYPE::HOLD) || (secHoldActive && order->type == DROID_ORDER_TYPE::REPAIR))
    {
  ...
  case DROID_ACTION::OBSERVE:
    ...
    else if (!hasCBSensor(psDroid) && ((!secHoldActive && order->type != DROID_ORDER_TYPE::HOLD) || (secHoldActive && order->type == DROID_ORDER_TYPE::OBSERVE)))
    {
      action = DROID_ACTION::MOVETOOBSERVE;
      moveDroidTo(psDroid, psActionTarget[0]->position.x,
                  psActionTarget[0]->position.y);
  ...
  case DROID_ACTION::FIRESUPPORT:
    action = DROID_ACTION::FIRESUPPORT;
    if (!isVtolDroid(psDroid) && !secHoldActive &&
        order->psObj->getType != OBJ_STRUCTURE)
    {
      moveDroidTo(psDroid, order->psObj->position.x,
                  order->psObj->position.y);		// movetotarget.
    }
    break;
  case DROID_ACTION::SULK:
  ...
  case DROID_ACTION::DROIDREPAIR:
  {
    ...
    else if (((!secHoldActive && order->type != DROID_ORDER_TYPE::HOLD) || (secHoldActive && order->type == DROID_ORDER_TYPE::DROIDREPAIR))
  ...
  case DROID_ACTION::RESTORE:
    ASSERT_OR_RETURN(, order->type == DROID_ORDER_TYPE::RESTORE, "cannot start restore action without a restore order");
    action = psAction->action;
    actionPos.x = psAction->x;
    actionPos.y = psAction->y;
    ASSERT_OR_RETURN(, (order->psObj != nullptr) && (order->psObj->getType == OBJ_STRUCTURE), "invalid target for restore order");
    order->psStats = order->psObj->stats;
    setDroidActionTarget(psDroid, psAction->targetObj, 0);
    if (order->type != DROID_ORDER_TYPE::HOLD)
    {
...
bool Droid::actionVisibleTarget(GameObject* psTarget, int weapon_slot)
{
  ASSERT_OR_RETURN(false, psTarget != nullptr, "Target is NULL");
  ASSERT_OR_RETURN(false, owningPlayer < MAX_PLAYERS, "psDroid->player (%" PRIu8 ") must be < MAX_PLAYERS", owningPlayer);
  if (!psTarget->visible[owningPlayer])
  {
    return false;
  }
  if ((numWeapons == 0 || isVtolDroid()) && visibleObject(psDroid, psTarget, false))
  {
    return true;
  }
...
int Droid::aiDroidRange(int weapon_slot)
{
  int32_t longRange;
  if (droidType == DROID_TYPE::SENSOR)
  {
    longRange = sensorRange();
  }
  else if (numWeapons == 0 || weaponList[0].nStat == 0)
  {
  ...
  else
  {
    ...
    longRange = proj_GetLongRange(psWStats, owningPlayer);
  }
 ...
 */
GameObject* Droid::orderStateObj(DROID_ORDER order)
{
  bool	match = false;
  ...
  case DROID_ORDER_TYPE::LINEBUILD:
  case DROID_ORDER_TYPE::HELPBUILD:
    if (activeOrder.type == DROID_ORDER_TYPE::BUILD ||
        activeOrder.type == DROID_ORDER_TYPE::HELPBUILD ||
        activeOrder.type == DROID_ORDER_TYPE::LINEBUILD)
    {
      match = true;
    }
    break;
  ...
  case DROID_ORDER_TYPE::REARM:
  case DROID_ORDER_TYPE::GUARD:
    if (activeOrder.type == order)
    {
      match = true;
    }
    break;
  case DROID_ORDER_TYPE::RTR:
    if (activeOrder.type == DROID_ORDER_TYPE::RTR ||
        activeOrder.type == DROID_ORDER_TYPE::RTR_SPECIFIED)
    {
      match = true;
    }
  default:
  ...
  case DROID_ORDER_TYPE::LINEBUILD:
    if (action == DROID_ACTION::BUILD ||
        action == DROID_ACTION::BUILDWANDER)
    {
      return order.psObj;
    }
    break;
...
void Droid::actionDroid(DROID_ACTION action)
{
...
void Droid::actionDroid(DROID_ACTION action, UDWORD x, UDWORD y)
{
  DROID_ACTION_DATA	sAction;
...
void Droid::actionDroid(DROID_ACTION action, GameObject *psObj)
{
  DROID_ACTION_DATA	sAction;
...
void Droid::actionDroid(DROID_ACTION action, GameObject *psObj, UDWORD x, UDWORD y)
{
  DROID_ACTION_DATA	sAction;
...
int Droid::sensorRange()
{
  const int ecmrange = asECMStats[asBits[COMP_ECM]]
                           .upgrade[owningPlayer]
                           .range;
  if (ecmrange > 0) {
    return ecmrange;
  }
...
{
  return (sMove.Status == MOVEINACTIVE ||
          sMove.Status == MOVEHOVER ||
          sMove.Status == MOVESHUFFLE);
}
 ...
 */
bool Droid::orderState(DROID_ORDER order) const
{
  if (order == DROID_ORDER_TYPE::RTR)
  {
    return activeOrder.type == DROID_ORDER_TYPE::RTR || activeOrder.type == DROID_ORDER_TYPE::RTR_SPECIFIED;
  }

  return activeOrder.type == order;
}

bool Droid::turretOnTarget(GameObject *targetObj, Weapon *weapon)
{
  bool b = Unit::turretOnTarget(targetObj, weapon);
  bool bRepair = (droidType == DROID_TYPE::REPAIR);

  int weapon_slot = weapon - weaponList;  // Should probably be passed weapon_slot instead of psWeapon.
  calcDroidMuzzleLocation(psDroid, &attackerMuzzlePos, weapon_slot);

  if (droidType == DROID_TYPE::WEAPON || isTransporter()
      || droidType == DROID_TYPE::COMMAND || droidType == DROID_TYPE::CYBORG
      || droidType == DROID_TYPE::CYBORG_SUPER)
  {
    pitchLowerLimit = DEG(psWeapStats->minElevation);
    pitchUpperLimit = DEG(psWeapStats->maxElevation);
  }
  else if (droidType == DROID_TYPE::REPAIR)
  {
 ...
 */
std::pair<Structure *, DROID_ACTION> Droid::checkForDamagedStruct()
{
  Structure *psFailedTarget = nullptr;
  if (action == DROID_ACTION::SULK)
  {
    psFailedTarget = (Structure *)psActionTarget[0];
  }

  unsigned radius = ((activeOrder.type == DROID_ORDER_TYPE::HOLD) || (activeOrder.type == DROID_ORDER_TYPE::NONE && secondaryGetState(psDroid, DSO_HALTTYPE) == DSS_HALT_HOLD)) ? REPAIR_RANGE : REPAIR_MAXDIST;
  ...
  for (GameObject *object : gridStartIterate(position.x, position.y, radius))
  {
    ...
    if (structure == nullptr ||  // Must be a structure.
        ...
        !aiCheckAlliances(owningPlayer, structure->owningPlayer) ||  // Must be a friendly structure.
    ...
    if (structure->status == Structure::BUILT && structIsDamaged(structure))
    {
      bestDistanceSq = distanceSq;
